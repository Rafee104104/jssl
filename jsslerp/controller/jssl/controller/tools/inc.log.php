...
`ip_address`, `tr_from`, `tr_no`, `tr_id`, `tr_type`,`execution_time`) VALUES 

('".$_SESSION['user']['access_id']."','".$_SESSION['user']['id']."', '".$_SESSION['user']['fname']."','".$_SESSION['user']['level']."', '".$module_id."', '".$page_id."', '".$page_name."', '".$page_link."', '".session_id()."', 
'".$ipaddress."',  '".$tr_from."' , '".$tr_no."' ,'".$tr_id."' ,'".$tr_type."',  '".$execution_time."')";
mysql_query($sql);
...
setcookie($c_name, $c_value, time() + (86400 * 1000), "/");

}

function insert_valid_access_record($company_id,$username,$password,$mod_id)
{
$c_name  = $company_id;
$access_date = date('Y-m-d');
$session_id = session_id();
...
<?
    ...
    if($count>0)
    {
        ...
        { if($m==4) $m=1; else $m++;
            ...
            echo '<div class="silverheader mhafuz'.$m.'"><a href="#"><i class="'; echo ($menu->icon!='')?$menu->icon:'fa fa-cubes'; echo '" aria-hidden="true"></i> '.$menu->feature_name.'<span ></span></a></div>';
...
{
?>
<h6 style="background-color:#4f64a5;color:white;padding:10px;text-align:center;"><a href="../../../<?=$module_data->module_link;?>"  style="color:white!important;" target="_blank"><?=$module_data->module_name; ?></a></h6>
<?
}
?>
