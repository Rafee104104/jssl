...
$con .= 'and b.service_id="'.$_POST['type'].'"';
 ...
 $res='select  c.*,b.*,t.type from service_customer c,bill_assign b,acc_bill_type t where c.customer_id=b.customer and b.service_id=t.id  '.$con.' order by c.customer_name asc';
$query = mysql_query($res);
$i=1;
while($data=mysql_fetch_object($query))
...
$res='select  b.*,c.customer_name,u.fname,t.type from bill_info b left join acc_bill_type t on t.id=b.service_type,service_customer c,user_activity_management u where b.customer=c.customer_id and b.entry_by=u.user_id  '.$con.' group by b.bill_no';
$query = mysql_query($res);
$i=1;
while($data=mysql_fetch_object($query))
...
echo $rec='select j.jv_date,j.tr_no, sum(j.cr_amt) as received_amt,d.customer_name from journal j,service_customer d where d.ledger_id=j.ledger_id '.$con.' group by j.ledger_id,j.tr_no';
$query = mysql_query($rec);
$i=1;
while($data=mysql_fetch_object($query))
...
$qrry = mysql_query($lfsql);
while($license=mysql_fetch_object($qrry)){
...
$mcqrry = mysql_query($mcsql);
while($monthly=mysql_fetch_object($mcqrry)){
...
$pqrry = mysql_query($psql);
while($bill_payable=mysql_fetch_object($pqrry)){
...
$rqrry = mysql_query($rsql);
while($bill_receive=mysql_fetch_object($rqrry)){
...
$pqrry = mysql_query($lpsql);
while($lbill_payable=mysql_fetch_object($pqrry)){
...
$lqrry = mysql_query($lsql);
while($lbill_receive=mysql_fetch_object($lqrry)){
...
$moqrry = mysql_query($mosql);
while($mopening=mysql_fetch_object($moqrry)){
...
$loqrry = mysql_query($losql);
while($lopening=mysql_fetch_object($loqrry)){
