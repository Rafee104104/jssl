...
$table = 'requisition_master_cfr';
$unique = 'req_no';
$status = 'UNCHECKED';
$target_url = '../cfr/cfr_print_view.php';
                    ...
                    <div class="form-group row m-0">
                        ...
                        <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0">
						 <select name="status" id="status">
							...
							<option value="manual" <?= ($_POST['status'] == 'manual') ? 'selected' : '' ?>>MANUAL</option>
							<option value="approve1_id" <?= ($_POST['status'] == 'approve1_id') ? 'selected' : '' ?>>APPROVAL1</option>
							<option value="approve2_id" <?= ($_POST['status'] == 'approve2_id') ? 'selected' : '' ?>>APPROVAL2</option>
							<option value="approve2_id" <?= ($_POST['status'] == 'approve3_id') ? 'selected' : '' ?>>APPROVAL3</option>
							<option value="approve2_id" <?= ($_POST['status'] == 'approve4_id') ? 'selected' : '' ?>>APPROVAL4</option>
							<option value="COMPLETE" <?= ($_POST['status'] == 'COMPLETE') ? 'selected' : '' ?>>COMPLETED</option>
...
<div class="container-fluid pt-5 p-0 ">
		...
		<? 
					...
					$res='select  a.req_no, a.req_date, a.req_for, b.warehouse_name as warehouse, a.req_note as note, a.need_by, c.fname as entry_by, a.entry_at,a.status from requisition_master_cfr a,warehouse b,user_activity_management c where  a.warehouse_id=b.warehouse_id and a.entry_by=c.user_id '.$con.'  order by a.req_no';
