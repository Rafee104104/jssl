	...
	while($row=mysql_fetch_object($rs)){
		...
		?>
		  <tr>
                ...
                <td><?=$stock=(int)find_a_field('journal_item','sum(item_in-item_ex)','item_id="'.$row->item_id.'" and warehouse_id='.$row->warehouse_id);
                ...
                <td><?=$row->order_qty?></td>
                <td>
					<input type="number" step="any" id="id<?=$row->id?>" name="id<?=$row->id?>" value="<?=ceil($row->qty);?>" required/>
                </td>
            </tr>
		<?
	}
?>
    ...
    <!--button design start-->
      ...
      <div class="container-fluid p-0 ">
        ...
        <div class="n-form-btn-class">
			  <input name="return" type="submit" class="btn1 btn1-bg-cancel" value="RETURN AND SEND PI"/>
