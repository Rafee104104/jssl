...
$title='Delivery Challan Approved List';
        ...
        <div class="container-fluid pt-5 p-0 ">
            <table class="table1  table-striped table-bordered table-hover table-sm">
				...
				if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and c.chalan_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
					...
					$res="select c.chalan_no,m.customer, c.chalan_date,c.booking_no,c.sr_no,sum(c.total_unit) as unit,  m.do_no, m.job_no,  m.do_date, m.dealer_code,  d.dealer_name_e, c.entry_by, c.status from 
				sale_do_master m, sale_do_chalan c, dealer_info d, user_group u 
				where 
				
				 m.group_for=u.id and c.aprroved_by3>0  and m.dealer_code=d.dealer_code and m.do_no=c.do_no ".$group_for_con.$con.$dealer_con.$job_no_con.$chalan_con."  group by c.chalan_no order by  c.chalan_date DESC, m.do_no,c.chalan_no ";
				...
				?>
				  <?=$data2->sr_no.", ";?>
				  <? } ?>	</td>
				  <td><?=$data->unit; $tot_qty+=$data->unit;?></td>
