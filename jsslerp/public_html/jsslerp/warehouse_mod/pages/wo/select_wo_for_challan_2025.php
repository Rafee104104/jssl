...
$target_url = '../wo/delivery_challan_2025.php';
...
$con .= ' and m.do_no in ('.$_POST['do_no'].') ';*/
 ...
 $res="select  m.do_no,d.booking_no,d.sr_no, m.job_no, m.dealer_code, m.do_date,  m.status, sum(d.dist_unit) as wo_qty,m.status,m.customer
 
 
  from sale_do_master m, sale_do_details d 
  
 
 where m.do_no=d.do_no and  m.status  in ('PROCESSING') ".$con." group by m.do_no order by  m.do_no asc ";
		...
		?>
          <?
					...
					while($row = mysql_fetch_object($query)){
						$chalan_qty=find_a_field('sale_do_chalan','sum(total_unit)','do_no='.$row->do_no);
					?>
          <tr>
            <td><?=$row->do_no?></td>
            <td><?=$row->do_date?></td>
			<td><?=$row->booking_no?></td>
            <td><?=find_a_field('paid_booking','name','booking_number_eng="'.$row->booking_no.'"');?></td>
            <td><?=find_a_field('paid_booking','mobile_no','booking_number_eng="'.$row->booking_no.'"');?></td>
            <td> <?
				  $res2="select c.do_no, c.do_date,  c.booking_no,c.sr_no from 
				sale_do_details c
				where 
				...
				?>
				  <?=$data2->sr_no.", ";?>
				  <? } ?>	</td>
			<td><?=$total_qty=$row->wo_qty-$chalan_qty; $tot_qty+=$total_qty;?></td>
            <td><?=$row->job_no?></td>
            <td <?=$row->do_no?>><?=$row->customer; //find_a_field('paid_booking','father_name','booking_number_eng="'.$row->booking_no.'"');?></td>
            <td><?=$row->status?></td>
            <td><input type="button" value="Complete SR" onClick="custom(<?=$row->do_no;?>);" class="btn1 btn1-bg-submit" / >            </td>
          </tr>
          <?
						}
						?>
