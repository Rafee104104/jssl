...
function calculateDays(val) {
	...
	var balance = (document.getElementById('balance_'+val).value)*1;
	var balance2 = (document.getElementById('balance2_'+val).value)*1;
	...
	var loanDate = new Date(document.getElementById('loan_date_'+val).value);
		...
		tot_bal=balance-balance2;
		
		if (sr_loan_x == balance) {
			var totalSRInterest = 0;
		}
		else if ( sr_loan_x>balance2) {
			var totalSRInterest =  sr_loan_x-tot_bal;
		} 
		else if(sr_loan_x <= balance )
		{
          ...
          <div class="container n-form2">
			...
			$max_chalan_no = find_a_field('sale_do_chalan','max(chalan_no)','1')+1;
              ...
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
			   ...
			   ?>
                <input name="sr_loan" style="color:#28a745;" id="sr_loan" value="<?=$received=find_a_field(' sr_loan l, journal j','sum(j.dr_amt)','l.sr_loan_id=j.tr_no and j.tr_from="SR Loan"  and l.booking_number ="' . $booking . '" and j.dr_amt>0 and ledger_id='.$acc_ledger);?>"  type="text" />
			...
			<div class="form-group row m-0 pb-1">
              ...
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
                ...
                <input name="chalan" type="text" id="chalan" required="required" value="<?=$paid=$tot_paid_amt ?> "/>
			...
			<div class="form-group row m-0 pb-1">
              ...
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
				...
				<input name="chalan_date" type="text" style="color:#dc3545;" id="chalan_date" required="required" value="<? $loan_due=$received-$paid; if($loan_due>0) { echo  $loan_due;}?> ">
        ...
        <div class="col-sm-4 col-md-4 col-lg-4 col-xl-4">
          <div class="container n-form2">
			...
			<div class="form-group row m-0 pb-1">
              ...
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
                <input  name="rec_intest" type="text" style="color:#007bff;" value="<? $paid_interest=($received-$paid)*(-1); if($paid_interest>0) { echo $paid_inter=$paid_interest;}?>" id="rec_intest" />
              </div>
            </div>
			<div class="form-group row m-0 pb-1">
        ...
        <thead class="thead1">
          <tr class="bgc-info">
		   ...
		   <th>Challan Issue</th>
            ...
            <th>Pend. </th>
            <th>Seeds Loan</th>
            <th width="10%">Bag Loan</th>
            <th width="10%">Farmer Loan</th>
            <th width="10%">SR Loan</th>
            <th width="10%">Others Loan</th>
            <!--<th>Loan Days</th>-->
			      <!--<th>Loan Paid</th>-->
            <th>Interest</th>
            <th>Cash Ledger </th>
        ...
        <tbody class="tbody1">
          ...
          <tr>
		   ...
		   <? if($unso_qty>=0){ $cow++;?>
            ...
            <td align="center"><input type="text" style="width:65px !important; font-size:3px;" name="seeds_loan_<?=$row->id?>" id="seeds_loan_<?=$row->id?>" onKeyUp="loan_calculate(<?=$row->id?>);" value="<? //$loan22->seeds_loan; $tot_seeds_loan+=$loan22->seeds_loan;?>"/>
            ...
            <td align="center" width="10%"><input type="text" style="width:65px !important; font-size:3px;" name="bag_loan_<?=$row->id?>" id="bag_loan_<?=$row->id?>" onKeyUp="loan_calculate(<?=$row->id?>);" />
			<input type="hidden" name="bag_loans_<?=$row->id?>" id="bag_loans_<?=$row->id?>" value="<?=$loan22->bag_loan; $tot_bag_loan+=$loan22->bag_loan;?>"/></td>
            <td align="center" width="10%"><input type="text" style="width:65px !important; font-size:3px;" name="farmer_loan_<?=$row->id?>" id="farmer_loan_<?=$row->id?>" onKeyUp="loan_calculate(<?=$row->id?>);" />
            ...
            <td align="center"><input type="text" style="width:65px !important; font-size:3px;" name="sr_loan_<?=$row->id?>" id="sr_loan_<?=$row->id?>" onKeyUp="loan_calculate(<?=$row->id?>);" />
			<input type="hidden" name="sr_loans_<?=$row->id?>" id="sr_loans_<?=$row->id?>" value="<?=$loan22->sr_loan; $tot_sr_loan+=$loan22->sr_loan;?>"/></td>
            <td align="center"><input type="text" style="width:65px !important; font-size:3px;" name="others_loan_<?=$row->id?>" id="others_loan_<?=$row->id?>" onKeyUp="loan_calculate(<?=$row->id?>);" />
			...
			<input type="hidden" name="balance_<?=$row->id?>" id="balance_<?=$row->id?>" value="<?=find_a_field(' sr_loan l, journal j','sum(j.dr_amt)','l.sr_loan_id=j.tr_no and j.tr_from="SR Loan"  and l.booking_number ="' . $booking . '" and j.dr_amt>0 and ledger_id='.$acc_ledger);?>"/>
			<input type="hidden" name="balance2_<?=$row->id?>" id="balance2_<?=$row->id?>" value="<?=find_a_field('sale_do_chalan c,journal j','SUM(DISTINCT j.cr_amt)','c.chalan_no=j.tr_no and j.tr_from="Sales" and c.booking_no ="' . $booking . '" and j.ledger_id="'.$acc_ledger.'" ');?>"/>
			
			<input type="hidden" name="others_loans_<?=$row->id?>" id="others_loans_<?=$row->id?>" value="<?=$loan22->others_loan; $tot_others_loan+=$loan22->others_loan;?>"/></td>				<input name="loan_days_<?=$row->id?>" type="hidden" id="loan_days_<?=$row->id?>" value="" readonly />
           <!-- <td align="center"><input name="loan_days_<?=$row->id?>" type="hidden" id="loan_days_<?=$row->id?>" value="" readonly />-->
			<input name="loan_date_<?=$row->id?>" id="loan_date_<?=$row->id?>" type="hidden" value="<?=$startDate?>"  />
             ...
             <td align="center"><input type="text" style="width:65px !important; font-size:3px;" name="total_interest_<?=$row->id?>" id="total_interest_<?=$row->id?>" onKeyUp="loan_calculate(<?=$row->id?>);"  /></td>
             <td align="center">
