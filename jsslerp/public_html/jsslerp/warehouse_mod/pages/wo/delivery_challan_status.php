        ...
        <div class="container-fluid pt-5 p-0 ">
            <table class="table1  table-striped table-bordered table-hover table-sm">
				...
				if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and c.chalan_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
					...
					$res="select c.chalan_no, c.chalan_date,  m.do_no, m.job_no,c.booking_no,c.sr_no, c.rec_name, m.do_date, m.dealer_code,m.customer,sum(c.total_unit) as wo_qty,  d.dealer_name_e, c.entry_by, c.status from 
				sale_do_master m, sale_do_chalan c, dealer_info d, user_group u 
				where 
				
				 m.group_for=u.id  and m.dealer_code=d.dealer_code and m.do_no=c.do_no ".$group_for_con.$con.$dealer_con.$job_no_con.$chalan_con."  group by c.chalan_no order by  c.chalan_no DESC, m.do_no,c.chalan_no ";
				...
				<tr <?=($data->RCV_AMT>0)?'style="background-color:#FFCCFF"':'';?>>
				  <td><a title="Challan Preview" target="_blank" href="challan_invoice_print_view.php?v_no=<?=$data->chalan_no?>" style=" font-weight:700; color:#000000" >
					<?=$data->chalan_no;?>
				  </a></td>
				...
				?>
				  <?=$data2->sr_no.", ";?>
				  <? } ?>				  </td>
				  <td><?=$data->wo_qty; $tot_qty+=$data->wo_qty;?></td>
				  <td><?=$data->rec_name;?></td>
				  <td><?=$data->customer;?></td>
				<td><?=$data->status;?></td>
				</tr>
