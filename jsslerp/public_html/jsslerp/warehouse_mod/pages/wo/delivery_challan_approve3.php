...
$title='Delivery Challan Unapproved';
        ...
        <div class="container-fluid pt-5 p-0 ">
            <table class="table1  table-striped table-bordered table-hover table-sm">
				...
				if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and c.chalan_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
					...
					$res="select c.chalan_no,m.customer,sum(c.total_unit) as unit, c.chalan_date,  m.do_no, m.job_no,  m.do_date, m.dealer_code,  d.dealer_name_e, c.entry_by,c.booking_no, c.status,c.rec_name from 
				sale_do_master m, sale_do_chalan c, dealer_info d, user_group u 
				where 
				
				 m.group_for=u.id and c.approved_status=''  and m.dealer_code=d.dealer_code and m.do_no=c.do_no ".$group_for_con.$con.$dealer_con.$job_no_con.$chalan_con."  group by c.chalan_no order by  c.chalan_date asc ";
				...
				<tr <?=($data->RCV_AMT>0)?'style="background-color:#FFCCFF"':'';?>>
				  ...
				  <td> <?
				  $res2="select c.chalan_no, c.chalan_date,  c.booking_no,c.sr_no from 
				sale_do_chalan c
				where 
				
				  c.chalan_no='".$data->chalan_no."' ";
				...
				?>
				  ...
				  <? } ?>	</td>
				  <td><?=$data->unit;?></td>
				  <td><a title="Challan Preview" target="_blank" href="delivery_challan_approved_view3.php?v_no=<?=$data->chalan_no?>" style=" font-weight:700; color:#000000" >View</a></td>
				</tr>
				<?
				...
				if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and c.chalan_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
					...
					$res="select c.chalan_no, c.chalan_date, sum(c.total_unit) as unit,m.customer, m.do_no, m.job_no,  m.do_date, m.dealer_code,  d.dealer_name_e, c.entry_by,c.booking_no, c.status,c.rec_name from 
				sale_do_master m, sale_do_chalan c, dealer_info d, user_group u 
				where 
				
				 m.group_for=u.id and c.approved_status=''  and m.dealer_code=d.dealer_code and m.do_no=c.do_no ".$group_for_con.$dealer_con.$job_no_con.$chalan_con."  group by c.chalan_no order by  c.chalan_date asc ";
				$query = mysql_query($res);
