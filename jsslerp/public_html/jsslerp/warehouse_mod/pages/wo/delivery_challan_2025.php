...
if(isset($_POST['return'])){
		...
		echo '<script>window.location.replace("select_wo_for_challan_2025.php")</script>';
...
if(isset($_POST['confirm'])){
		...
		while($data=mysql_fetch_object($query)){
			...
			if($_POST['chalan_'.$data->id]>0 && $_POST['chalan_'.$data->id] <= $_POST['stk_qty_'.$data->id]){
				...
				$challan_per_kg_price =$_POST['challan_per_kg_price_'.$data->id];
				$challan_in_kg =$_POST['challan_in_kg_'.$data->id];
...
$do_qty=find_a_field('sale_do_details','sum(dist_unit)','do_no='.$_GET['do_no']);
$chalan_qty=find_a_field('sale_do_chalan','sum(total_unit)','do_no='.$_GET['do_no']);

if($do_qty==$chalan_qty)
{

 $update="update sale_do_master set status='COMPLETED' where do_no='".$_GET['do_no']."'";
mysql_query($update);
}
...
header('Location: select_wo_for_challan_2025.php');
}
...
<div class="form-container_large">
  ...
  <div class="modal fade" id="loanDetailsModal" tabindex="-1" aria-labelledby="loanDetailsModalLabel" aria-hidden="true">
					<div class="modal-dialog modal-lg">
						<div class="modal-content shadow-lg rounded">
							...
							<div class="modal-body bg-light">
								<h5 class="section-heading"><i class="bi bi-journal-text"></i> SR Loan Details</h5>
				...
				//                                echo "<th></th>";
							...
							<div class="modal-body bg-light">
								<h5 class="section-heading"><i class="bi bi-journal-text"></i> Bag Loan Details</h5>
								...
								<div class="table-responsive">
									<table class="table table-striped table-bordered table-hover">
										...
										<tbody>
											...
											$booking=find_a_field('sale_do_details','booking_no','do_no='.$$unique);
										 	$sqlDate = 'SELECT  l.date, l.interest_amt, l.amount FROM bag_loan l, journal j WHERE l.sr_loan_id=j.tr_no and j.tr_from="Bag Sales"  and l.booking_number ="' . $booking . '" and j.cr_amt>0 group by l.sr_loan_id ORDER BY l.date ASC';
											$sqlQuery = mysql_query($sqlDate);
											$tot_interest = 0;
									...
									<table class="table table-striped table-bordered table-hover">
									 ...
									 $dealer_code=find_a_field('sale_do_details','dealer_code','do_no='.$$unique);
									   ...
									   $acc_ledger=find_a_field('dealer_info','account_code','dealer_code='.$dealer_code);
										   ...
										   $sqlDate = 'SELECT c.chalan_date, j.cr_amt,total_interest FROM sale_do_chalan c,journal j WHERE c.chalan_no=j.tr_no and j.tr_from="Sales" and c.booking_no ="' . $booking . '" and j.ledger_id="'.$acc_ledger.'" and j.cr_amt>0 group by j.tr_no ORDER BY c.chalan_date ASC';
													$sqlQuery = mysql_query($sqlDate);
													$tot_paid_amt = 0;
													...
													while ($sqlResult = mysql_fetch_object($sqlQuery)) {
														...
														$tot_paid_interest += $sqlResult->total_interest; // Accumulate total paid interest
				...
				//										echo "<td class='text-success'>" . number_format($perDayInterest, 2) . "</td>";
														echo "<td class='text-danger fw-bold'>" . number_format($sqlResult->total_interest, 2) . "</td>";
														echo "</tr>";
      ...
      <div class="row">
        ...
        <div class="col-sm-4 col-md-4 col-lg-4 col-xl-4">
		  ...
		  <div class="form-group row m-0 pb-1">
              ...
              <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Loan Date </label>
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
			  <?
			   ...
			   $booking=find_a_field('sale_do_details','booking_no','do_no='.$$unique);
			    	$sr_sql='select date as sr_date,amount from sr_loan where booking_number="'.$booking.'"  ';
					$r_query=mysql_query($sr_sql);
			  
			   ?>
			  	
				<select name="loan_date" id="loan_date" value="<?=$loan_date?>">
        ...
        echo "<option value='{$r->sr_date}' $selected>{$r->sr_date}   #> {$r->amount}&#2547;</option>";
			...
			$max_chalan_no = find_a_field('sale_do_chalan','max(chalan_no)','1')+1;
			   ...
			   $ch_data = find_all_field('sale_do_chalan','','chalan_no='.$_SESSION['chalan_no']);?>
              <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center mr-3 bg-form-titel-text" style="color:#28a745;">Loan Received:  </label>
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
			  
			  	 
                <input name="sr_loan" style="color:#28a745;" id="sr_loan" value="<? $receiveds=find_a_field(' sr_loan l, journal j','sum(j.dr_amt)','l.sr_loan_id=j.tr_no and j.tr_from="SR Loan"  and l.booking_number ="' . $booking . '" and j.dr_amt>0 and ledger_id='.$acc_ledger);   
				$bag_loans=find_a_field('bag_loan l, journal j','sum(j.cr_amt)','l.sr_loan_id=j.tr_no and j.tr_from="Bag Sales"  and l.booking_number ="' . $booking . '" and j.cr_amt>0 and ledger_id=3120010012'); echo $received=$bag_loans+$receiveds; ?>"  type="text" />
        ...
        <div class="col-sm-4 col-md-4 col-lg-4 col-xl-4">
          <div class="container n-form2">
			...
			<!--<div class="form-group row m-0 pb-1">
              <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text" style="color:#28a745;" >Rec Interest: </label>
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
                <input  name="rec_intest" style="color:#28a745;" type="text" value="<?=number_format($tot_interest=$tot_interest-$tot_paid_interest,2); ?>" id="rec_intest" />
              </div>
            </div>-->
			<div class="form-group row m-0 pb-1">
              <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text" style="color:#007bff;">Paid Interest : </label>
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
                <input  name="rec_intest" type="text" style="color:#007bff;" value="<? $dir_inter=find_a_field('sale_do_chalan c,journal j','sum(c.total_interest)','c.chalan_no=j.tr_no and c.booking_no ="' . $booking . '" and tr_from="Sales" and j.ledger_id="'. $acc_ledger.'" and j.cr_amt>0'); 
				$paid_sr_loan_separately_interest=find_a_field('sr_loan_return r,paid_booking p','sum(r.interest_amt)','r.booking_number=p.booking_number_eng and r.booking_number="'.$booking.'"  and r.chalan_no is null ');
			echo	$total_interst=$dir_inter+$paid_sr_loan_separately_interest;
				?>" id="rec_intest" />
              </div>
            </div>
			...
			<div class="form-group row m-0 pb-1">
              ...
              <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text" style="color:#007bff;">Loan Paid: </label>
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
			  
			  	
                <input name="chalan" type="text" id="chalan" required="required" value="<? 
				 $sr_loan_paid=list($year, $month, $day) = explode("-", $f_date);
		         $paid_sr_loan_separately=find_a_field('sr_loan_return r,paid_booking p','sum(r.total_paid)','r.booking_number=p.booking_number_eng and r.booking_number="'.$booking.'"  and r.chalan_no is null '); 
				 
			echo	 $paid=$tot_paid_amt+$paid_sr_loan_separately;
				  ?> "/>
			...
			<!--<div class="form-group row m-0 pb-1">
              <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text" style="color:#dc3545;">Due Interest : </label>
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
                <input  name="rec_intest" style="color:#dc3545;"" type="text" value="<?=number_format($interest_due=$tot_interest-$paid_inter,2);?>" id="rec_intest" />
              </div>
            </div>-->
			
			<div class="form-group row m-0 pb-1">
              <label  class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text"></label>
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
        ...
        <div class="col-sm-4 col-md-4 col-lg-4 col-xl-4">
          <div class="container n-form2">
			...
			<div class="form-group row m-0 pb-1">
              ...
              <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text" style="color:#dc3545;">Loan Due:  </label>
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
			  
			  	
				<input name="due_loan" type="text" style="color:#dc3545;" id="due_loan" required="required" value="<? $loan_due=$received-$paid; if($loan_due>0) { echo  $loan_due;}?> ">
				
				
              </div>
            </div>
            <!--<div class="form-group row m-0 pb-1">
              <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text" style="color:#28a745;">Total Rec: </label>
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
			  
			  
                <input style="color:#28a745;" name="rec_intest" id="rec_intest" type="text" value="<?=number_format($total_amt=$tot_interest+$received,2)?>" />
              </div>
            </div>-->
			<!--<div class="form-group row m-0 pb-1">
              <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text" style="color:#007bff;">Total Paid: </label>
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
			  
			  
                <input name="rec_intest" id="rec_intest" style="color:#007bff;" type="text" value="<?=number_format($paid,2);?>"  />
              </div>
            </div>-->
			<!--<div class="form-group row m-0 pb-1">
              <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text" style="color:#dc3545;">Total Due: </label>
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
			  
			 
                <input name="rec_intest" id="rec_intest" style="color:#dc3545;" type="text" value="<?=number_format($total_amt-$paid,2); ?>"  />
              </div>
            </div>-->
			<!--
            <div class="form-group row m-0 pb-1">
        ...
        <thead class="thead1">
          <tr class="bgc-info">
		   ...
		   <th>Challan Issue</th>
            ...
            <th style="width:120px !important;">Challan Issue Kg </th>
            <th>Per Kg Price </th>
            <th>Store Rent </th>
            <th>Labour Charge </th>
            <th>Stock Qty</th>
            ...
            <th>Bag Loan</th>
            <th>Farmer Loan</th>
            <th style="width:100px !important;">SR Loan</th>
            <th>Others Loan</th>
            <th>Loan Days</th>
			      <!--<th>Loan Paid</th>-->
            <th>Interest</th>
        ...
        <tbody class="tbody1">
          ...
          <tr>
            ...
            <td style="text-align:left"><?=$row->booking_no?>
			  ...
			  <input type="hidden" id="booking_type_<?=$row->id?>" name="booking_type_<?=$row->id?>" value="<?=$all_booking->booking_type?>"  /> 
		   ...
		   <? if($unso_qty>=0){    $cow++;?>
              <input name="chalan_<?=$row->id?>" type="text" id="chalan_<?=$row->id?>" value=""   onKeyUp="alldatacaculate(<?=$row->id?>);" />
			   ...
			   <td align="center"><input type="text" name="challan_in_kg_<?=$row->id?>" id="challan_in_kg_<?=$row->id?>" onKeyUp="alldatacaculate(<?=$row->id?>);" value="<? //$all_booking->total_amount;?>" style="width:120px !important;" />
			</td>
			
			<td align="center"><input type="text" name="challan_per_kg_price_<?=$row->id?>" id="challan_per_kg_price_<?=$row->id?>" value="<? if($all_booking->booking_type=='Paid Booking') { echo $all_booking->booking_rate; }else { echo  $all_booking->per_kg_price;}?>" style="width:60px !important;" />
			</td>
			   
            <td align="center"><input type="text" readonly="readonly" name="store_rent_<?=$row->id?>" id="store_rent_<?=$row->id?>" value="<? //$all_booking->total_amount;?>" style="width:60px !important;" />
            ...
            <td align="center"><input type="text" name="bag_loan_<?=$row->id?>" id="bag_loan_<?=$row->id?>"  />
            ...
            <td align="center"><input type="text" name="sr_loan_<?=$row->id?>" id="sr_loan_<?=$row->id?>" onKeyUp="loan_calculate(<?=$row->id?>);" style="width:100px !important;" />
			...
			<input name="loan_date_<?=$row->id?>" id="loan_date_<?=$row->id?>" type="hidden" value="<?=$startDate?>"  />
			  ...
			  <input name="interest_per_<?=$row->id?>" readonly="readonly" id="interest_per_<?=$row->id?>" type="hidden" value="<?=$loan_info->interest_per?>"  />		</td>
             ...
             <td align="center"><input type="text" readonly="readonly" name="total_interest_<?=$row->id?>" id="total_interest_<?=$row->id?>" onKeyUp="loan_calculate(<?=$row->id?>);"  /></td>
             <td align="center">
            ...
            <!--<td align="center"><? if($unso_qty>0){ $cow++;?>
              <input name="chalan_<?=$row->id?>" type="text" id="chalan_<?=$row->id?>" value=""   onKeyUp="alldatacaculate(<?=$row->id?>);" />
              <? } else echo 'Done';?>            </td>-->
			  <td align="center"><input type="text" readonly="readonly" name="total_amt_<?=$row->id?>" id="total_amt_<?=$row->id?>" value="" style="width:70px !important"/></td>
          </tr>
          <? }?>
		  <!--<tr>
...
function calculateDays(val) {
	...
	var challan_in_kg = (document.getElementById('challan_in_kg_'+val).value)*1;
	var challan_per_kg_price = (document.getElementById('challan_per_kg_price_'+val).value)*1;
	
	 var booking_type = (document.getElementById('booking_type_'+val).value);
  ...
  if(booking_type=='Paid Booking')
	{
	var tot_store_rent = parseFloat((challan_per_kg_price * challan_qty).toFixed(2)) || 0;
	var tot_labour_charge = parseFloat(((challan_in_kg * 0.50)).toFixed(2)) || 0;
	}
	else
	{
	var tot_store_rent = parseFloat((challan_per_kg_price * challan_in_kg).toFixed(2)) || 0;
	var tot_labour_charge = 0;
	}
	...
	document.getElementById('bag_loan_'+val).value=bag_loan;
	...
	document.getElementById('others_loan_'+val).value=tot_others_loan;
	 document.getElementById('loan_days_'+val).value = daysDifference;
	 document.getElementById('labour_charge_'+val).value=(tot_labour_charge % 1 >= 0.51) ? Math.ceil(tot_labour_charge) : Math.floor(tot_labour_charge);
	 document.getElementById('store_rent_'+val).value=(tot_store_rent % 1 >= 0.51) ? Math.ceil(tot_store_rent) : Math.floor(tot_store_rent);
	  document.getElementById('total_interest_'+val).value= (totinterest % 1 >= 0.51) ? Math.ceil(totinterest) : Math.floor(totinterest); 
	  
	var totAmount = ((totinterest % 1 >= 0.51) ? Math.ceil(totinterest) : Math.floor(totinterest))  + tot_seeds_loan + bag_loan + tot_farmer_loan + tot_sr_loan + tot_others_loan + tot_labour_charge + tot_store_rent;
	
    document.getElementById('total_amt_' + val).value =(totAmount % 1 >= 0.51) ? Math.ceil(totAmount) : Math.floor(totAmount); 
...
//	}
	...
	var total=seeeds_laon+farmeer_loan+srr_loan+otherss_loan+tot_store_rent+tot_labour_charge+totinterest;
	
    document.getElementById('total_amt_' + val).value =(total % 1 >= 0.51) ? Math.ceil(total) : Math.floor(total); 
