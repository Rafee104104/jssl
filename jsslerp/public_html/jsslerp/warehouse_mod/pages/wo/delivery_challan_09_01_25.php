...
//  $tot_cr_amt=$amount;
//  //DR
//	add_to_sec_journal($proj_id, $jv_no, $jv_date, $dr_ledger, $dnarr, $total_store_rent_actual, '0', $tr_from, $tr_no,'',$tr_id,$cc_code,'',$user_id,'',$r_from, $bank,$c_no,$cheq_date,$issue_from_ac,$checked,$type,$employee,$remarks,$reference_id);
//	
//	//DR cash ledger
//	add_to_sec_journal($proj_id, $jv_no, $jv_date, $cash_ledger, $dnarr, $total_cash, '0', $tr_from, $tr_no,'',$tr_id,$cc_code,'',$user_id,'',$r_from, $bank,$c_no,$cheq_date,$issue_from_ac,$checked,$type,$employee,$remarks,$reference_id);
//	
//	//CR
...
//	if($tot_interest>0)
//	{
//	add_to_sec_journal($proj_id, $jv_no, $jv_date, $cr_interest, $dnarr, '0', $tot_interest, $tr_from, $tr_no,'',$tr_id,$cc_code,'',$user_id,'',$r_from, $bank,$c_no,$cheq_date,$issue_to_ac,$checked,$type,$employee,$remarks,$reference_id);
//	
//	}
//	
//	//Agent Cr
      ...
      <table class="table1  table-striped table-bordered table-hover table-sm">
        ...
        <tbody class="tbody1">
          ...
          <tr>
           ...
           -->
		   <td align="center">
             <input type="text" id="chalan_<?= $row->id ?>" onkeyup="alldatacaculate(<?= $row->id ?>)">
                         </td>
            ...
            <td align="center"><input type="text" id="loan_days_<?= $row->id ?>" onkeyup="alldatacaculate(<?= $row->id ?>)">
			<input name="loan_date_<?=$row->id?>" id="loan_date_<?=$row->id?>" type="hidden" value="<?=$startDate?>"  />
             ...
             <td align="center"><input type="text" name="total_interest_<?=$row->id?>" id="total_interest_<?=$row->id?>"   /></td>
             <td align="center">
...
//    }
	...
	var daysDifference = (document.getElementById('loan_days_'+val).value)*1; 
	var rate = (document.getElementById('rate_'+val).value)*1; 
 
	 	
		
		var tot_seeds_loan = parseFloat((rate * challan_qty).toFixed(2)) || 0;
    var tot_bag_loan = parseFloat((rate * challan_qty).toFixed(2)) || 0;
    var tot_farmer_loan = parseFloat((rate * challan_qty).toFixed(2)) || 0;
    var tot_sr_loan = parseFloat((rate * challan_qty).toFixed(2)) || 0;
    var tot_others_loan = parseFloat((rate * challan_qty).toFixed(2)) || 0;
	...
	document.getElementById('others_loan_'+val).value=tot_others_loan;
	 ...
	 document.getElementById('store_rent_'+val).value=tot_store_rent;
	  document.getElementById('total_interest_'+val).value=totinterest.toFixed(2);
    ...
    document.getElementById('total_amt_' + val).value = totAmount.toFixed(2);
...
//	}
//	
//	var total=seeeds_laon+baag_loan+farmeer_loan+srr_loan+otherss_loan+tot_store_rent+tot_labour_charge;
//	
//    document.getElementById('total_amt_' + val).value =total.toFixed(2);
//	   
//	
//    
//	
