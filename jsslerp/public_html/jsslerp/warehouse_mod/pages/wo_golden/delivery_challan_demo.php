...
$qty_bag=$_POST['qty_bag_'.$data->id];

$bag_size=$_POST['bag_size_'.$data->id];
...
$cost_price=find_a_field('journal_item','final_price','item_id="'.$item_id.'" and tr_from in ("Purchase","Opening","Production Receive") order by id desc ');

$cost_amt = ($qty*$cost_price); 

$so_invoice = 'INSERT INTO sale_do_chalan (year, ch_id, chalan_no, chalan_date, order_no, do_no, job_no, do_date, item_id, dealer_code, unit_price, pkt_size, pkt_unit, dist_unit, total_unit, total_amt, discount, depot_id, group_for, rec_name, rec_mob,transport_id, vehicle_no, driver_name, driver_mobile, delivery_point, delivery_man, delivery_man_mobile, gluteen,target_cost, entry_by, entry_at, status, cost_price, cost_amt,qty_bag,bag_size)

VALUES("'.$YR.'","'.$ch_cy_id.'","'.$chalan_no.'","'.$ch_date.'","'.$data->id.'","'.$data->do_no.'","'.$data->job_no.'","'.$data->do_date.'","'.$item_id.'","'.$data->dealer_code.'","'.$rate.'","'.$pkt_size.'","'.$pkt_unit.'","'.$qty.'","'.$qty.'","'.$amount.'","'.$discount.'","'.$ms_data->depot_id.'","'.$ms_data->group_for.'","'.$rec_name.'","'.$rec_mob.'","'.$transport_id.'","'.$vehicle_no.'","'.$driver_name.'","'.$driver_mobile.'","'.$delivery_point.'","'.$delivery_man.'","'.$delivery_man_mobile.'","'.$gluteen.'","'.$target_cost.'","'.$entry_by.'","'.$entry_at.'","CHECKED", "'.$cost_price.'", "'.$cost_amt.'","'.$qty_bag.'","'.$bag_size.'")';
...
var stock_qty =((document.getElementById('stock_'+id).value)*1); 
...
<td rowspan="2" align="center" ><a title="WO Preview" target="_blank" href="../../../sales_mod/pages/wo_golden/sales_order_print_view.php?v_no=<?=$$unique?>" ><img src="../../../images/print.png" alt="" width="30" height="30" /></a></td>
...
<input name="vehicle_no" type="hidden" id="vehicle_no" value="<?=$ch_data->vehicle_no;?>" />

<?php /*?>					<input class="form-control" list="browsers" name="transport_id" id="transport_id" value="<?=find_a_field('transport_info','transport_name','transport_id="'.$transport_id.'"')."#".$transport_id;?>">

					<datalist id="browsers">
					  <? foreign_relation('transport_info','concat(transport_number,"#",transport_id)','concat(transport_number,"#",transport_id)',$transport_id,'1');?>
										<?php 
										$t_sql='select * from transport_info';
										$t_query=mysql_query($t_sql);
										while($t_row=mysql_fetch_object($t_query)){
										?>
											<option value="<?=$t_row->transport_number."#".$t_row->transport_id?>">
										<?php } ?>
					</datalist><?php */?>
					
					
					<select name="transport_id" id="transport_id"  onchange="get_delivery_info()">
					<option>-- Please Select --</option>
					<?php 
					$t_sql='select * from transport_setup';
					$t_query=mysql_query($t_sql);
					while($t_row=mysql_fetch_object($t_query)){
					?>
						<option value="<?=$t_row->transport_id?>"><?=find_a_field('transport_info','transport_number','transport_id="'.$t_row->transport_id.'"');?></option>
					<?php } ?>
					</select>
...
<input name="driver_name" type="text" id="driver_name" value="<?=$ch_data->driver_name;?>" readonly />
...
<input  name="driver_mobile"  type="text" id="driver_mobile" value="<?=$ch_data->driver_mobile;?>" readonly/>
...
<label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Transport Target</label>

<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">

<input type="text" name="target_cost"  id="target_cost" value="<?=$ch_data->target_cost;?>"  />
...
<label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Gluteen</label>

<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">

<input type="text" name="gluteen"  id="gluteen" value="<?=$ch_data->gluteen;?>"  />
...
<th>Challan Issue(Bag) </th>

<th>Challan Issue(KG) </th>
...
<td align="center"><?=$stock = find_a_field('journal_item','SUM(item_in-item_ex)','item_id="'.$row->item_id.'" and warehouse_id="'.$_SESSION['user']['depot'].'" '); ?>     
...
<input list="bag_list" name="bag_size_<?=$row->id?>" id="bag_size_<?=$row->id?>" value="<?php echo find_a_field('item_info','secondary_unit_quantity','item_id="'.$row->item_id.'"');?>">

<datalist id="bag_list">

<?php 

$b_sql='select * from item_info where item_id="'.$row->item_id.'"';

$b_query=mysql_query($b_sql);

while($b_row=mysql_fetch_object($b_query)){

?>

<option value="<?php echo $b_row->secondary_unit_quantity;?>"><?php echo $b_row->secondary_unit_quantity;?></option>
...
<input name="qty_bag_<?=$row->id?>" type="text" id="qty_bag_<?=$row->id?>" value=""     />
