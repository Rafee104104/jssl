...

{
			...
			$sql = "select a.*,b.*,b.id as bid from fg_transfer_master a,fg_transfer_details b,item_info i where a.status!='COMPLETE' and a.st_no=b.st_no and i.item_id=b.item_id and a.st_no=".$$unique."";
		
		$query = mysql_query($sql);
		
		while($row=mysql_fetch_object($query)){	
			
				$found = find_a_field('journal_item','count(tr_no)','tr_from="fg_transfer" and tr_no='.$row->bid);
				...
				journal_item_control($row->item_id,$row->warehouse_from,$row->st_date,0,$row->qty,$page_for,$row->bid,$row->rate,'',$row->st_no);
				journal_item_control($row->item_id,$row->warehouse_to,$row->st_date,$row->qty,0,$page_for,$row->bid,$row->rate,'',$row->st_no);
