...
where w.warehouse_id=j.warehouse_id ".$date_con.$item_con." order by j.ji_date asc ";

	break;
	case 30:

$report="Product BIN CARD (Date Wise)(Mhafuz)";
...

{
		...
		<td><?=number_format($pre_price,4)?></td>
		...
		<td><?=number_format($row->item_price,4)?></td>

		<td><?=number_format(($tot_rec=$row->item_in*$row->item_price),2); $total_rec+=$tot_rec;?></td>
		...
		<td><?=number_format(($tot_issue=$row->item_ex*$row->item_price),2); $total_issue+=$tot_issue;?></td>

		<? }else{echo '<td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>';}?>

		<td><?=$final_stock?></td>

		<td><?=number_format($final_price,4)?></td>
...
where w.warehouse_id=j.warehouse_id ".$date_con.$item_con." order by j.ji_date , j.id ";
		if($sql==NULL) return NULL;
		...
		<td><?=number_format($row->pre_price,4)?></td>
		...
		<td><?=number_format($row->final_price,4)?></td>
