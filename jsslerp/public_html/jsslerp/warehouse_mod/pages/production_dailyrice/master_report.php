...
$report="RM Consumption Receive Report";
?>
	...
	<table width="100%" border="0" cellpadding="2" cellspacing="0" id="ExportTable">
		...
		<thead>
		<tr>
			<td colspan="19" style="border:0px;">
				<?
				echo '<div class="header">';
				echo '<h1>'.find_a_field('project_info','company_name','1').'</h1>';
				if(isset($report)) 
				echo '<h2>'.$report.'</h2>';
				
		if(isset($batch_no))
		echo '<h2>Batch No :'.$batch_no.'</h2>';
		if(isset($t_date)) 
		...
		<tbody>
			...
			if($batch_no!=''){
				...
				$sql = "SELECT r.issue_no,r.issue_date,r.item_id,i.item_name,r.unit_name,r.bag_qty,r.issue_qty,r.batch_no,r.entry_by,r.entry_at  
				FROM rm_consumption r, item_info i 
				WHERE r.item_id=i.item_id AND r.issue_date BETWEEN '".$f_date."' AND '".$t_date."' AND r.batch_no = '".$batch_no."'";
			}
			else{
							$sql = "SELECT r.issue_no,r.issue_date,r.item_id,i.item_name,r.unit_name,r.bag_qty,r.issue_qty,r.batch_no,r.entry_by,r.entry_at  
				FROM rm_consumption r, item_info i 
				WHERE r.item_id=i.item_id AND r.issue_date BETWEEN '".$f_date."' AND '".$t_date."' ";
			}
	...
	<table width="100%" border="0" cellpadding="2" cellspacing="0" id="ExportTable">
		...
		<tbody>
			...
			<?php
				...
				$sql = "SELECT f.receive_no,f.receive_date, f.item_id, i.item_name,f.unit_name,f.receive_qty,f.batch_no,f.entry_by,f.entry_at FROM fg_production f, item_info i WHERE f.item_id=i.item_id AND f.receive_date BETWEEN '".$f_date."' AND '".$t_date."' AND f.batch_no = '".$batch_no."'";
				
				}
				else{
				
				$sql = "SELECT f.receive_no,f.receive_date, f.item_id, i.item_name,f.unit_name,f.receive_qty,f.batch_no,f.entry_by,f.entry_at FROM fg_production f, item_info i WHERE f.item_id=i.item_id AND f.receive_date BETWEEN '".$f_date."' AND '".$t_date."'";
