...

{
		...
		$YR = date('Y',strtotime($issue_date));
		
 		$year = date('y',strtotime($issue_date));
		
		$month = date('m',strtotime($issue_date));
		
		$inv_cy_id = find_a_field('rm_consumption','max(batch_no)','year="'.$YR.'"')+1;
...
//   		$chalan_no=''.$yer.''.$month.''.$cy_id;
  ...
  VALUES("'.$issue_no.'","'.$batch_no.'","'.$batch_type.'", "'.$issue_date.'", "'.$group_for.'", "'.$warehouse_id.'", "'.$item_id.'", "'.$data->unit_name.'", "'.$issue_qty.'", "'.$avg_price.'", "'.$total_amt.'", "'.$remarks.'",  "'.$entry_at.'", "'.$entry_by.'","'.$bag_qty.'")';
					...
					<!--<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6  pt-1 pb-1">
						...
						<div class="form-group row m-0">
							...
							<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0">
								...
								<input name="batch_no" type="text" id="batch_no"  value="<?=$_POST['batch_no']?>"   />
