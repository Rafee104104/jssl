...
//   		$chalan_no=''.$yer.''.$month.''.$cy_id;
		...
		while($data=mysql_fetch_object($query))
		{
			...
			if($_POST['total_amt_'.$data->item_id]>0)
			{
				...
				$batch_no=$_POST['batch_no_'.$data->item_id];



   $ins_invoice = 'INSERT INTO fg_production (receive_no, receive_date,batch_no, group_for, warehouse_id, item_id, unit_name, receive_qty, cost_price, total_amt, remarks, entry_at, entry_by)

  

  VALUES("'.$receive_no.'", "'.$receive_date.'", "'.$batch_no.'","'.$group_for.'", "'.$warehouse_id.'", "'.$item_id.'", "'.$data->unit_name.'", "'.$receive_qty.'", "'.$cost_price.'", "'.$total_amt.'", "'.$remarks.'", "'.$entry_at.'", "'.$entry_by.'")';
...
<style>
	...
	<div class="form-container_large">
		...
		<form action="" method="post" name="codz" id="codz">
				...
				<div class="row">
					...
					<!--<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6  pt-1 pb-1">
						...
						<div class="form-group row m-0">
							...
							<label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text req-input">BATCH NO</label>

							<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0">

								<input name="batch_no" type="text" id="batch_no"  value="<?=$_POST['batch_no']?>" required   />
			...
			<h2 class="text-center bg-titel bold pt-2 pb-2">Finish Goods Receive</h2>
				...
				<table class="table1  table-striped table-bordered table-hover table-sm">
					...
					$sql = "SELECT i.item_id, i.item_name, i.unit_name, i.cost_price FROM item_info i, item_sub_group s WHERE i.sub_group_id=s.sub_group_id  and s.group_id=200000000 and s.sub_group_id=200010000 order  by i.sub_group_id, i.item_id ";
					...
					while($data=mysql_fetch_object($query)){$i++;
						...
						<tr>
							<td><span><?=$data->item_id;?></span></td>
							<td><?=$data->item_name;?></td>
							<td><span><?=$data->unit_name;?></span></td>
							<td><?=number_format($stock_qty[$data->item_id],2);?></td>
							
							<td>
								<select name="batch_no_<?=$data->item_id?>" id="batch_no_<?=$data->item_id?>" >
									<option></option>
									<? foreign_relation('rm_consumption','batch_no','batch_no',$batch_no,' status in ("PROCESSING") group by batch_no');?>
								</select>
							</td>
							...
							<td>
								...
								<input name="receive_qty_<?=$data->item_id?>" id="receive_qty_<?=$data->item_id?>" type="text"  value=""  onkeyup="EXPcalculation(<?=$data->item_id?>)"  />	
							</td>

							<td>
							<input name="cost_price_<?=$data->item_id?>" id="cost_price_<?=$data->item_id?>" type="text"  value="<?=$data->cost_price;?>"  onkeyup="EXPcalculation(<?=$data->item_id?>)"  />
							</td>
			...
			<div class="container-fluid pt-5 p-0 ">
			<h1 class="text-center bg-titel bold pt-2 pb-2"> Wastage Entry     </h1>
				<table class="table1  table-striped table-bordered table-hover table-sm">
