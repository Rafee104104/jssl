...
$table_details='warehouse_ds_issue_detail';
...
if($_REQUEST['old_do_no']>0){
    ...
    $check_manual=find1("select status from warehouse_ds_issue where oi_no='".$_REQUEST['old_do_no']."'");

    if($check_manual!='MANUAL') { redirect('direct_sales.php?pal=2');}

}

if($_GET['del']>0){

    $_SESSION['oi_no2']=find_a_field('warehouse_ds_issue_detail','oi_no','id='.$_GET['del']); $del = $_GET['del'];
...

{
		...
		?><script>window.location.href = "direct_sales.php?pal=2";</script><?
...
if(isset($_POST['hold'])){
		...
		redirect('direct_sales.php?pal=2');
...
if(isset($_POST['confirmm'])){
	...
	$master_info=findall("select * from warehouse_ds_issue where oi_no='".$oi_no."'");
        ...
        $narration      = 'Direct Sales#'.$oi_no.'.Party:'.$master_info->customer_name;

        $narration_coll = 'Direct Sales Collection#'.$oi_no.'.Party:'.$master_info->customer_name;

        $narration2     = 'Collection From Direct Sales#'.$oi_no.'. Party:'.$master_info->customer_name;



// BIN CARD		

$sql = 'select w.* from warehouse_ds_issue_detail w, item_info i 
...
journal_item_control($data->item_id,$_SESSION['user']['depot'],$data->oi_date,0,$data->qty,$page_for,$tr_id,'66','',$oi_no, '', '','', '', '');
...
$collection     =find_a_field('sales_collection','sum(amount)','oi_no="'.$oi_no.'"');
...
$sql_upp="update warehouse_ds_issue set product_total='".$product_total."',discount_amt='".$discount_amt."',payable_amt='".$payable_amt."',collection='".$collection."'
...
$cSql='select * from sales_collection where oi_no="'.$oi_no.'"';
$cQuery=mysql_query($cSql);
while($cRow=mysql_fetch_object($cQuery)){
...
if(isset($_POST['add'])&&($_POST[$unique]>0)){
		...
		$_POST['rate']= $_POST['tp']-(($_POST['dis_per']/100)*$_POST['tp']);
...
<!--<input type="hidden" class="form-control" name="vendor_id" id="vendor_id" required="required" value="<?=$default_direct_sales_party?>" autocomplete="off">                        -->
...
<? if ($vendor_id<1) {?>
    ...
    <input class="form-control" list="party_list" name="vendor_id" id="vendor_id" required="required" autocomplete="off" onChange="getPartyinfo()">
...
<? }else{?>
    ...
    <select class="form-control" name="vendor_id" class="form-control"  required  >
...
<td><input  name="tp" type="text"  id="tp" value="" onkeyup="count()" required/></td>
        ...
        <div class="container-fluid pt-5 p-0 ">
			...
			<table class="table1  table-striped table-bordered table-hover table-sm">
                ...
                <tr class="bgc-info">
                    ...
                    <th>TP</th><th>Dis%</th><th>Price</th>
...
$res='select a.id,b.item_name,a.rate as unit_price,a.qty ,a.unit_name,a.amount,"x" , b.finish_goods_code, a.dis_per,a.tp

from warehouse_ds_issue_detail a,item_info b where b.item_id=a.item_id and a.oi_no='.$oi_no;
...
?>
		...
		<tr>
            ...
            <td><?=$data->tp?></td><td><?=$data->dis_per?></td><td><?=$data->unit_price?></td>
...
$res='select c.*,a.ledger_name from sales_collection c,accounts_ledger a where c.ledger_id=a.ledger_id and c.oi_no='.$oi_no;
...
<?php /*?><div class="form-group row">
    ...
    <label for="inputEmail3" class="col-sm-3 col-form-label">Special Discount</label>

    <div class="col-sm-2">

	  <input class="form-control border border-success" name="discount_amt" type="text" id="discount_amt" value="" onkeyup="count_discount()"/>
...
<div class="form-group row">
    ...
    <label for="inputEmail3" class="col-sm-3 col-form-label">Payable Amount</label>

    <div class="col-sm-2">

	  <input class="form-control border border-success" name="payable_amt" type="text" id="payable_amt" value="<?=$gtotal-$discount_amt;?>" readonly/>
