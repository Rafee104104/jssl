...
function convert_number_to_bangla($number) {
...
//   echo $res->warehouse_name;
//   // convert 110150 to ১১০১৫০ 
//   $warehouse_b = convert_number_to_bangla($res->warehouse_name);
//    $sqlup = 'update warehouse set warehouse_b = "'.$warehouse_b.'" 
...
function nav(lkf){document.location.href = '<?=$page?>?<?=$unique?>='+lkf;}
  ...
  <div class="container-fluid">
    <div class="row">
	 ...
	 $rrr = "select * from warehouse where use_type != 'PL' order by warehouse_id asc";
	$report=mysql_query($rrr);

	while($rp=mysql_fetch_row($report)){$i++; if($i%2==0)$cls=' class="alt"'; else $cls='';?>
            <tr<?=$cls?> >
              <td><?=$rp[0];?></td>
			  <?php /*?><td><?=find_a_field('user_group','group_name','id='.$rp[18]);?></td><?php */?>
			  <td><?=find_a_field('plant_information','plant_name','id='.$rp[23]);?></td>
