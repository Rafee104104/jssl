...
$title='Unapprove Other Receive';
do_calander('#fdate');
do_calander('#tdate');
$table_master='warehouse_other_received_master';
$unique_master='oi_no';
...
$tr_type="Show";
$table_detail='warehouse_other_received_master_details';
$unique_detail='id';
...
$cc_code = '1';
$tr_no = $or_no;
$tr_from = 'Inv. Other Receive';
$narration = 'Inv. Other Receive#'.$or_no;
$issue_ledger = find_a_field('config_group_class','inv_other_receive','group_for="'.$_SESSION['user']['group'].'"');
...
$do_master = find_all_field('purchase_return_master','pr_no','pr_no='.$or_no);
  ...
  $do_sql = "select w.id,w.item_id,w.warehouse_id,w.qty,w.rate,w.amount,w.oi_no,w.oi_date,s.item_ledger as ledger_id,w.issue_type 
 
 from warehouse_other_received_master_details w, item_info i, item_sub_group s 
 
 where w.item_id=i.item_id and i.sub_group_id=s.sub_group_id and w.oi_no='".$or_no."' and w.issue_type='Inv. Other Receive' ";
...
$do_query = mysql_query($do_sql);	
	...
	journal_item_control($do_data->item_id, $do_data->warehouse_id, $do_data->oi_date,$do_data->qty, 0,  $do_data->issue_type, $do_data->id, $do_data->rate, $do_data->warehouse_id, $do_data->oi_no, '', '',$do_master->group_for, $avg_rate, '' );
	
	
//add_to_sec_journal($proj_id, $jv_no, $jv_date, $do_data->ledger_id, $narration,  $do_data->amount, '0', $do_data->issue_type, $tr_no,'',$tr_id,$cc_code,$group_for);
  $total_amt +=$do_data->amount;
  $return_type = $do_data->issue_type;
		}
		
		$do_sql2 = "select w.id,w.item_id,w.warehouse_id,w.qty,w.rate,sum(w.amount) as amt,w.oi_no,w.oi_date,s.item_ledger as ledger_id,w.issue_type 
 
 from warehouse_other_received_master_details w, item_info i, item_sub_group s 
 
 where w.item_id=i.item_id and i.sub_group_id=s.sub_group_id and w.oi_no='".$or_no."' and w.issue_type='Inv. Other Receive' group by 
  s.item_ledger";

$do_query2 = mysql_query($do_sql2);	
        
		while($do_data2=mysql_fetch_object($do_query2))

		{
		add_to_sec_journal($proj_id, $jv_no, $jv_date, $do_data2->ledger_id, $narration,  $do_data2->amt, '0', $do_data2->issue_type, $tr_no,'',$tr_id,$cc_code,$group_for);
		
		}
		
	add_to_sec_journal($proj_id, $jv_no, $jv_date, $issue_ledger, $narration, '0', $total_amt, $return_type, $tr_no,'',$tr_id,$cc_code,$group_for);

		$sql2 = 'update warehouse_other_received_master set status="CHECKED" where oi_no = '.$or_no;
		mysql_query($sql2);
		
		  $sql3 = 'update warehouse_other_received_master_details set status="CHECKED" where oi_no = '.$or_no;
		mysql_query($sql3);
...
and "'.date('Y-m-d').'"';}
							 ...
							 $res ='select a.oi_no,a.oi_date,a.entry_by,a.issued_to,a.approved_by,a.oi_date,u.fname,a.requisition_from

  from warehouse_other_received_master a, user_activity_management u
  ...
  where a.entry_by=u.user_id and a.warehouse_id="'.$_SESSION['user']['depot'].'" and a.status="UNCHECKED" '.$con.' order by a.oi_no desc';
							...
							<td <?=$data->oi_no;?>;" <?=(++$z%2)?'':'class="alt"';?> style="text-align:center">&nbsp;<?=$data->requisition_from;?></td>
