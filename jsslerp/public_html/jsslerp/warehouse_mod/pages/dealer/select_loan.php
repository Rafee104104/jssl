...
<form action="" method="post" name="codz" id="codz">
  ...
  <? if(isset($_POST['submit'])){ ?>
  <?
  	$loan_amt =0;  $paid_amt = 0; $amount_assigned = 0;
  ?>
  <div class="row">
  	<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6 p-1">
			<table style="margin-left:100px;width: 50%;">
				<tr>
					<td><strong>Total Loan Amount:</strong></td>
					<td><?=$loan_amt = find_a_field('sr_loan','sum(amount)','booking_number like "'.$_POST['booking_number'].'" ');?></td>
				</tr>
				
				<tr>
					<td><strong>Already Paid:</strong></td>
					<td><?=$paid_amt = find_a_field('sr_loan_return','sum(total_paid)','booking_number like "'.$_POST['booking_number'].'" ');?></td>
				</tr>
				
				<tr>
					<td><strong>Alreary Assigned: </strong></td>
					<td><?=$amount_assigned = find_a_field('loan_assign','sum(amount_in)','booking_no like "'.$_POST['booking_number'].'" '); ?> </td>
				</tr>
				
				<tr>
					<td><strong>Balance for assign: </strong></td>
					<td><?=$balance = ($loan_amt-($paid_amt+$amount_assigned)); ?> </td>
				</tr>
			</table>
    ...
    <table class="table1  table-striped table-bordered table-hover table-sm">
      ...
      <tbody class="tbody1">
        ...
        <? if ($data->total_qty> 0) {
		       ...
		       $loan_amt = find_all_field('loan_assign','*','bag_mark like "'.$data->bag_mark.'" ');  
...
<?php */?>		  
          <td><input name="qty_<?=$data->or_no?>"  id="qty_sr_<?=$data->or_no?>" type="text"  value="<?=$loan_amt->sr_loan?>"  />
				...
				<input type="hidden" id="bag_mark<?=$data->or_no?>" value="<?=$data->bag_mark?>"  />		  </td>
		        <td><input name="qty_<?=$data->or_no?>"  id="seeds_loan_<?=$data->or_no?>" type="text"  value="<?=$loan_amt->seeds_loan?>"  /></td> 
				<td><input name="qty_<?=$data->or_no?>"  id="bag_loan_<?=$data->or_no?>" type="text"  value="<?=$loan_amt->bag_loan?>"  /></td> 
				   
				<td><input name="qty_<?=$data->or_no?>"  id="farmer_loan_<?=$data->or_no?>" type="text"  value="<?=$loan_amt->farmer_loan?>"  /></td>
        
                <td align="center"><input name="qty_<?=$data->or_no?>"  id="others_loan_<?=$data->or_no?>" type="text"  value="<?=$loan_amt->others_loan?>"  /></td>
          <td align="center"><center id="assign_warehouse<?=$data->or_no?>">
		   ...
		   <? }else{?>
              <button onclick="getData2('select_loan_ajax.php', 'assign_warehouse<?=$data->or_no?>', document.getElementById('qty_sr_<?=$data->or_no?>').value,  document.getElementById('or_no_<?=$data->or_no?>').value+'##'+document.getElementById('booking_number').value+'##'+document.getElementById('token_number<?=$data->or_no?>').value+'##'+document.getElementById('bag_mark<?=$data->or_no?>').value+'##'+document.getElementById('bag_loan_<?=$data->or_no?>').value+'##'+document.getElementById('seeds_loan_<?=$data->or_no?>').value+'##'+document.getElementById('farmer_loan_<?=$data->or_no?>').value+'##'+document.getElementById('others_loan_<?=$data->or_no?>').value+'##0');" type="button" class="btn1 btn1-bg-submit" >Add</button>
			 <? } ?>
            </center></td>
        </tr>
