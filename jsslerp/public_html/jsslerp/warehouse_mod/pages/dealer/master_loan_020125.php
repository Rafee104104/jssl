    ...
    <td><div id="pr">
      ...
      <table width="100%" class="tabledesign" border="1" bordercolor="#000000" cellspacing="0"
        ...
        <tr>
          ...
          <td align="center" bgcolor="#CCCCCC">Bag Mark No </td>
...
////////////// Loan Assign/////////////
echo $sql2='select bag_loan as tot_bag_loan,booking_no,bag_mark from loan_assign where 1 ';
...
while($row2=mysql_fetch_object($query2)){
	$get_tot_bag_loan[$row2->booking_no]=$row2->tot_bag_loan;
	 ...
	 echo  $td='select sum(b.amount) as tot_amount,b.interest_amt,b.booking_number,b.date as loan_date from  sr_loan b where 1 '.$booing_no.' group by b.sr_loan_id';
$report = mysql_query($td);

$sl = 1; 
while ($data = mysql_fetch_object($report)) {
//$amt+=$data->amount;
$avg=($data->tot_amount/$get_tot_qty[$data->booking_number]);
    ...
    ?>
        <tr>
          ...
          <td><?= $agent_name_get[$data->booking_number];?></td>
          <td>&nbsp;</td>
          <td><?=$village_name[$data->booking_number]?></td>
          <td></td>
          <td><?= $get_tot_bag_loan[$data->booking_number];?> </td>
          <td></td>
          <td></td>
          <td></td>
          <td><?=$data->tot_amount;?></td>
          <td></td>
          <td><?=$data->tot_amount;   $t_amount+=$data->tot_amount; ?></td>
          <td><?php $get_tot_qty[$data->booking_number];   $t_qty=$get_tot_qty[$data->booking_number];?></td>
          <td><?php number_format($avg,2); $tot_avg+=$avg;?></td>
        </tr>
        <?php } ?>
        <td colspan="11" align="center" valign="top"><div align="right"><strong>Total Amount: </strong></div></td>
