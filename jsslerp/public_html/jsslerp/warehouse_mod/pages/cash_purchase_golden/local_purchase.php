...
if(isset($_POST['confirmm']))
{
        ...
        $cc_code = '1';
        $tr_no = $or_no;
        $tr_from = 'Cash_Purchase';
        $narration = 'Cash_Purchase#'.$or_no;
        //$cashLedger = '1260010002'; cash_ledger
		$cashLedger = $_POST['cash_ledger'];
		...
		add_to_sec_journal($proj_id, $jv_no, $jv_date, $cashLedger , $narration, '0',$total_amt, $tr_from, $tr_no,'',$tr_id=0,$cc_code,$group_for);
		unset($_POST);
		$_POST[$unique]=$$unique;
		$_POST['entry_by']=$_SESSION['user']['id'];
		$_POST['entry_at']=date('Y-m-d h:s:i');
		...
		$crud   = new crud($table_master);
		$crud->update($unique);
		sec_journal_journal($jv_no,$jv_no,'Cash_Purchase');
		unset($$unique);
		unset($_SESSION['or_no2']);
		$type=1;
...
if($$unique>0) $btn_name='Update CP Information'; else $btn_name='Initiate CP Information';
            ...
            <div class="row">
                ...
                <div class="col-sm-6 col-md-6 col-lg-6 col-xl-6">
                    <div class="container n-form2">
						...
						<? $field='or_no';?>
                            <label for="<?=$field?>" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">CP NO</label>
                        ...
                        <div class="form-group row m-0 pb-1">
                            ...
                            <label for="<?=$field?>" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">CP Date</label>
                            <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
    ...
    <form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
        <div class="container-fluid p-0 ">
            ...
            <div class="n-form-btn-class">
				<input type="hidden" id="<?=$unique?>" name="<?=$unique?>" value="<?=$$unique?>"  />
				<input  name="receive_type" type="hidden" id="receive_type" value="<?=$page_for?>"  required="required"/>
				<input  name="cash_ledger" type="hidden" id="cash_ledger" value="<?=$cash_ledger?>"  required="required"/>
<input type="hidden" id="or_date" name="or_date" value="<?=$or_date?>"  />
                <input name="confirmm" type="submit" class="btn1 btn1-bg-submit" value="CONFIRM AND FORWARD CP" />
            </div>
