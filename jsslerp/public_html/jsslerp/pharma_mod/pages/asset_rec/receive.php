...
$journal_item_sql = 'insert into journal_item (`ji_date`,`item_id`,`warehouse_id`,`serial_no`,`item_in`,`item_price`,`final_price`,`tr_from`,`tr_no`,`sr_no`,`entry_by`,`entry_at`,`primary_id`) value("'.$data->entry_at.'","'.$data->item_id.'","'.$_SESSION['user']['depot'].'","'.$data->serial_no.'",1,"'.$journalAll->item_price.'","'.$journalAll->final_price.'","AssetReceive","'.$data->id.'","'.$data->m_id.'","'.$_SESSION['user']['id'].'","'.date('Y-m-d h:i:s').'","'.$data->id.'")';
...
echo $sql='insert into fixed_asset_receive_details(m_id,item_id,serial_no,date,quality,note,entry_by,type,PBI_ID) values

("'.$_POST['ids'].'","'.$_POST['item_id'].'","'.$_POST['serial_no'].'","'.date("Y-m-d").'","'.$_POST['quality'].'","'.$_POST['note'].'","'.$_SESSION['user']['id'].'","'.$_POST['type'].'","'.$_POST['PBI_ID'].'")';
					...
					<div class="container n-form2">
						...
						<div class="form-group row m-0 pb-1">
							...
							<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
								...
								<input name="department_name" type="text" id="department_name" value="<?= $department_name; ?>" />
