...
$data5=mysql_fetch_object($query);
  ...
  $sql1='SELECT  s.chalan_no,s.dealer_code,s.chalan_date,d.dealer_name_e,d.dealer_code,d.address_e,d.contact_no,d.contact_person_name,d.email,s.do_no, s.vat_no,m.po_no,m.po_date
	...
	<div class="container">
	  ...
	  <div class="row" id="header3">
		...
		<div class="col-8 text-center">
			...
			Cell: <?php echo find_a_field('user_group','phone','1')?>. Email: <?php echo find_a_field('user_group','email','1')?>  <br> <?php echo find_a_field('user_group','website','1')?>
		</div>
		<div class="col-2"></div>
	</div>
...
<div class="row">
		  ...
		  <div class="col-6">
			...
			<div class="input-group mb-3 input-group-sm">
			  ...
			  <input type="text" class="form-control" readonly="readonly" value="<?=find_a_field('sale_do_chalan','manual_po_no','chalan_no='.$data->chalan_no); ?>">
			</div>
			<div class="input-group mb-3 input-group-sm">
			  <div class="input-group-prepend">
...
<!--<h4 class="text-center"><u>Invoice</u></h4>-->
  ...
  <thead>
    <tr style="text-align:center;">
      <th>Challan Date</th>
      <th>Challan NO<br> <br></th>
	  <th>S Product Name</th>
	  <th>S Company</th>
      <th>Product Code</th>
      <th>Description of Product <br> <br></th>
      <th>Quantity <br> <br></th>
      <th>UOM <br> <br></th>
      <th>Unit Price</th>
	  <th>UP After Discount</th>
      <th>Amount(BDT) <br> <br></th>
      <th>Remarks <br> <br></th>
    </tr>
  </thead>
  <tbody>
  <?php
   $sql='SELECT s.order_no,s.chalan_no,s.item_id,s.dealer_code,s.unit_price,s.total_unit,s.total_amt,s.chalan_date,s.dealer_code,i.item_name,i.d_price,i.unit_name,s.do_no,d.c_item_id
from sale_do_chalan s,item_info i,sale_do_details d
where s.chalan_no="'.$challan.'" and s.item_id=i.item_id and s.order_no=d.id
';
$query=mysql_query($sql);
...
?>
    <tr class="text-center">
	  ...
	  <td><?=find_a_field('item_info_other','item_name','item_id='.$data->c_item_id); ?></td>
	   <td><?=find_a_field('item_info_other','company','item_id='.$data->c_item_id); ?></td>
...
?>
<tr class="text-center">
<td colspan="7" style="font-weight:bold;text-align:left;">VAT Amount <b>(<? if( $vat>0){ echo number_format($vat,0);echo '%'; }?>)</b></td>
...
<td colspan="7" style="font-weight:bold;text-align:left;">VAT Amount </td>
</td><td></td><td></td><td></td>
<td style="text-align:right;font-weight:bold;"><?php
if( $vat_amt>0){
//$vat_amt=($total1*$vat)/100;
...
?>
<tr class="text-center">
<td colspan="7" style="text-align:left;font-weight:bold;" >Transportation Expences</td></td><td></td><td></td><td></td>
...
?>
<tr class="text-center">
<td colspan="7" style="text-align:left;font-weight:bold;"> Cash Discount </td></td><td></td><td></td><td></td>
<td style="text-align:right;font-weight:bold;"><?php echo $cash_discount; ?></td>
<td></td>
</tr>
<?php } ?>
<tr class="text-center">
<td colspan="7" style="text-align:left;font-weight:bold;">Gross Total</td></td><td></td><td></td><td></td>
<td style="font-weight:bold;text-align:right"><?php echo number_format($all_amt2=($vat_amt+$total1+$cost)-$cash_discount,2);$all_amt=($vat_amt+$total1+$cost)-$cash_discount ?></td>
<td></td>
</tr>
...
<br><? $all_amt = number_format($all_amt,2,'.','');?>
