...
/*$ch_sql = 'select * from sale_do_chalan where 1';
$qry = mysql_query($ch_sql);
while($data=mysql_fetch_object($qry)){
$final_price= find_a_field('journal_item','item_price','item_id = "'.$data->item_id .'" and tr_from="Purchase" order by id desc');
$cost_amt = $data->total_unit*$final_price;
$update = 'update sale_do_chalan set cost_price="'.$final_price.'", cost_amt="'.$cost_amt.'" where id="'.$data->id.'" and item_id="'.$data->item_id.'" and depot_id="'.$data->depot_id.'" and chalan_no="'.$data->chalan_no.'"';
mysql_query($update);
}
*/
...
$update = 'update sale_do_chalan set journal_status=1 where chalan_no="'.$chalan_no.'"';
mysql_query($update);
}

$_SESSION['cmsg'] = '<span style="color:green;">Chalan Re-hit Successfully</span>';
header('location:done_chalan_list.php');	
