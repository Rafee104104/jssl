...
<!--            <div class="col-6"><p style="float:right">Reporting Time: -->
        ...
        <table class="table table-bordered border-primary  text-center">
            ...
            <tr>
                <td colspan="9" style="text-align: left; border-right:none;">
...
$gift_delivered = find_a_field('sale_do_chalan','sum(total_unit)','do_no="'.$row->do_no.'" and item_id="'.$row->gift_item_id.'"');
                ...
                <td colspan="13">
                    <table class="table table-bordered border-primary  text-center">
						...
						<? 
						 ...
						 $gift_item_id = find_a_field('sale_gift_offer','gift_id','item_id="'.$undel->item_id.'" and start_date<="'.$undel->do_date.'" and end_date>="'.$undel-> do_date.'" and dealer_type="'.$dealer_type.'"');
						 $gift_sql = 'select s.*,i.item_name,i.finish_goods_code from sale_do_details s, item_info i where s.item_id=i.item_id and s.do_no="'.$undel->do_no.'" and  s.item_id="'.$gift_item_id.'" and s.gift_id>0';
                         $gift_qry = mysql_query($gift_sql);
                         $gift_info = mysql_fetch_object($gift_qry);
                         $gift_delivered = find_a_field('sale_do_chalan','sum(total_unit)','do_no="'.$undel->do_no.'" and item_id="'.$gift_item_id.'"');
