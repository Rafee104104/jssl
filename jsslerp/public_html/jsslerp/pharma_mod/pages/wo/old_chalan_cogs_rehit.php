...
$title='COGS Confirmation';

//$final_price= find_a_field('journal_item','final_price',' warehouse_id = "25" and item_id = "200060281"  order by id desc');
...
if($chalan_no>0){
$ch_sql = 'select c.total_unit,i.item_id,i.cost_price from sale_do_chalan c, item_info i where i.item_id=c.item_id and c.chalan_no="'.$chalan_no.'"';
$ch_qry = mysql_query($ch_sql);
while($ch_data=mysql_fetch_object($ch_qry)){
$cost_amt = $ch_data->total_unit*$ch_data->cost_price;
$update_item = 'update sale_do_chalan set cost_price="'.$ch_data->cost_price.'", cost_amt="'.$cost_amt.'" where chalan_no="'.$chalan_no.'" and item_id="'.$ch_data->item_id.'"';
mysql_query($update_item);
}
//$jv_no = find_a_field('secondary_journal','jv_no','tr_no="'.$chalan_no.'" and tr_from in ("Sales")');
$delete = 'delete from secondary_journal where tr_no="'.$chalan_no.'" and tr_from in ("Sales","COGS")';
mysql_query($delete);
$delete2 = 'delete from journal where tr_no="'.$chalan_no.'" and tr_from in ("Sales","COGS")';
mysql_query($delete2);
//auto_insert_sales_cogs_secoundary2($chalan_no,$jv_no);
auto_insert_sales_chalan_secoundary_dev($chalan_no);
$update = 'update sale_do_chalan set cogs_rehit=1 where chalan_no="'.$chalan_no.'"';
mysql_query($update);
}

$_SESSION['cmsg'] = '<span style="color:green;">Cogs Re-hit Successfully</span>';
header('location:chalan_list_for_cogs.php');	
...
<div class="container-fluid bg-form-titel">
 ...
 from sale_do_chalan a,item_info b, item_sub_group s where b.item_id=a.item_id 
  and b.sub_group_id=s.sub_group_id and a.journal_status="0" and a.chalan_no='.$chalan_no;
