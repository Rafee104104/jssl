...
mysql_query($so_invoice);
	...
	journal_item_control($item_id, $ms_data->depot_id, $ch_date,  0, $pqty, 'Sales', $data->id, $rate, '', $chalan_no, '', '',$ms_data->group_for, $rate->unit_price, '' );
...
$d_qty = find_a_field('sale_do_details','sum(total_unit)','do_no="'.$do_no.'"');

$c_qty = find_a_field('sale_do_chalan','sum(total_unit)','do_no="'.$do_no.'"');

if($d_qty==$c_qty){

$master_update = 'update sale_do_master set status="COMPLETED" where do_no="'.$do_no.'"';
...
$sql='select a.id,m.do_no, a.item_id,a.gift_id, a.unit_price, b.item_name,  b.unit_name,  a.total_unit as qty,m.dealer_code,m.do_date,a.pkt_size,b.cost_price from sale_do_master m,sale_do_details a,item_info b, item_sub_group s where m.do_no=a.do_no and m.status="CHECKED" and m.dealer_type="Official" and b.item_id=a.item_id and b.sub_group_id=s.sub_group_id and m.dealer_code="'.$ms_data->dealer_code.'" and m.depot_id="'.$_SESSION['user']['depot'].'" and a.do_no!='.$do_no;
...
journal_item_control($item_id, $ms_data->depot_id, $undel->chalan_date,  0, $pqty, 'Sales', $undel->id, $rate, '', $chalan_no, '', '',$ms_data->group_for, $rate->unit_price, '' );





$d_qty = find_a_field('sale_do_details','sum(total_unit)','do_no="'.$undel->do_no.'"');

$c_qty = find_a_field('sale_do_chalan','sum(total_unit)','do_no="'.$undel->do_no.'" ');

if($d_qty==$c_qty){

$update = 'update sale_do_master set status="COMPLETED" where do_no="'.$undel->do_no.'"';
...
auto_insert_sales_chalan_secoundary_dev($chalan_no);
							...
							<div class="d-flex justify-content-center pt-2 pb-2">
								...
								<table class="table1  table-striped table-bordered table-hover table-sm">
									...
									<thead>
										...
										$sql='select distinct chalan_no, chalan_date,m.dealer_type from sale_do_chalan c, sale_do_master m where m.do_no=c.do_no and c.do_no='.$$unique.' order by c.id desc';
...
<div class="container-fluid bg-form-titel">
 ...
 from sale_do_master m, sale_do_details a left join item_info_other o on o.item_id=a.c_item_id,item_info b, item_sub_group s where m.do_no=a.do_no and m.dealer_type="Official" and b.item_id=a.item_id and b.sub_group_id=s.sub_group_id and  a.do_no='.$$unique;
...
?>
            ...
            <table class="table1  table-striped table-bordered table-hover table-sm">
                ...
                <tbody class="tbody1">
				 ...
				 $so_qty_ctn = (find_a_field('sale_do_chalan','sum(pkt_unit)','order_no="'.$row->id.'" and item_id="'.$row->item_id.'"')*(1));

				 $so_qty_pcs = (find_a_field('sale_do_chalan','sum(total_unit)','order_no="'.$row->id.'" and item_id="'.$row->item_id.'"')*(1));
          ...
          <? } } ?>
			...
			<div align="center"><h3>Undelivered Product</h3>&nbsp;<i class="fas fa-arrow-down"></i></div>

			<?

			  $dealer = find_a_field('sale_do_master','dealer_code','do_no="'.$$unique.'"');

			  $distributor = find_a_field('sale_do_master','distributor_id','do_no="'.$$unique.'"');

			  $sql='select a.id,m.do_no, a.item_id,  a.unit_price, b.item_name,  b.unit_name, a.pkt_unit as qty,a.pkt_size,a.total_unit as tqty,o.item_name as c_item_name,o.company 

 from sale_do_master m,sale_do_details a left join item_info_other o on o.item_id=a.c_item_id,item_info b, item_sub_group s where m.dealer_type="Official" and m.distributor_id="'.distributor.'" and m.do_no=a.do_no and m.status="CHECKED" and b.item_id=a.item_id 

  and b.sub_group_id=s.sub_group_id and m.dealer_code="'.$dealer.'" and m.depot_id="'.$_SESSION['user']['depot'].'" and a.do_no!='.$$unique;
                ...
                <tbody class="tbody1">
				 ...
				 <? while($row=mysql_fetch_object($res)){
				  ...
				  $bg++;
				   ...
				   /*$d_qty = find_a_field('sale_do_details','sum(total_unit)','id="'.$row->id.'" and do_no="'.$row->do_no.'"');

				   $c_qty = find_a_field('sale_do_chalan','sum(total_unit)','order_no="'.$row->id.'" and do_no="'.$row->do_no.'" ');
