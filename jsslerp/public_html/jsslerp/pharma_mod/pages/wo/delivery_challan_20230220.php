...
while($undel = mysql_fetch_object($res)){
...

			{
				...
				$cost_amt = ($pqty*$data->cost_price); 
				$pre_stock= find_all_field('journal_item','final_stock',' warehouse_id = "'.$ms_data->depot_id.'" and item_id = "'.$item_id .'"  order by id desc');
				//$final_price = ((($pre_stock->final_price*$pre_stock->final_stock)+($item_price*$pqty))/($pre_stock->final_stock+$pqty));
				$cost_amt = $pqty*$pre_stock->final_price;
