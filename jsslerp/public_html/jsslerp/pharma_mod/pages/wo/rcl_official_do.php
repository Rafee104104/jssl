...
$title='Pending RCL Official Sales Order List';
do_calander('#fdate');
do_calander('#tdate');
...
$target_url = '../wo/rcl_official_chalan.php';
...
$con .= ' and m.do_no in ('.$_POST['do_no'].') ';
 		...
 		$sql = "select   c.do_no, sum(c.total_unit) as ch_qty  from sale_do_master m, sale_do_chalan c where m.dealer_type='Official' and m.do_no=c.do_no  group by c.do_no ";
 ...
 where m.do_no=d.do_no and  m.status  in ('CHECKED') and m.dealer_type='Official' and m.depot_id='".$_SESSION['user']['depot']."' ".$con." group by m.do_no order by m.do_no desc";
