...
<!--            <div class="col-6"><p style="float:right">Reporting Time: -->
        ...
        <table class="table table-bordered border-primary  text-center">
			...
			$sql = 'select * from sale_do_details where do_no="'.$ch_all->do_no.'" order by id asc';
			$qry = mysql_query($sql);
			$c=1;
...
$sql='select c.*,sum(c.pkt_unit) as chalan_qty_ctn,sum(c.total_unit) as chalan_qty_pcs,o.item_name as c_item,o.company,offer.id as s_gift_id,offer.gift_id as gift_item_id from sale_do_chalan c,sale_do_details s left join item_info_other o on o.item_id=s.c_item_id left join sale_gift_offer offer on offer.item_id=s.item_id and offer.start_date<="'.$do_all->do_date.'" and offer.end_date>="'.$do_all->do_date.'" and offer.dealer_type="'.$dealer_type.'" where s.do_no=c.do_no and s.item_id=c.item_id and c.chalan_no="'.$chalan_no.'" and s.gift_id=0 and s.id=c.order_no group by c.item_id';
...
?>
                <td><?=++$i?></td>
                <td><?php echo $ch_all->do_no.'+'.$ch_all->do_date.'+'.$row_num[$ch_all->do_no][$row->item_id]?></td>
            ...
            <tr>
                <td colspan="10"><h5 style="text-align: center; font-weight: bold;">Undelivered Information</h5></td>
            </tr>
			...
			$sql='select a.id,m.do_no,m.do_date,m.depot_id, a.item_id,a.pkt_unit,  a.unit_price, b.item_name,b.finish_goods_code, b.unit_name,  a.total_unit as qty,o.item_name as c_item,o.company,a.total_amt from sale_do_master m,sale_do_details a left join item_info_other o on o.item_id=a.c_item_id,item_info b, item_sub_group s where m.do_no=a.do_no and m.status="CHECKED" and b.item_id=a.item_id and b.sub_group_id=s.sub_group_id and m.dealer_code="'.$ch_all->dealer_code.'" and a.gift_id=0 and m.depot_id="'.$_SESSION['user']['depot'].'" and a.do_no!='.$ch_all->do_no;
						...
						?>
                            <tr>
								...
								<td><?=$undel->qty+$gift_info->total_unit?></td>
                                <td><?=$undel->qty?> </td>
                                <td><?=$gift_info->total_unit?> </td>
								<td><?=$undel->total_amt?> </td>
                                <td><?=$depo_code?></td>
								<td></td>
                            </tr>
							<? } } ?>
