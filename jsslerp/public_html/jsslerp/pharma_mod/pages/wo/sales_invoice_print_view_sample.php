...
$invoice_edit_per=find_a_field('user_roll_activity','edit','page_id=431 and user_id="'.$_SESSION['user']['id'].'"');
  $sql='select m.do_no,m.vat

from sample_do_master m,sample_do_chalan c
...
$data5=mysql_fetch_object($query);
   ...
   $sql1='SELECT  s.chalan_no,s.dealer_code,s.chalan_date, m.distributor_id,s.do_no, s.vat_no,m.po_no,m.po_date
	
	from sample_do_chalan s,sample_do_master m
	
	where s.do_no=m.do_no and s.chalan_no="'.$challan.'"  limit 1 ';   
	...
	<div class="container">
	  ...
	  <div class="row" id="header3">
		...
		<div class="col-8 text-center">
			...
			Cell: <?php echo find_a_field('project_info','proj_phone','1')?>. Email: <?php echo find_a_field('project_info','proj_email','1')?>  <br> <?php echo find_a_field('project_info','website','1')?>
		</div>
		<div class="col-2"></div>
	</div>
...
<div class="row">
		  ...
		  <div class="col-6">
			...
			<div class="input-group mb-3 input-group-sm">
			  ...
			  <input type="text" class="form-control" readonly="readonly" value="<?=find_a_field('user_activity_management','fname','user_id="'.$data->dealer_code.'"');?>">
			</div>
			<div class="input-group mb-3 input-group-sm">
			  <div class="input-group-prepend">
			...
			<div class="input-group mb-3 input-group-sm">
			  ...
			  <input type="text" class="form-control" readonly="readonly"  value="<?=find_a_field('dealer_info','dealer_name_e','dealer_code="'.$data->distributor_id.'"');?>">
			</div>
			...
			<div class="input-group mb-3 input-group-sm">
			  ...
			  <input type="text" class="form-control" readonly="readonly"  value="<?php $do_date=find_a_field('sample_do_master','do_date','do_no="'.$data->do_no.'"');
		...
		<!--	  <input type="text" class="form-control" readonly="readonly"  value="<?=$data->address_e;?>"  >-->
			  <textarea  class="form-control"  ><?=find_a_field('dealer_info','address_e','dealer_code="'.$data->distributor_id.'"');?></textarea>
			</div>
			<div class="input-group mb-3 input-group-sm">
			  <div class="input-group-prepend">
				<span class="input-group-text"  style="font-weight:bold;">Cell NO</span>
			  </div>
			  <input type="text" class="form-control" readonly="readonly"  value="<?=find_a_field('dealer_info','contact_no','dealer_code="'.$data->distributor_id.'"');?>" >
			</div>
			...
			<div class="input-group mb-3 input-group-sm">
			  ...
			  <input type="text" class="form-control" readonly="readonly"   value="<?=$data->vat_no?>" >
			</div>
...
<!--<div class="info">
 ...
 <?php 
     $sql='SELECT  s.chalan_no,s.dealer_code,s.chalan_date,d.dealer_name_e,d.address_e,d.mobile_no,d.contact_person_name,d.email
	
	from sample_do_chalan s,dealer_info d
	
	where s.chalan_no="'.$challan.'"  limit 1 ';     
            $query=mysql_query($sql);
...
<!--<h4 class="text-center"><u>Invoice</u></h4>-->
  ...
  <tbody>
  <?php
   $sql='SELECT s.order_no,s.chalan_no,s.item_id,s.dealer_code,s.unit_price,s.total_unit,s.total_amt,s.chalan_date,s.dealer_code,i.item_name,i.d_price,i.unit_name,s.do_no 
from sample_do_chalan s,item_info i
where s.chalan_no="'.$challan.'" and s.item_id=i.item_id
';
$query=mysql_query($sql);

while($data = mysql_fetch_object($query)){
//$do_price = find_a_field('sale_do_details','do_price','id="'.$data->order_no.'"');
?>
    <tr class="text-center">
	  ...
	  <td style="text-align:right;"><?=number_format($data->d_price,2);?></td>
	   <td style="text-align:right;"><?php if($data->unit_price==0){echo number_format($data->d_price,2);}else{ echo number_format($data->unit_price,2);}?></td>
 ...
 $vat=find_a_field('sample_do_master','vat','do_no="'.$data->do_no.'"');
    // $vat=$data->vat_amount;
   ?> 
    <?php } ?>
<tr class="text-center">
...
?>
<tr class="text-center">
<td colspan="5" style="font-weight:bold;text-align:left;">VAT Amount <b>(<? if( $vat>0){ echo number_format($vat,0);echo '%'; }?>)</b></td>
...
<td colspan="5" style="font-weight:bold;text-align:left;">VAT Amount </td>
</td><td></td><td></td><td></td>
<td style="text-align:right;font-weight:bold;"><?php
if( $vat_amt>0){
//$vat_amt=($total1*$vat)/100;
...
?>
<tr class="text-center">
<td colspan="5" style="text-align:left;font-weight:bold;" >Transportation Expences</td></td><td></td><td></td><td></td>
...
?>
<tr class="text-center">
<td colspan="5" style="text-align:left;font-weight:bold;"> Cash Discount </td></td><td></td><td></td><td></td>
<td style="text-align:right;font-weight:bold;"><?php echo $cash_discount; ?></td>
<td></td>
</tr>
<?php } ?>
<tr class="text-center">
<td colspan="5" style="text-align:left;font-weight:bold;">Gross Total</td></td><td></td><td></td><td></td>
...
$val='yes';
$up_sql="update sample_do_chalan set invoice_check='".$val."',check_inv_by='".$_SESSION['user']['id']."' where 	chalan_no='".$challan."' ";
mysql_query($up_sql);
}
if(isset($_POST['approve'])){
$val='yes';
$up_sql="update sample_do_chalan set approve_status='".$val."',check_appr_by='".$_SESSION['user']['id']."' where 	chalan_no='".$challan."'";
mysql_query($up_sql);
...
<div class="row">
	...
	<div class="col-4">
	<?php 
	$all_appro=find_all_field('sample_do_chalan','','chalan_no="'.$challan.'"');
	$check_sta=find_a_field('sample_do_chalan','invoice_check','chalan_no="'.$challan.'"');
	$appro_sta=find_a_field('sample_do_chalan','approve_status','chalan_no="'.$challan.'"');

				if($check_sta!='yes'){
					
					if($_SESSION['user']['id']==10095 || $_SESSION['user']['id']==10115 || $_SESSION['user']['id']==1 || $_SESSION['user']['id']==10115 || $_SESSION['user']['id']==10102 || $_SESSION['user']['id']==10101 || $_SESSION['user']['id']==10044){
				?>
				<input class="btn btn-info" style="color:white;"  type="submit" name="check" value="Check">
				<?php } } 
				else{
				if($appro_sta!='yes'){
				if($_SESSION['user']['id']==10100 || $_SESSION['user']['id']==10140 || $_SESSION['user']['id']==1 || $_SESSION['user']['id']==10134 || $_SESSION['user']['id']==10096){
				?>
 ...
 <div class="row">
              ...
              <div class="col-3 text-center" style="margin-top:25px">
                ...
                <br>
               <p style="border-top:1px solid"><strong>  Authorized Signature  </strong></p>
