...
while($undel = mysql_fetch_object($res)){
  ...
  VALUES("'.$YR.'","'.$ch_cy_id.'","'.$chalan_no.'","'.$undel->chalan_date.'","'.$undel->id.'","'.$undel->do_no.'","'.$undel->job_no.'","'.$undel->do_date.'","'.$item_id.'","'.$undel->dealer_code.'","'.$rate.'","'.$pkt_size.'","'.$pkt_unit.'","'.$qty.'","'.$qty.'","'.$amount.'","'.$discount.'","'.$ms_data->depot_id.'","'.$ms_data->group_for.'","'.$rec_name.'","'.$rec_mob.'","'.$vehicle_no.'","'.$driver_name.'","'.$driver_mobile.'","'.$delivery_point.'","'.$delivery_man.'","'.$delivery_man_mobile.'","'.$entry_by.'","'.$entry_at.'","CHECKED", "'.$undel->cost_price.'", "'.$cost_amt.'")';
mysql_query($so_invoice);
journal_item_control($item_id, $ms_data->depot_id, $undel->chalan_date,  0, $qty, 'Sales', $undel->id, $rate, '', $chalan_no, '', '',$ms_data->group_for, $rate->unit_price, '' );


echo $d_qty = find_a_field('sale_do_details','sum(total_unit)','do_no="'.$undel->do_no.'"');
echo $c_qty = find_a_field('sale_do_chalan','sum(total_unit)','do_no="'.$undel->do_no.'" ');
if($d_qty==$c_qty){
echo $update = 'update sale_do_master set status="COMPLETED" where do_no="'.$undel->do_no.'"';
mysql_query($update);
}
                        ...
                        <div class="form-group row m-0 pb-1">
                            ...
                            <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
        						...
        						<select name="vehicle_no" id="vehicle_no" list="vehicle_list" onchange="get_delivery_info()">
								 <option></option>
								 <? foreign_relation('vehicle_info','vehicle_id','concat(vech_reg_no,"#",vehicle_name,"#",driver_name)',$vehicle_no,'1 and depot_id="'.$_SESSION['user']['depot'].'"');?>
...
?>
          ...
          <? }?>
			...
			<?
			  ...
			  $sql='select a.id,m.do_no, a.item_id,  a.unit_price, b.item_name,  b.unit_name,  a.total_unit as qty 
 from sale_do_master m,sale_do_details a,item_info b, item_sub_group s where m.do_no=a.do_no and m.status="CHECKED" and b.item_id=a.item_id 
...
$res=mysql_query($sql);
          ...
          <tr>
			...
			<input type="hidden" name="rate_<?=$row->id?>" id="rate_<?=$row->id?>" value="<?=$row->unit_price?>" />	</td>
              ...
              <td align="center"><? echo number_format($so_qty = (find_a_field('sale_do_chalan','sum(total_unit)','do_no="'.$row->do_no.'" and item_id="'.$row->item_id.'"')*(1)),2);?></td>
