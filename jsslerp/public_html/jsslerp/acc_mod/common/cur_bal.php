...
$a1=mysql_fetch_row(mysql_query($a2));
if($a1[0]<0)
$a1[0]=$a1[0]*(-1);
else
if(is_null($a1[0]))
...
if(isset($ledger_id))
	{
		$chk_balance = mysql_query("SELECT SUM(cr_amt)-SUM(dr_amt) AS 'balance' FROM journal WHERE ledger_id=$ledger_id");
		$balance 	 = mysql_result($chk_balance,0,'balance');
		if( $balance < 0 )
			$balance *= (-1);
		//$balance 		= $str;	
	}
