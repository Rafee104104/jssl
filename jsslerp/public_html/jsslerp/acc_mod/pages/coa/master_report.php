...
<th bgcolor="#66CCCC">TOTAL AMT</th>
		...
		<?
		while($row=mysql_fetch_object($rescl)){
...
$o=0;
		$po_sql = 'SELECT po_no, invoice_no FROM purchase_master WHERE vendor_id="'.$row->vendor_id.'" and po_date="'.$_REQUEST['t_date'].'" and status="CHECKED" GROUP by po_no order by  invoice_no';
			$po_query=mysql_query($po_sql);
			while($po_data= mysql_fetch_object($po_query)){
			$o++;
			if ($o>1) echo ', ';
...
elseif($_POST['report']==251022001)
{
		...
		<th width="5%" bgcolor="#d6eaf8" style="text-align:center">L-02</th>
		<th width="14%" bgcolor="#d6eaf8" style="text-align:center">SUB CLASS </th>
		<th width="6%" bgcolor="#d6eaf8" style="text-align:center">L-03</th>
		<th width="14%" bgcolor="#d6eaf8" style="text-align:center">SUB SUB CLASS   </th>
		<th width="7%" bgcolor="#d6eaf8" style="text-align:center">L-04</th>
		<th width="17%" bgcolor="#d6eaf8" style="text-align:center">GL Group </th>
		<th width="5%" bgcolor="#d6eaf8" style="text-align:center">L-05</th>
		<th width="18%" bgcolor="#d6eaf8" style="text-align:center">Head OF ACCOUNTS  </th>
		</tr>
		</thead><tbody>
		<? $sl=1;
		...
		$sql = "select group_id, group_name from ledger_group where 1";
		 ...
		 $sql="select a.* from accounts_ledger a  where group_for='".$_SESSION[user][group]."' ".$acc_con." ";
...
elseif($_POST['report']==251022002)
{
		...
		<td  width="50%" style="border:0px; border-color:white;">
			<table width="100%"  >
				...
				<tr align="center" >
					<td style="font-size:14px; border:0px; border-color:white;"><strong>
					    Date: <?=$_POST['fdate'];?> To <?=$_POST['tdate'];?></strong></td>	
				</tr>
		...
		<tr style="text-transform:uppercase">
		<th width="2%" bgcolor="#d6eaf8">L-01</th>
		
	
		<th width="15%" bgcolor="#d6eaf8" style="text-align:center">Head OF ACCOUNTS  </th>
		
		<th width="12%" bgcolor="#d6eaf8" style="text-align:center">Budget Amount</th>
		<th width="12%" bgcolor="#d6eaf8" style="text-align:center">Expense Amount</th>
		<th width="12%" bgcolor="#d6eaf8" style="text-align:center">Current Balance</th>
		</tr>
		<tr>
		    <th></th>
		    <th colspan="4" align="left">Regular Expenses- Factory</th>
		</tr>
		</thead><tbody>
		<? $sl=1;
		...
		$sql = "select sum(dr_amt-cr_amt) as closing,ledger_id from journal where jv_date between '".$_POST['fdate']."' and '".$_POST['tdate']."' group by ledger_id";
		 $query = mysql_query($sql);
		 while($info=mysql_fetch_object($query)){
  		 $clsoing_balance[$info->ledger_id]=$info->closing;
		}
		
		
		$sql = "select sum(dr_amt) as budget,ledger_id from budget_balance where jv_date between '".$_POST['fdate']."' and '".$_POST['tdate']."' group by ledger_id";
		 $query = mysql_query($sql);
		 while($info=mysql_fetch_object($query)){
  		 $budget_balance[$info->ledger_id]=$info->budget;
		}

		$s=1;
		 $sql="select a.* from accounts_ledger a  where a.ledger_group_id in (417001)";
        ...
        if($clsoing_balance[$row->ledger_id]!=0 || $budget_balance[$row->ledger_id]!=0)
        {
		...
		<td align="right"><?=round($b_amt=$budget_balance[$row->ledger_id],0); $tot_b_amt+=$b_amt; ?></td>
		<td align="right">
			
			<?
				$e_amt=$clsoing_balance[$row->ledger_id];
			//find_a_field('journal','sum(dr_amt-cr_amt)','ledger_id="'.$row->ledger_id.'" and jv_date<="'.$_POST['tdate'].'" ');
			if($e_amt<0){
				echo round($cost_amt=$e_amt*(-1),0);
		...
		<td align="right"><? round($bal=$b_amt-$e_amt,0); 
...
<?  } } ?>
	...
	<tr>
		    ...
		    <td align="right"><strong><?=$tot_b_amt?></strong></td>
		    <td align="right"><strong><?=$tot_e_amt?></strong></td>
		    <td align="right"><strong><?=$tot_bal?></strong></td>
		...
		<tr>
		    <th></th>
		    <th colspan="4" align="left">Regular Expenses- Administrative Overhead</th>
		</tr>
		</thead><tbody>
		<? $sl=1;
		...
		$s=1;
		 $sql="select a.* from accounts_ledger a  where a.ledger_group_id in (4113002)";
		...
		<td width="15%"><?=$row->ledger_name;?></td>
		
		<td width="12%" align="right"><?=round($b_amt=$budget_balance[$row->ledger_id],0); $tot_b_amt2+=$b_amt; ?></td>
		...
		<td width="12%" align="right"><? round($bal=$b_amt-$e_amt,0); 
...
<?  } } ?>
	...
	<tr>
		    ...
		    <td colspan="2" align="center"><strong>Sub Total</strong></td>
		    <td align="right"><strong><?=$tot_b_amt2?></strong></td>
		    <td align="right"><strong><?=$tot_e_amt2?></strong></td>
		    <td align="right"><strong><?=$tot_bal2?></strong></td>
		...
		<tr>
		    ...
		    <td align="right"><strong><?=$tot_b_amt+$tot_b_amt2?></strong></td>
		    <td align="right"><strong><?=$tot_e_amt+$tot_e_amt2?></strong></td>
		    <td align="right"><strong><?=$tot_bal+$tot_bal2?></strong></td>
