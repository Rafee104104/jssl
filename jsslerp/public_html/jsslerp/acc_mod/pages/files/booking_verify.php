...
$title='Paid Booking Varification';
$now=time();
							...
							if($_SESSION['user']['group']>1) $group_s='AND j.group_for='.$_SESSION['user']['group'];
									...
									if($_POST['dealer_group']!='') {$dealer_group_con=' and d.product_group="'.$_POST['dealer_group'].'"';}
									   $sql="SELECT DISTINCT 
											  ...
											  j.checked,
											  j.jv_no,
											  u.fname,m.booking_id,m.booking_number_eng
											 ...
											 d.account_code = l.ledger_id and 
											  j.tr_from in('Paid Booking','Normal Booking') AND 
											  j.tr_no = m.booking_id and d.dealer_code_eng = m.agent_id and j.cr_amt>0 and 	  j.jv_date between '".$_POST['do_date_fr']."' AND '".$_POST['do_date_to']."'  ".$group_s." ".$depot_con.$checked_con.$dealer_type_con.$dealer_group_con." 
						 ...
						 // $received = $received + ($data->cr_amt);
						  ?>
                            ...
                            <tr <?=($i%2==0)?'class="alt"':'';?> >
								  ...
								  <td align="center"><? echo $data->booking_id;?></td>
								  <td align="center"><? echo $data->booking_number_eng;?></td>
							...
							if(($data->checked=='YES')){
							?>
							<input type="button" name="Button" value="YES"  onclick="window.open('sales_sec_print_view3.php?jv_no=<?=$data->jv_no;?>');" class="btn1 btn1-bg-submit"/>
							<?
							}elseif(($data->checked=='NO')){
							?>
							<input type="button" name="Button" value="NO"  onclick="window.open('sales_sec_print_view3.php?jv_no=<?=$data->jv_no;?>');" class="btn1 btn1-bg-cancel"/>
							<? }?>
          </span></td>
                        </tr>
