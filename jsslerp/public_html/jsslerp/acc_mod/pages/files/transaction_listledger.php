...
$ledger_id=$_REQUEST["ledger_id"];
	if(substr($ledger_id,12,12)=='000000000000')
...
?>
<?php  $led=mysql_query("select ledger_id,ledger_name from accounts_ledger where 1 order by ledger_name");
      $data = '[';
      $data .= '{ name: "All", id: "%" },';
					...
					<div class="container">
						...
						<div class="form-group row  m-0 mb-1 pl-3 pr-3">
							...
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">
								...
								<input name="ledger_id" list="ledger_ids" id="ledger_id" value="<?=$_POST['ledger_id']?>" class="form-control" autocomplete="off"  />

									<datalist id="ledger_ids">
									<option value="%">All</option>

									<?
									foreign_relation('accounts_ledger a, ledger_group g, acc_sub_sub_class s','a.ledger_id','concat(a.ledger_id,"::",a.ledger_name,">",g.group_name,">>",s.sub_sub_class_name)','',"a.ledger_group_id=g.group_id and g.acc_sub_sub_class=s.id   order by a.ledger_id");
									
									//foreign_relation('accounts_ledger','ledger_id','concat(ledger_name,"","","ledger_name")',$ledger_id,"1  order by ledger_id");
						...
						<div class="form-group row  m-0 mb-1 pl-3 pr-3">
							...
							<option value="Sales"<?php if($vtype=='Sales') echo "selected"?>>Sales</option>
							<option value="Purchase"<?php if($vtype=='Purchase') echo "selected"?>>Purchase</option>
							<option value="Purchase Return"<?php if($vtype=='Purchase Return') echo "selected"?>>Purchase Return</option>
							<option value="Paid Booking"<?php if($vtype=='Paid Booking') echo "selected"?>>Paid Booking</option>
							<option value="Bag Sales"<?php if($vtype=='Bag Sales') echo "selected"?>>Bag Sales</option>
							<option value="Consumption"<?php if($vtype=='Consumption') echo "selected"?>>Consumption</option>
							<option value="SR Loan"<?php if($vtype=='SR Loan') echo "selected"?>>SR Loan</option>
							<option value="SR Loan Return"<?php if($vtype=='SR Loan Return') echo "selected"?>>SR Loan Return</option>
				...
				<div id="grp">
					...
					<table class="table1  table-striped table-bordered table-hover table-sm" style="zoom:90%">
						...
						if(isset($_REQUEST['show']))
						{
							...
							$total_sql = "select sum(a.dr_amt),sum(a.cr_amt) from journal a,accounts_ledger b".$d_table." where a.ledger_id=b.ledger_id and a.jv_date between '$fdate' AND '$tdate' and a.ledger_id like '$sledger_id%' ".$cc_con.$d_where.$v_type;
							...
							$c="select sum(a.dr_amt)-sum(a.cr_amt) from journal a,accounts_ledger b".$d_table." where a.ledger_id=b.ledger_id and a.jv_date<'$fdate' and a.ledger_id like '$sledger_id%'  ".$cc_con.$d_where.$v_type;

							 $p="select a.jv_date,b.ledger_name,a.dr_amt,a.cr_amt,a.tr_from,a.narration,a.jv_no,a.tr_no,a.jv_no,a.cheq_no,a.cheq_date,a.relavent_cash_head , a.cc_code,a.cc_code
		 from journal a,accounts_ledger b".$d_table." where a.ledger_id=b.ledger_id and a.jv_date between '$fdate' AND '$tdate' ".$cc_con.$v_type." and a.ledger_id like '$sledger_id%'  ".$d_where;
							...
							<?php } ?>
							<tr >
								<th colspan="6" class="bg-table1">Closing Balance : <?php echo number_format($t_total,2)." ".$t_type?> </th>
