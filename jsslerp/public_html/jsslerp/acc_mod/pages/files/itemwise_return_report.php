...
?>
<?php $led=mysql_query("SELECT group_id, inventory_type FROM item_group GROUP BY inventory_type");
...
$led1 = mysql_query("SELECT group_id, group_name FROM `item_group`");
if(mysql_num_rows($led1) > 0)
{
	$data1 = '[';
	while($ledg = mysql_fetch_row($led1)){
...
$led2 = mysql_query("select * from item_info");
if(mysql_num_rows($led2) > 0)
{
	$data2 = '[';
	while($ledg = mysql_fetch_row($led2)){
