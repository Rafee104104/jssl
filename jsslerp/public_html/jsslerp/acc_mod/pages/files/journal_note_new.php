...
$tr_no = $_SESSION['tr_no'];
	...
	$update_dr="update secondary_journal set cv_no='".$cv_no."' where jv_no='".$jv_no."'";

	mysql_query($update_dr);
	}
}else{
	 if($jv_no>0){	
	 ...
	 $update_cr="update secondary_journal set cv_no='".$cv_no."' where jv_no='".$jv_no."'";

	 mysql_query($update_cr);
	}
...
if(isset($_POST['limmit'])){
	  ...
	  if($_SESSION['tr_no']>0){
		if($_SESSION['jv_no']>0){
			...
			$up = 'update secondary_journal set checked="NO", $cv_no="'.$_REQUEST['cv_no'].'" where jv_no="'.$_SESSION['jv_no'].'"';
...
if($sa_config=="Yes"){
 ...
 $sa_up='update secondary_journal set secondary_approval="Yes",cv_no="'.$j_data->cv_no.'", om_checked_at="'.$time_now.'", om_checked="'.$_SESSION['user']['id'].'" where jv_no="'.$j_data->jv_no.'" and tr_from="'.$tr_from.'"';
...
$time_now = date('Y-m-d H:i:s');
 ...
 $up2='update secondary_journal set checked="YES",cv_no="'.$j_data->cv_no.'",checked_at="'.$time_now.'", checked_by="'.$_SESSION['user']['id'].'" where jv_no="'.$j_data->jv_no.'" and tr_from="'.$tr_from.'"';

mysql_query($up2);

  $sa_up2='update journal set secondary_approval="Yes",cv_no="'.$j_data->cv_no.'", checked="YES", checked_by="'.$_SESSION['user']['id'].'", checked_at="'.$time_now.'", om_checked_at="'.$time_now.'" ,om_checked="'.$_SESSION['user']['id'].'" where jv_no="'.$j_data->jv_no.'" and tr_from="'.$tr_from.'"';
mysql_query($sa_up2);
...
$sa_up='update secondary_journal set secondary_approval="No",cv_no="'.$j_data->cv_no.'" where jv_no="'.$j_data->jv_no.'" and tr_from="'.$tr_from.'"';
			...
			<div class="row">
				...
				<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6">
					<div class="container n-form2">
						...
						<div class="form-group row m-0 pb-1">
							<label class="col-sm-2 col-md-2 col-lg-2 col-xl-2 m-0 p-0 d-flex justify-content-end align-items-center pr-1 font-size12 bg-form-titel-text">CV No:</label>
							<div class="col-sm-10 col-md-10 col-lg-10 col-xl-10 p-0 pr-2">
							<? 
									$cv_no = find_a_field('secondary_journal','cv_no','tr_from = "Journal"  and entry_by="'.$_SESSION['user']['id'].'" and cv_no != "" order by jv_no desc ');
									$datas = explode("/",$cv_no);
									$checked = find_a_field('secondary_journal','checked','tr_from = "Journal"  and entry_by="'.$_SESSION['user']['id'].'" and cv_no != "" order by jv_no desc ');						if($checked=='YES')
									{
									
									$cv_no = $datas[0].'/'.($datas[1]+1);
									
									}
									else
									{
									
										$cv_no = $datas[0].'/'.($datas[1]);
									}
								?>	
								<input name="cv_no" type="text" id="cv_no" tabindex="3" value="<?php echo $cv_no?>" <? //if($jv->tr_no>0) echo 'readonly'?> class="form-control"/>
							</div>
						</div>
				...
				<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6">
					<div class="container n-form2">
						<table  class="table1  table-striped table-bordered table-hover table-sm" border="0" cellspacing="0" cellpadding="0">
							...
							$sql2="select a.tr_no, a.cr_amt, a.narration,a.jv_date , a.jv_no,a.checked,a.cv_no from  secondary_journal a where a.tr_from='Journal' and  SUBSTRING(a.tr_no,5,1)='0' and  a.cr_amt>0 and a.group_for=".$_SESSION['user']['group']." group by a.tr_no order by a.id desc limit 5";
...
document.getElementById('ledger_id').value);" tabindex="5"  value="<?=$jv_data->ledger_id?>" />-->
						...
						<input type="text" id="ledgerID"   name="ledger_id" list="ledger_ids" onblur="getData2('acc_reference_ajax.php', 'acc_reference', this.value,
								document.getElementById('ledgerID').value);" tabindex="11"   value="<?=$jv_data->ledger_id?>"/>
							<datalist id="ledger_ids">	
							 <?  
						foreign_relation('accounts_ledger a, ledger_group g, acc_sub_sub_class s','a.ledger_id','concat(a.ledger_id,"::",a.ledger_name,">>>",g.group_name,">>>",s.sub_sub_class_name)','',"a.ledger_group_id=g.group_id and g.acc_sub_sub_class=s.id and a.parent=0  and  a.balance_type IN ('Credit','Both')  order by a.ledger_id");?>
							</datalist>
					</td>
					...
					<td>
						<span id="acc_reference">
							<table width="100%" border="1" align="center"  style="border-collapse:collapse; border:1px solid #C1DAD7;" cellpadding="2" cellspacing="2">
								<tr>
									<td><input type="text" class="form-control" id="ledger_name" name="ledger_name" tabindex="6" value="<?=$jv_data->ledger_name?>" />
...
?>
            ...
            <tr>
              <td width="6%" align="center" bgcolor="#45777B"><span class="style3">Type</span></td>
              <td width="8%" align="center" bgcolor="#45777B"><span class="style3">GL Code </span></td>
              <td width="20%" align="center" bgcolor="#45777B"><span class="style3">GL Name </span></td>
...
<table width="100%" border="1" align="center"  style="border-collapse:collapse; border:1px solid #C1DAD7;" cellpadding="2" cellspacing="2">
  ...
  <? foreign_relation('cost_center','id','center_name',$jv_data->cc_code,"1");?>
              ...
              <td align="center">
              	...
              	<input name="detail" type="text" id="detail"  tabindex="10" class="form-control"/>              </td>
