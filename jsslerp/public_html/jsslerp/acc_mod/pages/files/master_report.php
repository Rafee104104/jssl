...
<!--		<img src="../../../logo/fg-logo.png" style="width:100px;">-->
			<img src="../../../logo/<?=$_SESSION['proj_id']?>.png" style=" height:70px; width:auto;" />
		</td>
...
'.$group_for_con.$zone_con.$item_sub_con.$item_group_con.$item_mother_group_con.$item_type_con.$dealer_con.'  group by c.dealer_code, month(a.chalan_date), year(a.chalan_date) order by a.chalan_date';
	   	...
	   	$year_query = mysql_query($year_sql );
		while($value = mysql_fetch_object($year_query)){
	  ...
	  ?>
		...
		'.$group_for_con.$zone_con.$item_sub_con.$item_group_con.$item_mother_group_con.$item_type_con.$dealer_con.' group by  c.dealer_code  order by c.zone_code';
	   	$query_101 = mysql_query($sql_101 );
		$sl = 1 ;
		while($r = mysql_fetch_object($query_101)){
...
$pt = mysql_query($p);
while($pur_t=mysql_fetch_object($pt)){
$pur_t->pur_qty;
}
...
while($dealer=@mysql_fetch_object($query)){
...
elseif($_REQUEST['report']==210925)
{
?>

<table width="100%" border="1" cellpadding="0" cellspacing="0">
<tr>
<th style="border:none;" colspan="8"><?=$str;?></th>
</tr>
<tr>
<th style="border:none; text-align:center" colspan="8"><?=find_a_field('user_group','address','1');?></th>
</tr>
<tr>
<th style="border:none; text-align:center" colspan="8">Date : <?=$_POST['f_date'];?> To <?=$_POST['t_date']?></th>
</tr>
<tr>
<th style="border:none; text-align:center" colspan="8">S.R Loan Return Report</th>
</tr>
  <tr>
    <th>SL No</th>
    ...
    <th>Ledger Name </th>
    <th>Party Name</th>
    <th>SR Loan <?=$_POST['booking_year']?>/ Payment</th>
    <th>Collection Amount</th>
    <th>Due Amount</th>
  </tr>
  <?
  
  $ledger=explode('-',$_POST['ledger_id']);
  $ledger_id=$ledger[0];
  $dealer=find_a_field('dealer_info','dealer_code_eng','account_code="'.$ledger_id.'"');
  if($ledger_id!='')
  {
  	$agent=' and p.agent_id="'.$dealer.'"';
  
  }
  
  $s=1;
   $sql="select p.*,r.*,sum(r.amount) as amt from paid_booking p,sr_loan r where p.booking_number_eng=r.booking_number and p.booking_year='".$_POST['booking_year']."' ".$agent." group by r.booking_number ";
  $q=mysql_query($sql);
  while($r=mysql_fetch_object($q))
{
  ...
  <tr>
    ...
    <td><?=find_a_field('dealer_info','dealer_name2','dealer_code_eng='.$r->agent_id);?>;</td>
    <td><?=$r->name?></td>
    <td><?=$r->amt; $tot_amt+=$r->amt;?></td>
    <td><? $delivery_collection=find_a_field('sale_do_chalan','sum(sr_loan)','booking_no="'.$r->booking_number.'" and chalan_date between "'.$_POST['f_date'].'" and "'.$_POST['t_date'].'" ');
	$sep_return=find_a_field('sr_loan_return','sum(total_paid)','booking_number="'.$r->booking_number.'" and recdate between "'.$_POST['f_date'].'" and "'.$_POST['t_date'].'" and chalan_no is null ');
	echo $tot_collection=$delivery_collection+$sep_return;
	$tot_collect+=$tot_collection;
	?></td>
    <td><?=$due=$r->amount-$tot_collection; $tot_due+=$due;?></td>
  </tr>
  <? } ?>
  <tr>
  	<td colspan="5" align="center"><strong>Total</strong></td>
	<td><strong><?=$tot_amt?></strong></td>
	<td><strong><?=$tot_collect?></strong></td>
...
elseif($_REQUEST['report']==2109255)
{
?>
<table width="100%" border="1" cellpadding="0" cellspacing="0" id="ExportTable">
	<tr>
    <th colspan="6" style="border:none"><?=$str;?></th>
  
  </tr>
  <tr>
    <th colspan="6" style="border:none; text-align:center; font-size:13px;">Item Wise Requisition/Purchase/Consumption Report</th>
  
  </tr>
  <tr>
    <th colspan="6" style="border:none; text-align:center; font-size:13px;">Date: <?=$_POST['f_date'];?> To  <?=$_POST['t_date'];?></th>
  
  </tr>
   <tr>
    <th colspan="6" style="border:none; text-align:center;">&nbsp;</th>
  ...
  if($_POST['item_id']!='')
  {
  	$item=' and item_id='.$_POST['item_id'].'';
  
  }
  
  	$sql2="select sum(o.qty) as tot_qty,o.item_id from item_info i, requisition_order o,requisition_master m where m.req_no=o.req_no and m.status!='MANUAL' and i.item_id=o.item_id and  o.req_date between '".$_POST['f_date']."' and '".$_POST['t_date']."' group by o.item_id";
	$re=mysql_query($sql2);
	while($req=mysql_fetch_object($re))
	{
		$req_qty[$req->item_id]=$req->tot_qty;
	}
  	
	$sql3="select sum(r.qty) as tot_qty,r.item_id from item_info i, purchase_receive r where i.item_id=r.item_id and  r.rec_date between '".$_POST['f_date']."' and '".$_POST['t_date']."' group by r.item_id";
	$re3=mysql_query($sql3);
	while($rec=mysql_fetch_object($re3))
	{
		$rec_qty[$rec->item_id]=$rec->tot_qty;
	}
	
	$sql3="select sum(r.qty) as tot_qty,r.item_id from item_info i, warehouse_other_issue_detail r,warehouse_other_issue m where m.oi_no=r.oi_no and m.status!='MANUAL' and i.item_id=r.item_id and  r.oi_date between '".$_POST['f_date']."' and '".$_POST['t_date']."' group by r.item_id";
	$re3=mysql_query($sql3);
	while($rec=mysql_fetch_object($re3))
	{
		$issue_qty[$rec->item_id]=$rec->tot_qty;
	}
	
  	$s=1;
  	 $sql='select * from item_info where 1 and item_id!=100010001 '.$item.'';
	$q=mysql_query($sql);
	while($r=mysql_fetch_object($q))
	{
  ?>
  <tr>
    ...
    <td><?=$r->item_name;?></td>
    <td><?=$r->unit_name;?></td>
    <td align="right"><?=$req_qty[$r->item_id]; $t_req+=$req_qty[$r->item_id]; ?></td>
    <td align="right"><?=$rec_qty[$r->item_id]; $t_rec+=$rec_qty[$r->item_id]; ?></td>
	 <td align="right"><?=$issue_qty[$r->item_id]; $t_issue+=$issue_qty[$r->item_id];?></td>
   
  </tr>
  <? } ?>
  <tr>
  	<td colspan="3" align="center"><strong>Total</strong></td>
	<td align="right"><strong><?=$t_req?></strong></td>
	<td align="right"><strong><?=$t_rec?></strong></td>
	<td align="right"><strong><?=$t_issue?></strong></td>
  </tr>
</table>

<?
}
elseif($_REQUEST['report']==21092555)
{
?>
<table width="100%" border="1" cellpadding="0" cellspacing="0" id="ExportTable">
	<tr>
    <th colspan="11" style="border:none"><?=$str;?></th>
  </tr>
  <tr>
    <th colspan="11" style="border:none; text-align:center; font-size:13px;">Item Wise Requisition/Purchase/Consumption Amount Report</th>
  </tr>
  <tr>
    <th colspan="11" style="border:none; text-align:center; font-size:13px;">Date: <?=$_POST['f_date'];?> To  <?=$_POST['t_date'];?></th>
  </tr>
   <tr>
    <th colspan="11" style="border:none; text-align:center;">&nbsp;</th>
  </tr>
  <tr>
    <th>SL No </th>
    <th>Item Name </th>
  ...
  if($_POST['item_id']!='')
  {
  	...
  	$sql2="select sum(o.qty) as tot_qty,sum(o.est_price) as est_amt,o.item_id from item_info i, requisition_order o,requisition_master m where m.req_no=o.req_no and m.status!='MANUAL' and i.item_id=o.item_id and  o.req_date between '".$_POST['f_date']."' and '".$_POST['t_date']."' group by o.item_id";
	$re=mysql_query($sql2);
	while($req=mysql_fetch_object($re))
	{
		$req_qty[$req->item_id]=$req->tot_qty;
		$req_est[$req->item_id]=$req->est_amt;
	}
  	
	$sql3="select sum(r.qty) as tot_qty,sum(r.amount) as rec_amt,r.item_id from item_info i, purchase_receive r where i.item_id=r.item_id and  r.rec_date between '".$_POST['f_date']."' and '".$_POST['t_date']."' group by r.item_id";
	$re3=mysql_query($sql3);
	while($rec=mysql_fetch_object($re3))
	{
		$rec_qty[$rec->item_id]=$rec->tot_qty;
		$rec_amtt[$rec->item_id]=$rec->rec_amt;
	}
	
	$sql3="select sum(r.qty) as tot_qty,sum(r.amount) as issue_amt,r.item_id from item_info i, warehouse_other_issue_detail r,warehouse_other_issue m where m.oi_no=r.oi_no and m.status!='MANUAL' and i.item_id=r.item_id and  r.oi_date between '".$_POST['f_date']."' and '".$_POST['t_date']."' group by r.item_id";
	$re3=mysql_query($sql3);
	while($rec=mysql_fetch_object($re3))
	{
		$issue_qty[$rec->item_id]=$rec->tot_qty;
		$issue_amtt[$rec->item_id]=$rec->issue_amt;
	}
  ...
  <tr>
    ...
    <td><?=$r->item_name;?></td>
    <td><?=$r->unit_name;?></td>
    <td align="right"><?=$req_qty[$r->item_id]; $treq_qty+=$req_qty[$r->item_id]; ?></td>
    <td align="right"><?=$req_est[$r->item_id]; $t_req+=$req_est[$r->item_id]; ?></td>
    <td align="right"><?=$rec_qty[$r->item_id]; $trec_qty+=$rec_qty[$r->item_id]; ?></td>
    <td align="right"><?=number_format($rec_amtt[$r->item_id]/$rec_qty[$r->item_id],2);  ?></td>
    <td align="right"><?=$rec_amtt[$r->item_id]; $t_rec+=$rec_amtt[$r->item_id]; ?></td>
	 <td align="right"><?=$issue_qty[$r->item_id]; $tissue_qty+=$issue_qty[$r->item_id];?></td>
	 <td align="right"><?=number_format($issue_amtt[$r->item_id]/$issue_qty[$r->item_id],2);  ?></td>
	 <td align="right"><?=$issue_amtt[$r->item_id]; $t_issue+=$issue_amtt[$r->item_id];?></td>
  </tr>
  <? } ?>
  <tr>
  	<td colspan="3" align="center"><strong>Total</strong></td>
	<td align="right"><?=$treq_qty?></td>
	<td align="right"><strong><?=$t_req?></strong></td>
	<td align="right"><?=$trec_qty?></td>
	<td align="right">&nbsp;</td>
	<td align="right"><strong><?=$t_rec?></strong></td>
	<td align="right"><?=$tissue_qty?></td>
	<td align="right">&nbsp;</td>
	<td align="right"><strong><?=$t_issue?></strong></td>
  </tr>
</table>
