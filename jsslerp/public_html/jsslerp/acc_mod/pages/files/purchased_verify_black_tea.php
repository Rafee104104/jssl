    ...
    <div class="form-container_large">
	  ...
	  $i=0;
		...
		if($_POST['vendor_id']!='')
		 ...
		 $vendor_con=' and l.ledger_id="'.$vendor_ledger.'"';}
	    ...
	    $sql="SELECT DISTINCT 
				...
				WHERE
				  ...
				  j.tr_from = 'Purchase' AND 
					j.ledger_id=v.ledger_id and 
				 

				  j.jv_date between '".$_POST['do_date_fr']."' AND  '".$_POST['do_date_to']."' AND j.ledger_id = l.ledger_id ".$group_s.$checked_con.$depot_con.$vendor_con." group by j.jv_no order by j.jv_no desc";
...
$received = $received + $data[1];
	...
	<tr>
      ...
      <td ><? echo  find_a_field ('purchase_receive','po_no','pr_no='.$data[0]); ?></td>

      <td><? echo $data[0];?></td>

      <td><? echo $data[2];?></td>
      <td><? echo find_a_field ('vendor','vendor_name','ledger_id='.$data[8]);?></td>

      

      <td><? echo find_a_field ('user_activity_management','fname','user_id='.$data[7]); ?></td>
	  <td><?=number_format($data[1],2)?> </td>

      <td><a target="_blank" href="purchase_sec_print_view.php?jv_no=<?=$data[3] ?>"><img src="../images/print_hover.png" width="20" height="20" /></a></td>
		  ...
		  <span id="divi_<?=$data[0]?>">
            ...
            <? 
			  ...
			  if(($data[9]=='YES')){
					...
					<input type="button" name="Button" value="YES"  onclick="window.open('purchase_sec_print_view.php?jv_no=<?=$data[3] ?>');" class="btn1 btn1-bg-submit"/>
					...
					<input type="button" name="Button" value="NO"  onclick="window.open('purchase_sec_print_view.php?jv_no=<?=$data[3] ?>');" class="btn1 btn1-bg-cancel"/>
