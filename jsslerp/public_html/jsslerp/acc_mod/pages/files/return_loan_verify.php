...
$title='SR Loan Return Varification';
$now=time();
		  ...
		  ?>
						 ...
						 // $received = $received + ($data->cr_amt);
						  ?>
							...
							if(($data->checked=='YES')){
							?>
							<input type="button" name="Button" value="YES"  onclick="window.open('sales_sec_print_view5.php?jv_no=<?=$data->jv_no;?>');" class="btn1 btn1-bg-submit"/>
							<?
							}elseif(($data->checked=='NO')){
							?>
							<input type="button" name="Button" value="NO"  onclick="window.open('sales_sec_print_view5.php?jv_no=<?=$data->jv_no;?>');" class="btn1 btn1-bg-cancel"/>
							<? }?>
          </span></td>
                        </tr>
