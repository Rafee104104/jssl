						...
						<div class="form-group row  m-0 mb-1 pl-3 pr-3">
							...
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">
								...
								<select name="ledger_id" id="ledger_id" class="form-control">
									...
									foreign_relation('accounts_ledger','ledger_id','ledger_name',$b_id,"ledger_group_id=".$bank_group."  order by ledger_id");
		...
		<div class="container-fluid">
			...
			<div id="reporting">
				<div id="grp">
					...
					<table class="table1  table-striped table-bordered table-hover table-sm" style=" width: 100%; border-collapse: collapse; ">
						...
						if(isset($_REQUEST['show']))
						{
							...
							$join=" and tr_from in('Receipt','Payment','Opening','Journal_info','Cash Purchase','Contra')";
							...
							?>
							<tr<?=$cls?>>
							    <td colspan="8">Opening</td>
								<td><?php if($balance>0) echo '(Dr)'.number_format($balance,2); elseif($balance<0) echo '(Cr) '.number_format(((-1)*$balance),2);else echo "0.00";?></td>
							</tr>
							...
							while($rp=mysql_fetch_row($report)){
								...
								<tr<?=$cls?>>
									...
									<td>
									<?php $balance = $balance+($rp[3]-$rp[2]);  if($balance>0) echo '(Dr) '.number_format($balance,2); elseif($balance<0) echo '(Cr) '.number_format(((-1)*$balance),2);else echo "0.00"; ?>
									</td>
								</tr>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" style="border-bottom:#FFFFFF">
						...
						<tr>
							...
							<td style="border-bottom:#FFFFFF; border-left:0px;border-right:0px; border-bottom:0px;"><div align="center">Prepared by Accountant</div></td>
