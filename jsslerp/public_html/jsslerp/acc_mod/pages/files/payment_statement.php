...
<?php
	if(isset($_REQUEST['show']))
	{
		...
		if($cc_code > 0)
		{
			$total=mysql_fetch_row(mysql_query("select sum(dr_amt),sum(cr_amt) from journal where tr_from='Payment' and jv_date>'$fdate' and jv_date<'$tdate' and ledger_id like '$ledger_id' and 1 AND cc_code=$cc_code"));
		...
		{
			$total=mysql_fetch_row(mysql_query("select sum(dr_amt),sum(cr_amt) from journal where tr_from='Payment' and jv_date>'$fdate' and jv_date<'$tdate' and ledger_id like '$ledger_id' and 1"));
