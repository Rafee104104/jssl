...
<option <? if(substr($data[2],0,1)==substr($liabilities,0,1)) echo 'Selected ';?>value="<?=$liabilities?>">Liabilities</option>
 ...
 <option <? if($datas->id==$data[9]) echo 'Selected ';?> value="<?=$datas->id?>"><?=$datas->group_name?></option>
    ...
    <div class="col-sm-10">
      <div class="md-form mt-0">
 <? if($data[0]==""){?><input name="ngroup" type="submit" id="ngroup" value="Record" onclick="return checkUserName()" class="btn" /><? }?>
 <? if($data[0]!=""){?><input name="mgroup" type="submit" id="mgroup" value="Modify" class="btn" /><? }?>
