				...
				for($i=0; $i<count($employee); $i++)
					{
						...
						//exit();
							$invoice_no=mysql_fetch_row(mysql_query("select MAX(jv_no) from journal LIMIT 1"));
							$jv= date("Ymd")."0000";
							if($invoice_no[0]>$jv)
						...
						$insertq=mysql_query("insert into emp_salary_info values(".$employee[$i].", ".$new_salary_dt.", ".$month[$i].",".$year[$i].", ".$basic_amt[$i].", ".$home_rent[$i].", ".$convence_bill[$i].", ".$phone_bill[$i].", ".$medical_allowance[$i].", ".$income_tax[$i].", ".$pf_amt[$i].", ".$advance[$i].", ".$arear[$i].", ".$total_attend[$i].", ".$jv_no.")");
						    ...
						    $id=mysql_query("select ei.emp_name,sl.ledger_id,sl.sub_ledger_id from sub_ledger sl join employee_info ei on sl.sub_ledger_id=ei.sub_ledger_id where ei.emp_id=".$employee[$i]."");
							$emp_name=mysql_result($id,0,'ei.emp_name');
							$ledger_id=mysql_result($id,0,'sl.ledger_id');
							$sub_ledger_id=mysql_result($id,0,'sl.sub_ledger_id');					
							
						$insertque=mysql_query("insert into journal values('".$proj_id."', ".$jv_no.", ".$new_salary_dt.", ".$ledger_id.", 'Salary Payable ".$emp_name." for the month of ".$month[$i]." ".$year[$i]."',0, ".$net_payable[$i].", 'Payroll',0, ".$sub_ledger_id.")");
					
					$insertquery=mysql_query($insertque);
					}	
			}
		if($action_type == 'update')
			{
				...
				for($i=0; $i<count($employee); $i++)
					...
					$new_salary_dt = strtotime($salary_datef[$i]);
						...
						$updateq=mysql_query("update emp_salary_info set salary_date=".$new_salary_dt.", month=".$month[$i].", year=".$year[$i].", basic_amt=".$basic_amt[$i].", home_rent=".$home_rent[$i].", convence_bill=".$convence_bill[$i].", phone_bill=".$phone_bill[$i].", medical_allowance=".$medical_allowance[$i].", income_tax=".$income_tax[$i].", pf_amt=".$pf_amt[$i].", advance=".$advance[$i].", arear=".$arear[$i].", total_attend=".$total_attend[$i]." where emp_id=".$employee[$i]." and month=$emonth and year=$eyear");

						$selectque="select jv_no from emp_salary_info where emp_id=$employee[$i] and month=$emonth and year=$eyear";
						$resultjvn1=mysql_query($selectque);
						$resultjvno=mysql_result($resultjvn1,0,'jv_no');
						$updateque=mysql_query("update journal set cr_amt =".$net_payable[$i]." where jv_no=".$resultjvno."");
					}
    ...
    <tr>
      ...
      <td width="35%" align="left"><label>
        <select name="dept_name" id="dept_name">
          <?php 
			$deptq=mysql_query("select dept_id,dept_name from depertment_info ");
			while($deptv=mysql_fetch_row($deptq))
	        { 
			?>
...
$info=mysql_fetch_row(mysql_query($sql));
?>
