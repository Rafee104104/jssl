...
?>
<?php $led=mysql_query("select group_id,group_name from ledger_group where group_for=".$_SESSION['user']['group']." order by group_id");
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  if(isset($_REQUEST['show']))
  {
		...
		if($cc_code > 0)
		{
		  $total=mysql_fetch_row(mysql_query("select sum(j.dr_amt),sum(j.cr_amt) from journal j, accounts_ledger a, ledger_group l where j.ledger_id=a.ledger_id and a.ledger_group_id=l.group_id and j.jv_date between '$fdate' AND '$tdate' and l.group_id = $ledger_id AND j.cc_code=$cc_code and l.group_for=".$_SESSION['user']['group']));
		...
		{
		   $total=mysql_fetch_row(mysql_query("select sum(j.dr_amt),sum(j.cr_amt) from journal j, accounts_ledger a, ledger_group l where j.ledger_id=a.ledger_id and a.ledger_group_id=l.group_id and j.jv_date between '$fdate' AND '$tdate' and l.group_id = $ledger_id and l.group_for=".$_SESSION['user']['group']));
