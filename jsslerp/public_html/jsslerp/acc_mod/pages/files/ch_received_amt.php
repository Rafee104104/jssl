							...
							if($_SESSION['user']['group']>1) $group_s='AND j.group_for='.$_SESSION['user']['group'];
									...
									if($_POST['dealer_group']!='') {$dealer_group_con=' and d.product_group="'.$_POST['dealer_group'].'"';}
									   $sql="SELECT DISTINCT 
											...
											group by  j.tr_no order by tr_no desc";
						 ...
						 // $received = $received + ($data->cr_amt);
						  ?>
                            ...
                            <tr <?=($i%2==0)?'class="alt"':'';?> >
								  ...
								  <td align="center"><? echo $data->booking_no;?></td>
								  <td align="center"><? echo find_a_field('paid_booking','name','booking_number_eng="'.$data->booking_no.'"');?></td>
							...
							if(($data->checked=='YES')){
							?>
							<input type="button" name="Button" value="YES"  onclick="window.open('sales_sec_print_view.php?jv_no=<?=$data->jv_no;?>');" class="btn1 btn1-bg-submit"/>
							<?
							}elseif(($data->checked=='')){
							?>
							<input type="button" name="Button" value="NO"  onclick="window.open('sales_sec_print_view.php?jv_no=<?=$data->jv_no;?>');" class="btn1 btn1-bg-cancel"/>
							<? }?>
          </span></td>
                        </tr>
