...
if(isset($_POST['limmit'])){
	  ...
	  if($_SESSION['tr_no']>0){
		if($_SESSION['jv_no']>0){
			...
			add_to_sec_journal($proj_id, $_SESSION['jv_no'], $j_data->jv_date, $j_data->relavent_cash_head,  $detail, '0',$_POST['t_amount'], $tr_from, $_SESSION['tr_no'],'','',0,'',$_SESSION['user']['id'],'',$j_data->received_from,$j_data->bank,$j_data->cheq_no,$j_data->cheq_date,$j_data->relavent_cash_head,$checked,
...
$up2='update secondary_journal set checked="YES",checked_at="'.$time_now.'",cv_no="'.$j_data->cv_no.'", checked_by="'.$_SESSION['user']['id'].'" where jv_no="'.$j_data->jv_no.'" and tr_from="'.$tr_from.'"';

mysql_query($up2);

$sa_up2='update journal set secondary_approval="Yes", checked="YES", checked_by="'.$_SESSION['user']['id'].'",cv_no="'.$j_data->cv_no.'", checked_at="'.$time_now.'", om_checked_at="'.$time_now.'" ,om_checked="'.$_SESSION['user']['id'].'" where jv_no="'.$j_data->jv_no.'" and tr_from="'.$tr_from.'"';
mysql_query($sa_up2);
  ...
  var check_type = document.getElementById('type').value;
    ...
    if(check_type=='CASH'){
	   ...
	   document.getElementById('cv_no').value = "<?php 
									...
									$cv_cash = find_a_field('secondary_journal','cv_no','tr_from = "Payment" and type="CASH" and entry_by="'.$_SESSION['user']['id'].'" and cv_no != "" order by jv_no desc ');
	   ...
	   document.getElementById('cv_no').value = "<?php 
  $cv_nos = find_a_field('journal','cv_no','tr_from = "Payment" and type="BANK" and entry_by="'.$_SESSION['user']['id'].'" and cv_no != "" order by jv_no desc ');
  ...
  echo $cv_nos; ?>";
	    ...
	    document.getElementById('cash_check').style.display='';
		 ...
		 document.getElementById('cv_no').value = "<?php 
									...
									$cv_cash = find_a_field('journal','cv_no','tr_from = "Payment" and entry_by="'.$_SESSION['user']['id'].'" and type="CASH" and cv_no != "" order by jv_no desc ');
									$datas_cash = explode("/",$cv_cash);
									$cv_cash = $datas_cash[0].'/'.($datas_cash[1]+1); echo $cv_cash; ?>";
							...
							<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6 pl-0">
								<div class="form-group row m-0 pb-1">
									...
									<div class="col-sm-7 col-md-7 col-lg-7 col-xl-7 p-0 pr-2 ">
									  <input name="jv_date" type="text" id="jv_date" value="<?php if($jv->jv_date=="0000-00-00"){echo $today=date('Y-m-d');}else{ echo $jv->jv_date; } ?>" size="10" <?php /*?><? if($jv->tr_no>0) echo 'disabled'?><?php */?>  tabindex="2"required class="form-control" />
									</div>
								</div>
							</div>
						</div>
	...
	<?php 
						...
						<div id="of_bank_check" class="form-group row m-0 pb-1">
							...
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">
								...
								<input name="bank" type="text" id="bank" value="<?php echo $jv->bank?>"  tabindex="9" <? if($jv->tr_no>0) echo 'readonly'?> class="form-control"/>
				...
				<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6">
					<div class="container n-form2">
						<table  class="table1  table-striped table-bordered table-hover table-sm" border="0" cellspacing="0" cellpadding="0">
							...
							$sql2="select a.tr_no, a.cr_amt, a.narration,a.jv_date , a.jv_no,a.checked,a.cv_no from  secondary_journal a where a.tr_from='Payment' and  SUBSTRING(a.tr_no,5,1)='0' and  a.cr_amt>0 and a.group_for=".$_SESSION['user']['group']." group by a.tr_no order by a.id desc limit 5";
