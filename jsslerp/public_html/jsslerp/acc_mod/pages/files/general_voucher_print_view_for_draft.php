...
<?php if($cid == 'golden' || $cid == 'demo-golden'){?>
<style>
	.footer1{
		position: relative;
		width: 100%;
...
<div class="body">
	<div class="header">
		...
		<td class="titel">
				...
				<p class="text" style="font-size:12px; "><?=find_a_field('user_group','address',"1"); ?></p>
				<p class="text" style="font-size:12px; line-height: 0px !important; "><?=find_a_field('user_group','factory_address',"1"); ?></p>
<!--				<p class="text">Cell: <?=$group_data->mobile?>. Email: <?=$group_data->email?> <br> <?=$group_data->vat_reg?></p>-->
	...
	<div class="row">
		...
		<div class="col-md-6 col-sm-6 col-lg-6 right">
			...
			<p class="right-text mt-1 mb-1">CV No : <span> <?=$jv->cv_no;?>  </span></p>
...
<table class="table1">
    ...
    <tbody>
        ...
        JOIN secondary_journal b ON a.ledger_id = b.ledger_id
        WHERE b.jv_no = '$jv_no'
        ORDER BY b.id ASC
        ";
        ...
        <tr>
            ...
            <td><?= $info->ledger_id ?></td>
            <td><?= find_a_field('ledger_group','group_name','group_id='.$info->ledger_group_id); ?></td>
            <td><?= $info->ledger_name ?></td>
            <td><?= find_a_field('cost_center','center_name','id='.$info->cc_code); ?></td>
            <td><?= $info->narration ?></td>

            <td align="right">
                <?= number_format($info->dr_amt,2); $ttd += $info->dr_amt; ?>
            </td>

            <td align="right">
                <?= number_format($info->cr_amt,2); $ttc += $info->cr_amt; ?>
            </td>
        </tr>
        <?php } ?>
...
<p class="p bold">In Words : <?= convertNumberMhafuz($ttc) ?>.</p>
...
<div class="footer footer1"  id="footer">
	...
	<?php /*?>	  <p style="font-weight:600; margin: 0;"> <?=find_a_field('user_activity_management','fname','user_id='.$jv->om_checked);?></p>
		  <p style="font-size:11px; margin: 0;">(<?=find_a_field('user_activity_management','designation','user_id='.$jv->om_checked);?>) <br/> <?=$jv->om_checked_at?></p><?php */?>
		  </td>
