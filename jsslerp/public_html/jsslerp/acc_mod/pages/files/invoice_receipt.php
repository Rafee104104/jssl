...
if(!isset($_GET['v_d'])&&!isset($_GET['v_type']))

	{
		...
		if($voucher_mode != $old_voucher_mode) mysql_query("UPDATE project_info SET voucher_mode=$voucher_mode WHERE 1");			
...

{
	...
	$r_from		= mysql_real_escape_string($_REQUEST['r_from']);

	$c_no		= mysql_real_escape_string($_REQUEST['c_no']);

	$c_date		= mysql_real_escape_string($_REQUEST['c_date']);
