...
$time_now = date('Y-m-d H:i:s');
	...
	$ssql='update secondary_journal set jv_date="'.$voucher_date.'", cc_code="'.$cc_code.'", checked_at="'.$time_now.'", checked_by="'.$_SESSION['user']['id'].'", checked="YES" , 	final_jv_no="'.$jv.'" where jv_no="'.$jv_no.'"';
	mysql_query($ssql);
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#CCCCCC">
  <tr>
    ...
    <td width="180" align="right"><a target="_blank" href="../../warehouse_mod/pages/do/chalan_bill_view.php?v_no=<?=$ch->chalan_no?>">CH-
        <?=$ch->chalan_no?>
    </a></td>
...
<a target="_blank" href="../../warehouse_mod/pages/do/chalan_bill_view.php?v_no=<?=$ch->chalan_no?>"></a></div><? }else{?>
<div align="left">
<form action="" method="post">
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#339900">
  <tr>
    <td width="1"><input name="button" type="button" onclick="hide();window.print();" value="Print" /></td>
    <td width="200" align="right"><a target="_blank" href="../../warehouse_mod/pages/do/chalan_bill_view.php?v_no=<?=$ch->chalan_no?>">CH-
        <?=$ch->chalan_no?>
    </a></td>
...
<a target="_blank" href="../../warehouse_mod/pages/pr/chalan_view2.php?v_no=<?=$ch->pr_no?>"></a></div><? }?>
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#000000" class="tabledesign" style="font-size:12px">
	  ...
	  <?
$sql2="SELECT a.ledger_name,a.ledger_id,sum(cr_amt) as cr_amt,sum(dr_amt) as dr_amt FROM accounts_ledger a, secondary_journal b where a.ledger_id not in (".$config_ledger->sales_cash_discount.",".$config_ledger->sales_trade_offer.",".$config_ledger->sales_ledger."".") and  b.jv_no='$jv_no' and a.ledger_id=b.ledger_id group by a.ledger_id";
...
<?php }?>
	  <?
$sql2="SELECT a.ledger_id,a.ledger_name,sum(cr_amt) as cr_amt,sum(dr_amt) as dr_amt FROM accounts_ledger a, secondary_journal b where a.ledger_id in ('".$config_ledger->sales_cash_discount."','".$config_ledger->sales_trade_offer."','".$config_ledger->sales_ledger."'".") and b.jv_no='$jv_no' and a.ledger_id=b.ledger_id group by a.ledger_id";
$data2=mysql_query($sql2);
while($info=mysql_fetch_object($data2)){		  
	  ?>
      <tr>
