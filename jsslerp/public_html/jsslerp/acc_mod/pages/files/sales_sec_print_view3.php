...

{
 ...
 $ssql='update secondary_journal set jv_date="'.$voucher_date.'",  checked_at="'.$time_now.'", checked_by="'.$_SESSION['user']['id'].'", checked="YES" where jv_no="'.$jv_no.'"';
	...
	sec_journal_journal($jv_no,$jv,'Paid Booking');
...
<a target="_blank" href="../../../sales_mod/pages/dealer/paid_booking_print_view.php?booking_id=<?=$jv->tr_no?>"></a></div><? }else{?>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#339900">
  ...
  <tr>
    ...
    <td width="200" align="right"><a target="_blank" href="../../../sales_mod/pages/dealer/paid_booking_print_view.php?booking_id=<?=$jv->tr_no?>">Booking View-
...
<a target="_blank" href="../../warehouse_mod/pages/pr/chalan_view2.php?v_no=<?=$ch->pr_no?>"></a></div><? }?>
  ...
  $sql2="SELECT a.ledger_id,a.ledger_name,cr_amt,dr_amt,b.narration,b.tr_no FROM accounts_ledger a, secondary_journal b where b.jv_no='$jv_no' and a.ledger_id=b.ledger_id and b.tr_from='Paid Booking'";

$data2=mysql_query($sql2);

while($info=mysql_fetch_object($data2)){		  
$all_booking=find_all_field('paid_booking','','booking_id='.$info->tr_no);
	  ?>
      ...
      <tr>
        ...
        <td align="left">Booking No:<?=$all_booking->booking_number_eng?>,  Booking Rate: <?=$all_booking->booking_rate?>, Bag Qty: <?=$all_booking->bag_quantity?></td>
