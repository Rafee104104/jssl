...
$query_grp=mysql_query($sql_grp);
while($data_grp=mysql_fetch_row($query_grp)){
$p++;
$pi=0;
$total_amt=0;
$total_amtl=0;
...
?>
  <tr <?php  if($pi%2==0) echo "bgcolor=\"#E8FFFF\""; else echo "bgcolor=\"#F0F0FF\"";?>>
	...
	$amt=mysql_fetch_row(mysql_query("select SUM(dr_amt)-SUM(cr_amt) from journal where ledger_id ='$data[1]' and jv_date between '$fdate' and '$tdate' and 1"));
$total_amt=$total_amt+$amt[0];
	 if($amt[0]==0) echo "0.00"; echo $amt[0];?></td>
    <td width="18%" align="right"><?php
	$amtl=mysql_fetch_row(mysql_query("select SUM(dr_amt)-SUM(cr_amt) from fiscal_journal where ledger_id ='$data[1]' and jv_date between '$lastfdate' and '$lasttdate' and 1"));
