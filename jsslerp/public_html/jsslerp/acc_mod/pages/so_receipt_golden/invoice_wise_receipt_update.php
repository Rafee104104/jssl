...
$of_bank=$_POST['of_bank'];
   ...
   $so_invoice = 'INSERT INTO receipt_from_customer (receipt_no, receipt_date, do_no, chalan_no, dealer_code, ledger_id, dr_ledger_id, receipt_amt, status, entry_at, entry_by, chalan_date, sales_jv_no,payment_method,cheque_no,cheque_date,of_bank)
  
  VALUES("'.$receipt_no.'", "'.$receipt_date.'", "'.$data->tr_id.'", "'.$tr_no.'", "'.$data->dealer_code.'", "'.$account_code.'", "'.$dr_ledger_id.'", "'.$receipt_amt.'", "COMPLETE", "'.$entry_at.'", "'.$entry_by.'", "'.$data->jv_date.'", "'.$data->jv_no.'", "'.$payment_method.'","'.$cheque_no.'","'.$cheque_date.'","'.$of_bank.'")';
		...
		<? if(isset($_POST['submit'])){ ?>
         ...
         <input name="received_from" type="text" id="received_from"  readonly=""  value="<?=$dealer_data->dealer_name_e;?>"  required tabindex="105" />
				 ...
				 value="<? if ($dealer_closing>0) { echo  number_format($closing_balance,2). ""; } else { echo number_format($closing_balance*(-1),2). ""; }?>"/>
							...
							<div class="form-group row m-0 pb-1">
								<label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Party Bank Name</label>
								<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
