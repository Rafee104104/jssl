...
$con .='and a.warehouse_name like "%'.$_POST['warehouse_name'].'%" ';
 ...
 $td='select a.'.$unique.',  a.'.$shown.', a.acc_sub_class, a.acc_class,a.notes  from '.$table.' a where 1  '.$con.' order by a.id  ';
...
while($rp=mysql_fetch_row($report)){$i++; if($i%2==0)$cls=' class="alt"'; else $cls='';?>
            <tr<?=$cls?> onclick="DoNav('<?php echo $rp[0];?>');">
              <td><?=$rp[0];?></td>
              <td align="left"><?=find_a_field('accounts_notes','name','id='.$rp[4]);;?></td>
...
document.getElementById('acc_class').value);">
        ...
        <div class="form-group row m-0 pl-3 pr-3 p-1">
          ...
          <div class="col-sm-8 p-0"> <span id="sub_class">
		...
		<div class="form-group row m-0 pl-3 pr-3 p-1">
          <label for="group_name" class="req-input col-sm-4 pl-0 pr-0 col-form-label">NOTES:</label>
          <div class="col-sm-8 p-0"> 
            <select name="notes" required id="notes"  tabindex="2">
              <option></option>
              <? foreign_relation('accounts_notes','id','CONCAT(id, ": ", name)',$notes,'1');?>
            </select>
           </div>
        </div>
        <div class="form-group row m-0 pl-3 pr-3 p-1">
