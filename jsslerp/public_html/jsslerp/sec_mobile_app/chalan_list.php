<?php
session_start();
include 'config/db.php';
include 'config/function.php';
include 'config/access.php';
$user_id	=$_SESSION['user_id'];

$page="home";

include "inc/header.php";

?>
<!-- main page content -->
<div class="main-container container">
<h3>Chalan List</h3>
<form action="" method="post" name="codz" id="codz">

<div class="row">
        
            <div class="col-3"><label>Date Form</label></div>  
            <div class="col-6"><input type="date" class="form-control" name="fdate" id="fdate" 
			value="<?=$_POST['fdate']?$_POST['fdate']:date('Y-m-01')?>" /></div>
</div>           
       
	   
<div class="row">	    
            <div class="col-3"><label>Date To</label></div> 
            <div class="col-6"><input type="date" class="form-control" name="tdate" id="tdate" 
			value="<?=$_POST['tdate']?$_POST['tdate']:date('Y-m-d')?>" />
        </div> 
</div> 	

<div class="row mt-2 mb-10 mb-2">
	<div class="col-4"><label class="control-label" for="market_id">Shop<span class="required"></span></label></div>
	<div class="col-7"><select type="text" name="dealer_code" autocomplete="off" class="form-control">
	        <option><?=$_POST['dealer_code']?></option>
	        <? 
			optionlist("select dealer_code,shop_name from ss_shop where region_id='".$_SESSION['region_id']."' and zone_id='".$_SESSION['zone_id']."' and area_id='".$_SESSION['area_id']."' order by 
			region_id,zone_id,area_id,shop_name");
			?>
	    </select></div>
</div>


<div class="row">	
        <div class="col-12 form-group position-relative">
            <button type="submit" name="submitit" id="submitit" class="btn btn-success position-absolute top-50 end-0 translate-middle">View</button>
        </div> 
    </div>
  </form>
  
<p><br>  

            
<!-- User list items  -->
<div class="row">

                
<? 
if(isset($_POST['submitit'])){

if($_POST['fdate']!=''&&$_POST['tdate']!='')
$con .= 'and c.chalan_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';

if($_POST['dealer_code']){ $shop_con=' and s.dealer_code="'.$_POST['dealer_code'].'"';}

$sql = "select c.do_no,c.chalan_date,c.chalan_no,s.shop_name, s.*,sum(c.total_amt) as total_amt 
from ss_shop s,ss_do_chalan c 
where s.dealer_code=c.dealer_code
and c.entry_by='".$emp_code."'
".$con.$shop_con."
group by c.chalan_no order by c.chalan_no";

$query=mysqli_query($conn, $sql);
while($data=mysqli_fetch_object($query)){
?>                            
<div class="col-12">
    <div class="card shadow-sm mb-2">        
            <ul class="list-group list-group-flush bg-none">
            <li class="list-group-item border-0">
                <a href="chalan_view.php?v=<?=$data->chalan_no?>">
                <div class="row">
                    <div class="col-auto">
                        <div class="card">
                            <div class="card-body p-0">
                                <figure class="avatar avatar-50 rounded-15">
                                    <img src="assets/img/user1.jpg" alt="">
                                </figure>
                            </div>
                        </div>
                    </div>
                    <div class="col px-0">
                        <p class="mb-0">Chalan: <?=$data->chalan_no?>, Order No: <?=$data->do_no?>  <?=$data->shop_name?>
						<br><small class="text-secondary"><?=$data->chalan_date?></small></p>
                    </div>
                    <div class="col-auto text-end">
                        <p class="text-secondary text-muted size-10 mb-0">Chalan Amt</p>
                        <p class="text-info">
                            <strong><?=$data->total_amt;?></strong>
                        </p>
                    </div>
                </div></a>
            </li>
    
        </ul>
         
    </div>
</div>
           <? } ?> 
</div>  

<? } ?>
            
            


        </div>
        <!-- main page content ends -->


    </main>
    <!-- Page ends-->

<?php include "inc/footer.php"; ?>