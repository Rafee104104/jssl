...
if (isset($_POST['acSave'])) {
  ...
  if ($_SESSION['cid'] != '') {
    ...
    mysql_query('delete from module_access where company_id="' . $_SESSION['cid'] . '"');

    foreach ($_POST['chkSelect'] as $value) {

      $_POST['company_id'] = $_SESSION['cid'];
      $_POST['module_id'] = $value;
      $_POST['status'] = "ON";
      $crud = new crud('module_access');
      $crud->insert();
    }
    //////////////////////////////////*********module array************ *///////////////////////////////////////////
...
<div class="container row">
  <div class="col-md-6">
    ...
    <h3>Access for: <b><u><?= find_a_field('company_info', 'company_name', 'id="' . $_SESSION['cid'] . '"');  ?></u></b></h3>
    <form action="" method="post">
      ...
      <table class="table table-bordered">
        ...
        <tbody>
          <?php
          $mSql = 'select * from module_access where company_id=' . $_SESSION['cid'] . '';
          $mQuery = mysql_query($mSql);
          while ($mRow = mysql_fetch_object($mQuery)) {

            $mData[$mRow->company_id][$mRow->module_id] = 1;
          }
          ...
          ?>
            <tr>
              ...
              <td> <input type="checkbox" class="child" name="chkSelect[]" value="<?= $data->id; ?>" <?php if ($mData[$_SESSION['cid']][$data->id] == 1) {
