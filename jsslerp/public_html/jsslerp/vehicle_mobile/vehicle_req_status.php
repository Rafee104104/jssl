...
<!-- User list items  -->
<div class="row">
<div class="row text-center mb-2"><h4>Vehicle Request Status<br /><?=$_SESSION['msg'];unset($_SESSION['msg']);?></h4></div>    
...
$sql = "select m.* from vehicle_requisition m where 1 ".$con." order by m.req_no asc";
$query=mysqli_query($conn, $sql);
while($data=mysqli_fetch_object($query)){
$vehicle_name = find_a_field('vehicle_info','vehicle_name','vehicle_id="'.$data->vehicle_assign.'"');
?>                            
<div class="col-12">
        ...
        <!--<a href="do_checking.php?expense_no=<?=$data->expense_no?>">   -->
            <li class="list-group-item border-0">
                <div class="row">
                    ...
                    <div class="col px-0">
                        <p>Request No: <?=$data->req_no?><br><small class="text-secondary"><?=find_a_field('user_activity_management','fname','user_id="'.$data->entry_by.'"')?><br />Request Date: <?=$data->req_date?><br />Total Person: <?=$data->person?><br />Status: <?=$data->status?><br />Vehicle : <?=$vehicle_name?></small></p>
                    </div>
