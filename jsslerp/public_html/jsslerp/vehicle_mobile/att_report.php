...
<? 
if(isset($_POST['show'])){
	if($_POST['fdate'] !='' && $_POST['tdate'] !='') $con = ' and m.access_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'" ' ;
}
$sql = "select m.* from ss_location_log m where 1 and user_id='".$_SESSION['user_id']."' ".$con." group by m.access_date order by m.id asc";
        ...
        <!--<a href="do_checking.php?expense_no=<?=$data->expense_no?>">   -->
            <li class="list-group-item border-0">
                <div class="row">
                    ...
                    <div class="col px-0">
                        <p>Date: <?=date('Y-M-d',strtotime($data->access_date))?><br>
						<?
						$sql2 = "select m.* from ss_location_log m where 1 and user_id='".$_SESSION['user_id']."' and access_date='".$data->access_date."' ".$con." order by m.id asc";
$query2=mysqli_query($conn, $sql2);
while($data2=mysqli_fetch_object($query2)){
						?>
						<small class="text-secondary"><?=$data2->attendance_type?> : <?=date('H:i:s',strtotime($data2->access_time))?></small><br />
						<? } ?>
						</p>
                    </div>
