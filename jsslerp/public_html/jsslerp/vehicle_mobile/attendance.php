...
if(isset($_REQUEST['in_time'])){
    ...
    if($_POST['user_id']>0){
    $insert_id=@insert('ss_location_log');
	$img = $_POST['image'];
...
<? if($check_intime!=''){ }else{ ?>
                    <div class="col-6 d-grid"><button type="submit" onClick="take_snapshot()" name="in_time" class="btn btn-default btn-lg btn-rounded shadow-sm">IN PUNCH</button></div>
<? } ?>

<? if($check_outtime=='' && $check_intime!=''){ ?>
                    <div class="col-6 d-grid"><button type="submit" onClick="take_snapshot()" name="out_time" class="btn btn-warning btn-lg btn-rounded shadow-sm">OUT PUNCH</button></div>
<? } ?>                  
                </div>
            </div>
