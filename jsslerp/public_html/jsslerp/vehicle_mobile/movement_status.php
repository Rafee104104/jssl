...
<!-- User list items  -->
<div class="row">
<div class="row text-center mb-2"><h4>Log Book</h4></div>    
...
$sql = "select m.*,sum(m.net_km) as net_km,u.fname from vehicle_log_book m,user_activity_management u where m.vehicle_user=u.user_id and 1 and m.status='CHECKED' ".$con." group by m.log_no order by m.log_no asc";
        ...
        <!--<a href="vehicle_movement.php?log_no=<?=$data->log_no?>">  --> 
            <li class="list-group-item border-0">
                <div class="row">
                    ...
                    <div class="col px-0">
                        <p>Log Date: <?=date('d-M-y',strtotime($data->log_date))?><br>User: <?=$data->fname?><br><?
						$sql2 = "select m.* from vehicle_log_book m where 1 and log_no='".$data->log_no."'";
$query2=mysqli_query($conn, $sql2);
while($data2=mysqli_fetch_object($query2)){
						?>
						<small class="text-secondary">Journey:<?=$data2->journey?> (<?=number_format($data2->start_meter_reading,2)?>-<?=number_format($data2->end_meter_reading,2)?>)</small><br />
