...
if($_GET['pal']==2) { unset($$unique); unset($_SESSION['req_no']); $type=1;}
...
if(isset($_POST['new'])){
		...
		$_POST['entry_at']	=date('Y-m-d H:i:s');
			...
			?>
			<script>window.location.href='vehicle_req_status.php';</script>
		...
		$vehicle = explode("#",$_POST['vehicle_assign']);
		$_POST['vehicle_assign'] = $vehicle[1];
		$_POST['approved_by']	=$_SESSION['user_id'];
		$_POST['approved_at']	=date('Y-m-d H:i:s');
		$_POST['status']	='VEHICLE ASSIGNED';
		$crud->update($unique);
		$type=1;
		unset($$unique);
		unset($_SESSION['req_no']);
		$_SESSION['msg'] = '<span style="color:green;">Vehicle Assigned Successfully</span>';
			?>
			<script>window.location.href='vehicle_assign.php';</script>
...
if($$unique>0) $btn_name='Update Request'; else $btn_name='Confirm Request';
...
<div class="form-group row">
            ...
            <div class="col-8 mb-1"><div class="col-sm-3"><? $field='req_no';?><input  class="form-control border border-info" name="<?=$field?>" type="text" id="<?=$field?>" value="<?=$$field?>" disabled="disabled"/></div></div>

            <div class="col-4"><label for="req_date" class="col-form-label">Need By</label></div> 
            <div class="col-8"><? $field='req_date'; if($req_date=='') $req_date =date('Y-m-d'); ?><input  class="form-control border border-info" name="<?=$field?>" type="date" id="<?=$field?>" value="<?=$$field?>" required/></div>
</div>    
...
<div class="form-group row mb-2">
			...
			<div class="col-2"><label for="shop" class="col-sm-2 col-form-label size-12">Vehicle:</label></div> 
            <div class="col-10">
				<input class="form-control border border-info" name="vehicle_assign" list="vehicle" id="vehicle_assign" value="<?=$vehicle_assign?>" tabindex="1" autocomplete="off" required/>
				 <datalist id="vehicle">
    	<?php optionlist('select concat(vehicle_name,"#",vehicle_id) from vehicle_info where 1 order by vehicle_name asc');?>
    </datalist>
            </div>
