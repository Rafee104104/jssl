...
//mysql_query($sql);
 ...
 $sqlrb='select * from master_requisition_master where req_no='.$req_no;
     ...
     $rsrb = mysql_query($sqlrb);

	 $rowrb=mysql_fetch_object($rsrb);
...
//
//
//mysql_query("UPDATE `production_issue_detail` SET `status` = 'RECEIVE',total_unit='".$row->qty."' WHERE `req_id` = ".$row->id);
//
//
//
//mysql_query("UPDATE `master_requisition_details` SET `status` = 'RECEIVE',qty='".$row->qty."',journal=1 WHERE `id` = ".$row->id);
//
//}
//
//}
...
//mysql_query($sqUp);
//
//mysql_query("UPDATE `production_issue_master` SET `status` = 'RECEIVED'  WHERE `pi_no` = ".$pi_no);
