...
$con2 .= 'and a.vendor_id="'.$_POST['vendor_id'].'"';

if($_POST['item_id']>0)
$con3 .= 'and r.item_id="'.$_POST['item_id'].'"';

 $res='select  a.po_no,r.pr_no,r.rec_date, a.po_date,i.item_name,r.rate,r.qty,r.amount, v.vendor_name,u.fname as entry_by
from 
purchase_master a,warehouse b,purchase_receive r, vendor v,user_activity_management u,item_info i where i.item_id=r.item_id and u.user_id=a.entry_by and a.po_no=r.po_no and a.warehouse_id=b.warehouse_id and  a.vendor_id=v.vendor_id  '.$con.$con2.$con3.' order by a.po_no desc';
...
?>
	...
	<tr>
	  ...
	  <td valign="top"><?=$data->pr_no;?></td>
	  <td valign="top"><?=$data->rec_date;?></td>
	  <td valign="top"><?=$data->vendor_name;?></td>
	  <td><?=$data->item_name;?></td>
	  <td><?=$data->rate;?></td>
	  <td><?=$data->qty; $tot_qty+=$data->qty;?></td>
	  <td><?=$data->amount; $tot_amt+=$data->amount;?></td>
...
<? }?>
	<tr>
	  ...
	  <td><div align="left">
	    <?=number_format(($tot_qty),2);?></div></td>
...
$qquery = mysql_query($ssql); while($sec = mysql_fetch_object($qquery)){
...
$company_con = ' and m.warehouse_id in ('.$warehouse_list.')';
 ...
 $sql3='SELECT  m.req_no,m.req_date,o.req_no as req_no,o.item_id,o.qty as req_qty,m.req_for,m.req_note,m.status,i.unit_name,m.warehouse_id,
...
$po_qty = find_a_field('purchase_invoice','sum(qty)','req_no="'.$data->req_no.'" and item_id="'.$data->item_id.'"');

$value = '';
$ssql = 'select id from purchase_invoice where  item_id="'.$data->item_id.'" and req_no='.$data->req_no;
