...
$vendor=find_all_field('vendor','','vendor_id="'.$_POST['vendor_id'].'"');
?>
                ...
                <div class="col-sm-6 col-md-6 col-lg-6 col-xl-6">
                    <div class="container n-form2">
                        ...
                        <div class="form-group row m-0 pb-1">
                            ...
                            <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
                             <? if($vendor_id<1) { ?>
							 <input type="text" list="vendor_id_draft_list" name="vendor_id_draft_a" id="vendor_id_draft_a" required  autocomplete="off">

<datalist id="vendor_id_draft_list">
<?php 
 $v_sql='select * from vendor where group_for="'.$_SESSION['user']['group'].'"';
...
?>
  ...
  <?php } ?>
				...
				<!--		<select name="vendor_id_draft" id="vendor_id_draft" required   >
						...
						<? foreign_relation($table,$get_field,$show_field,$$field,' group_for="'.$_SESSION['user']['group'].'"');?>
...
<? if($_SESSION[$unique]>0){?>
    ...
    <form action="?<?=$unique?>=<?=$$unique?>" method="post" name="cloud" id="cloud">
		...
		if(($vendor->ledger_id==0)&&($group_for==2||$group_for==15)){ ?>
  ...
  ?>
                ...
                <tr>
				 ...
				 <input type="hidden" name="po_qty<?=$i?>" id="po_qty<?=$i?>" value="<?=$already_po_qty?>" /></td>
					...
					<td><input type="text" onkeyup="count_1(<?=$i?>);total_cal()" onblur="update_item(<?=$data->id?>)" name="qty<?=$data->id?>" id="qty<?=$i?>" value="<?=$this_po->qty?>" /></td>
					
					<td><?=$data->Unit ?></td>
					
					<td><input type="hidden" required name="details_id<?=$data->id?>"  value="<?  echo $data->id; ?>" />
				
				<input type="text" onkeyup="count_1(<?=$i?>);total_cal()" onblur="update_item(<?=$data->id?>)"  name="unit_price<?=$data->id?>" id="unit_price<?=$i?>" value="<?=$this_po->rate?>" /></td>
					
					<td><input type="text" readonly  name="amount<?=$data->id?>" id="amount<?=$i?>" value="<?=$this_po->amount?>" /> 
