...
<div class="main-content">
	...
	<table class="table1">
		...
		<?php
			...
			$supplementary_duty_amt=(($total*$master->supplementary_duty)/100);
			...
			$item_code=find_a_field('item_info','item_id','item_id='.$info->item_id);
		...
		<? }?>
			  ...
			  <tr  align="right">
				<td colspan="6" style=" text-align:right;"><strong>Discount (<?=number_format($master->cash_discount,0)?>%):</strong></td>
				<td align="right" style="text-align:right;"><strong>
				  <?  echo number_format($dist_total=(($total*$master->cash_discount)/100),2);?>
				</strong></td>
			  </tr>
			  <? }?>
			  ...
			  <? if($master->ait>0) {?>
			  <tr  align="right">
				<td colspan="6" style=" text-align:right;"><strong>AIT (<?=number_format($master->ait,0)?>%):</strong></td>
				<td align="right" style="text-align:right;"><strong>
			  ...
			  <tr >
				...
				<td align="right" style="text-align:right;"><strong><? echo number_format($payable_amount=(($total+$tax_total+$ait_total)-$dist_total),2);?></strong></td>
			  </tr>
...
<p class="p-text"> The Vendor has agreed to provide the above goods/services to <?=$group_data->group_name?> agreed to accept the merchandise from the Vendor in the quantities, at the prices, at the time and subject to the terms, provisions and conditions stated below: <p>
	...
	<table class="terms-table">
			...
			<tr style="letter-spacing:.3px; line-height: 16px;">
			  <td class="text-left w-95 pl-5 pt-3"><?=$tc_data->terms_condition;?></td>
			  </tr> 
			 <? }?>
	</table>
