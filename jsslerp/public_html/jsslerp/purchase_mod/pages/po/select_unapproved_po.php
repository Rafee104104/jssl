                    ...
                    <tbody class="tbody1">
						<? 
							...
							$con .= 'and p.po_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
							 ...
							 $res='select p.po_no,p.po_no,p.po_date,v.vendor_name,w.warehouse_name,u.fname as created_by,p.entry_at,p.status from purchase_master p, user_activity_management u, vendor v, warehouse w where p.warehouse_id=w.warehouse_id and v.vendor_id=p.vendor_id and u.user_id=p.entry_by and p.status="approval2" '.$con. ' order by p.po_no desc';
