...
{
        $del = mysql_query('delete from purchase_invoice where po_no="'.$_POST['po_no'].'"');
		...
		while($data=mysql_fetch_object($query)){
		 ...
		 $vat_with_price = $_POST['vat_with_price'.$data->id];
		 $grand_amount = $_POST['grand_amount'.$data->id];
		 if($poQty>0)
		 {
		 $po_details_insert = 'insert into purchase_invoice(`po_no`,`po_date`,`req_no`,`req_id`,`vendor_id`,`item_id`,`warehouse_id`,`rate`,`qty`,`amount`,`entry_by`,`entry_at`,vat_with_price,grand_amount) value("'.$po_no.'","'.$po_date.'","'.$data->req_no.'","'.$data->id.'","'.$vendor_id.'","'.$data->item_code.'","'.$data->warehouse_id.'","'.$poRate.'","'.$poQty.'","'.$poAmount.'","'.$_SESSION['user']['id'].'","'.date('Y-m-d H:s:i').'","'.$vat_with_price.'","'.$grand_amount.'")';
		 mysql_query($po_details_insert);
		 }
		 }
...
<div class="form-container_large">
    <form form action="" method="post" name="codz" enctype="multipart/form-data" id="codz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
<!--        top form start hear-->
...
$v_query=mysql_query($v_sql);
while($row=mysql_fetch_object($v_query)){
?>
  ...
  ?>
                ...
                <tr>
				 ...
				 <input type="hidden" name="po_qty<?=$i?>" id="po_qty<?=$i?>" value="<?=$already_po_qty?>" /></td>
					...
					<td><input type="text" onkeyup="count_1(<?=$i?>);total_cal();count2(<?=$data->id?>);" onblur="update_item(<?=$data->id?>)" name="qty<?=$data->id?>" id="qty<?=$i?>" value="<?=$this_po->qty?>" required/></td>
					
					<td><?=$data->Unit ?></td>
					
					<td><input type="hidden" required name="details_id<?=$data->id?>"  value="<?  echo $data->id; ?>" />
				
				<input type="text" onkeyup="count_1(<?=$i?>);total_cal();count2(<?=$i?>);" onblur="update_item(<?=$data->id?>)" required name="unit_price<?=$data->id?>" id="unit_price<?=$i?>" value="<?=$this_po->rate?>" /></td>
					
					<input name="vat<?=$data->id?>" type="hidden" class="input3" id="vat<?=$i?>" value=""  />
					<input name="tax<?=$data->id?>" type="hidden" class="input3" id="tax<?=$i?>" value=""  />
					<input name="grand_amount<?=$data->id?>" type="hidden" class="input3" id="grand_amount<?=$i?>" value=""  />
					<input name="vat_with_price<?=$data->id?>" type="hidden" class="input3" id="vat_with_price<?=$i?>" value=""  />
					<input name="vat_percent<?=$data->id?>" type="hidden" class="input3" id="vat_percent<?=$i?>" value="<?=find_a_field('purchase_master','vat','po_no='.$$unique);?>"  />
					<input name="tax_percent<?=$data->id?>" type="hidden" class="input3" id="tax_percent<?=$i?>" value="<?=find_a_field('purchase_master','tax','po_no='.$$unique);?>"  />
...
function count2(id) {
    var unitPrice = parseFloat(document.getElementById('unit_price' + id)?.value) || 0;
    var qty = parseFloat(document.getElementById('qty' + id)?.value) || 1; 
    var vatPercent = parseFloat(document.getElementById('vat_percent' + id)?.value) || 0;
    var taxPercent = parseFloat(document.getElementById('tax_percent' + id)?.value) || 0;

    var vatPrice = (unitPrice * vatPercent) / 100;
    var taxPrice = (unitPrice * taxPercent) / 100;
    var vatWithPrice = unitPrice + vatPrice + taxPrice;
    var grandTotal = vatWithPrice * qty;

    console.log({ unitPrice, qty, vatPercent, taxPercent, vatPrice, taxPrice, vatWithPrice, grandTotal });

    if (document.getElementById('vat' + id)) document.getElementById('vat' + id).value = vatPrice.toFixed(2);
    if (document.getElementById('tax' + id)) document.getElementById('tax' + id).value = taxPrice.toFixed(2);
    if (document.getElementById('vat_with_price' + id)) document.getElementById('vat_with_price' + id).value = vatWithPrice.toFixed(2);
    if (document.getElementById('grand_amount' + id)) document.getElementById('grand_amount' + id).value = grandTotal.toFixed(2);
}
