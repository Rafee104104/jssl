...
if (isset($_POST['submit']) && isset($_POST['report']) && $_POST['report'] > 0) {
	...
	if ($_POST['JOB_LOCATION'] != '')	$con .= ' and a.JOB_LOCATION = "' . $_POST['JOB_LOCATION'] . '"';
	if ($_POST['PBI_GROUP'] != '')
...
AND a.pay_code = e.pay_code" . $proj_con . $date_con . $flat_con . $party_con . " order by a.inst_date";
		...
		case 1003:
			...
			if ($_POST['dealer_name_e'] != '') $con .= ' and a.dealer_name_e like "%' . $_POST['dealer_name_e'] . '%"';
			if ($_POST['dealer_code'] != '')
			...
			if ($_POST['region_code'] != '') $con .= ' and a.area_code in (select p.AREA_CODE from area p,zon z where p.ZONE_ID=z.ZONE_CODE and z.REGION_ID="' . $_POST['zone_code'] . '") ';
			elseif ($_POST['zone_code'] != '')
...
personnel_basic_info a where 1 " . $con . " order by a.PBI_DOJ asc";
			...
			if ($_POST['report'] == 404) { // Task List
			
			

				if ($_POST['start_date'] != '') {
					$conV = ' and t.task_start= "' . $_POST['start_date'] . '"';
				}
				if ($_POST['PBI_ID'] != '') {
					$conV .= ' and t.assign_person="' . $_POST['PBI_ID'] . '"';
				}
				if ($_POST['status'] != '') {
				
					$conV .= ' and t.status="' . $_POST['status'] . '"';
				}
				
				if ($_POST['project'] != '') {
				
					$conV .= ' and t.project="' . $_POST['project'] . '"';
				}


				 $sql = 'select i.PBI_NAME as assign_name,(select name from crm_project_org where id=t.project) as project,t.task_name,t.task_des,concat(t.task_start,"(",t.task_start_time,")") as start_time,concat(t.task_end,"(",t.task_end_time,")") as end_time,(select priority from mis_task_priority where id=t.task_priority) as task_priority,t.status from task_manage t,personnel_basic_info i where t.assign_person=i.PBI_ID and t.assign_person!=0 ' . $conV . '';
			}
			if ($_POST['report'] == 505) { // Task List

				if ($_POST['start_date'] != '' && $_POST['end_date'] != '') {
					$conV = ' and t.task_start between "' . $_POST['start_date'] . '" and "' . $_POST['end_date'] . '"';
				}
				if ($_POST['PBI_ID'] != '') {
					$conV .= ' and t.assign_person="' . $_POST['PBI_ID'] . '"';
				}
				...
				$sql = 'select i.PBI_NAME as name,t.company,t.activity_name,t.activity_des,t.task_start as date,t.task_start_time,t.task_end_time,t.status from activity_manage t,personnel_basic_info i where t.assign_person=i.PBI_ID and t.assign_person!=0 ' . $conV . '';
			}
			
			if ($_POST['report'] == 606) { // Issue List

			if ($_POST['project'] != '') {
				
					$conV .= ' and i.project="' . $_POST['project'] . '"';
				}


				$sql = 'select (select name from crm_project_org where id=i.project) as project, i.issue_name as issue_name,i.issue_des as description,i.issue_date as issue_date,i.deadline_date as deadline_date,i.status as status from task_issue i where 1 '.$conV.'';
			}
			...
			?>
				...
				<table width="60%" cellspacing="0" cellpadding="2" style="margin:auto;" align="center">
 ...
 $conn .= ' and p.request_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
						...
						?>
							...
							<tr>
								...
								<td colspan="4" style="border:0px;" align="center"><span style="font-weight:bold; border:0px; border-bottom:0px; font-size:15px;">Task Name: <?= $data->task ?>-<?= $s++; ?></span></td>

							</tr>

							<tr>

								<td style="border:1px solid #000;" bgcolor="#CCCCCC">Project Name</td>

								<td style="border:1px solid #000;"><?= find_a_field('project', 'PROJECT_NAME', 'project_id="' . $data->project_id . '"') ?></td>

								<td style="border:1px solid #000;" bgcolor="#CCCCCC">Given Date</td>

								<td style="border:1px solid #000;"><?= $data->task_date ?></td>
							...
							<tr>
								...
								<td style="border:1px solid #000;" bgcolor="#CCCCCC">Task</td>

								<td style="border:1px solid #000;" bgcolor="#CCCCCC">Request By</td>

								<td style="border:1px solid #000;" bgcolor="#CCCCCC">Given Date</td>

								<td style="border:1px solid #000;" bgcolor="#CCCCCC">End Date</td>

								<td style="border:1px solid #000;" bgcolor="#CCCCCC">Status</td>

							</tr>
							<?

							$res2 = 'select a.*,p.PROJECT_NAME from user_productivity a, project p where p.PROJECT_ID=a.project_id and task_id="' . $data->id . '"';

							$queryd2 = mysql_query($res2);
							...
							<tr>
								...
								<td style="border:1px solid #000;" bgcolor="#CCCCCC">Commented By</td>

								<td style="border:1px solid #000;" bgcolor="#CCCCCC">Commented At</td>

								<td style="border:1px solid #000;" bgcolor="#CCCCCC">Comment</td>
							...
							$res3 = 'select c.* from comment_list c where  c.m_id="' . $data->id . '" and tr_from="task"';

							$queryd3 = mysql_query($res3);

							while ($data3 = mysql_fetch_object($queryd3)) {

							?>

								<tr>

									<td><?= find_a_field('user_activity_management', 'fname', 'user_id="' . $data3->entry_by . '"') ?></td>



									<td><?= $data3->entry_at ?></td>
...
personnel_basic_info a, salary_info s where a.PBI_ID=s.PBI_ID  " . $con . " order by a.PBI_DOJ asc, (s.consolidated_salary+s.basic_salary) desc";
			...
			?><table width="100%" cellspacing="0" cellpadding="2" border="0">
					<thead>
						<tr>
							<td style="border:0px;" colspan="13" align="center"><?= $str ?></td>
						</tr>
						<tr>
							<th rowspan="3">
							...
							<th rowspan="3">
								<div align="center">Desgnation</div>
							</th>
		...
		order by b.bonus_amt desc";
						...
						<tr>
							...
							<td align="right"><strong><?= $tot_basic ?></strong></td>
							<td>&nbsp;</td>
			...
			?><table width="100%" cellspacing="0" cellpadding="2" border="0">
					<thead>
						...
						<tr>
							...
							<th colspan="3">
								<div align="center">Extra Allowance </div>
							</th>
							<th rowspan="3">
								<div align="center">OT. Hrs.</div>
							</th>
							<th rowspan="3">
								<div align="center">OT. Amt. </div>
							</th>
							...
							<th rowspan="3" align="center">
								<div align="center">Net Salary Payment </div>
							</th>
							<th rowspan="3">
						...
						<tr>
							<th colspan="9">
								<div align="center">No of Leave's </div>
							</th>
							<th rowspan="2">
							...
							<th rowspan="2">
								<div align="center">Loan /Advance</div>
							</th>
							<th rowspan="2">
					...
					<tbody>
						...
						if ($found == 0) {
							...
							$sqld = 'select t.*, a.PBI_ID, a.PBI_NAME, a.PBI_DOJ, d.DESG_SHORT_NAME as Designation from salary_attendence t,designation d, personnel_basic_info a


 where t.designation = d.DESG_ID and t.pay>0 and  t.mon=' . $_POST['mon'] . ' and t.year=' . $_POST['year'] . ' and t.PBI_ID=a.PBI_ID ' . $salaryCon . ' order by (t.total_payable) desc';
						} else {
							...
							$sqld = 'select t.*, a.PBI_ID, a.PBI_NAME, a.PBI_DOJ, d.DESG_SHORT_NAME as Designation from salary_attendence_lock t,designation d, personnel_basic_info a
 ...
 where t.designation = d.DESG_ID and t.pay>0 and  t.mon=' . $_POST['mon'] . ' and t.year=' . $_POST['year'] . ' and t.PBI_ID=a.PBI_ID ' . $salaryConn . ' order by (t.total_payable) desc';
						...
						?>
							...
							<tr>
								...
								<td align="right"><?= ($data->special_allowance > 0) ? $data->special_allowance : '';
													$totspecial += $data->special_allowance ?></td>
								<td align="right"><?= ($data->food_allowance > 0) ? $data->food_allowance : '';
													$totFood += $data->food_allowance ?></td>
								<td align="right"><?= ($data->transport_allowance > 0) ? $data->transport_allowance : '';
													$totTransport += $data->transport_allowance ?></td>


								<td align="right"><?= ($data->other_allowance > 0) ? $data->other_allowance : '';
													$totOther += $data->other_allowance ?></td>


								<td align="right"><?= ($data->over_time_hour > 0) ? $data->over_time_hour : '';
													$totOverTimeHr += $data->over_time_hour ?></td>
								<td align="right"><?= ($data->over_time_amount > 0) ? $data->over_time_amount : '';
													$totOverTimeAmt += $data->over_time_amount ?></td>
								<td align="right"><?= ($data->mobile_deduction > 0) ? $data->mobile_deduction : '';
													$totMobileDeduct += $data->mobile_deduction ?></td>
								<td align="right"><?= ($data->income_tax > 0) ? $data->income_tax : '';
													$totIincomeTax += $data->income_tax ?></td>
								<td align="right"><?= ($data->food_deduction > 0) ? $data->food_deduction : '';
													$totFoodDeduct += $data->food_deduction ?></td>
								<td align="right"><?= ($data->advance_install > 0) ? $data->advance_install : '';
													$totAdvInstall += $data->advance_install ?></td>
						...
						<?
							...
							<strong>
								<td align="right"><?= ($totTransport > 0) ? $totTransport : ''; ?></td>
							</strong>
					...
					<thead>
						...
						<tr>
							<td style="border:0px;" colspan="28">
								<div align="center" style="font-size:20px;">Leave Report of <? $test = new DateTime('01-' . $_POST['mon'] . '-' . $_POST['year'] . ' ');
						...
						<tr>
							...
							<th colspan="2">
								<center>Date Intervel</center>
							</th>


							<th rowspan="2">
								<center>Total Days</center>
							</th>
							...
							<th rowspan="2">
								<center>Reporting Authority<center>
							</th>


							<th rowspan="2">
								<center>HR Approval<center>
							</th>
...
$total_days = $leaveData->total_days;
			...
			?>
				...
				<table width="100%" cellspacing="0" cellpadding="2" border="0">
					...
					<thead>
						...
						<tr>
							<td style="border:0px;" colspan="28">
								<div align="center" style="font-size:20px;">Short Leave Report of <? $test = new DateTime('01-' . $_POST['mon'] . '-' . $_POST['year'] . ' ');
																									echo date_format($test, 'M-Y');
																									?></div>
							</td>
						</tr>
			...
			?>
				<table width="100%" cellspacing="0" cellpadding="2" border="0">
					...
					<thead>
						<tr>
							<td style="border:0px; text-align:center; font-size:20px; font-family:bankgothic;" colspan="28">AKSID CORPORATION LTD</td>
						</tr>
						<tr>
							<td style="border:0px;" colspan="28">
								<div align="center" style="font-size:20px;">OD Report of <? $test = new DateTime('01-' . $_POST['mon'] . '-' . $_POST['year'] . ' ');
																							echo date_format($test, 'M-Y');
																							?>
								</div>
							</td>
						...
						<tr>
							...
							<th rowspan="2">
								<div align="center">Reporting Authority Name</div>
							</th>
							...
							<th rowspan="2">
								<div align="center">Reporting Authority Approval Date</div>
							</th>
							<th colspan="2">
								<div align="center">Date Interval</div>
							</th>
							...
							<th colspan="2">
								<div align="center">Time Interval</div>
							</th>
							...
							<th rowspan="2">
								<div align="center">OD Type</div>
							</th>



							<th rowspan="2">
								<div align="center">Day Tour Type</div>
							</th>
							<th colspan="3">
						...
						<tr>
							...
							<th>
								<div align="center">Place/Address</div>
							</th>
							<th>
...
and  p.DESG_ID=desg.DESG_ID ' . $od_con . 'order by s_date desc';
						...
						?>
							...
							<tr>
								...
								<td>
									<div align="center"><?= $leaveData->DEPT_DESC ?></div>
								</td>
								<td>
									<div align="center"><?= $leaveData->project ?></div>
								</td>



								<td>
									<div align="center"><?= $leaveData->reporting_auth ?></div>
								</td>
								...
								<td>
									<div align="center"><?= $leaveData->od_date ?></div>
								</td>


								<td>
									<div align="center"><?= $leaveData->auth_date ?></div>
								</td>
								...
								<td>
									<div align="center"><?= $leaveData->total_days ?></div>
								</td>


								<td>
									<div align="center"><?= $leaveData->s_time . ' ' . $leaveData->s_time_format ?></div>
								</td>
								<td>
									<div align="center"><?= $leaveData->e_time . ' ' . $leaveData->e_time_format ?></div>
								</td>


								<td>
									<div align="center"><?= $leaveData->total_hrs ?></div>
								</td>
								...
								<td>
									<div align="center"><?= find_a_field('od_type', 'type_name', 'id=' . $leaveData->type) ?></div>
								</td>



								<td>
									<div align="center"><?= $leaveData->daytour_name ?></div>
								</td>
								...
								<td>
									<div align="center"><?= $leaveData->project_name ?></div>
								</td>
			...
			?>
				<table width="100%" cellspacing="0" cellpadding="2" border="0">
					...
					<thead>
						<tr>
							<td style="border:0px; text-align:center; font-size:20px; font-family:bankgothic;" colspan="28"><?= $group->group_name ?></td>
						</tr>
						<tr>
							<td style="border:0px;" colspan="28">
								<div align="center" style="font-size:20px;">OD Report of <? $test = new DateTime('01-' . $_POST['mon'] . '-' . $_POST['year'] . ' ');
...
and  p.DESG_ID=desg.DESG_ID ' . $od_con . 'order by s_date desc';
						...
						?>
							...
							<tr>
								...
								<td align="center"><?= $leaveData->od_status ?></td>
							</tr>
						<? }
...
and  p.DESG_ID=desg.DESG_ID ' . $od_con . 'order by s_date desc';
			...
			?><table width="100%" cellspacing="0" cellpadding="2" border="0">
					<thead>
						<tr>
							<td style="border:0px; font-family:bank gothic; font-size:20px; font-weight:bold;" colspan="11" align="center">AKSID CORPORETION LTD</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<th>
								<div align="center">Total Mark</div>
							</th>
				...
				<table cellspacing="0" cellpadding="0" border="0" align="center">
					<tr>
						<thead>
							...
							<tr>
							<tr>
								<td style="border:0px; font-size:16px;" colspan="9" align="center">Billing Period <? $test = new DateTime('01-' . $_POST['mon'] . '-' . $_POST['year'] . ' ');
							...
							<th>
								<div align="center" style="width:auto;">Designation</div>
							</th>
							<th>
							...
							<th>
								<div align="center">Billing Amount</div>
							</th>
							<th>
								<div align="center">Deduction from Salary</div>
							</th>
							<th>
								<div align="center">Remarks</div>
							</th>
 ...
 where i.PBI_ID = t.PBI_ID and t.designation = d.DESG_ID and t.mon=' . $_POST['mon'] . ' and t.year=' . $_POST['year'] . ' and t.PBI_ID=a.PBI_ID and t.mobile_bill_amt>0 ' . $mobileCon . ' order by a.PBI_MOBILE asc';
							...
							$queryd = mysql_query($sqld);
							while ($data = mysql_fetch_object($queryd)) {
								$entry_by = $data->entry_by; ?> <td nowrap="nowrap"><?= $data->PBI_ID ?></td>
							<td nowrap="nowrap"><?= $data->PBI_NAME ?></td>
							<td align="left"><?= $data->Designation ?></td>
							<td><?= $data->PBI_MOBILE ?></td>
							<td>
								<div align="center"><? echo ($data->mobile_bill_limit > 0) ? $data->mobile_bill_limit : '';
													$total_mobile_bill_limit = $total_mobile_bill_limit + $data->mobile_bill_limit;
													?></div>
							</td>


							<td>
								<div align="center"><?= ($data->mobile_bill_amt > 0) ? $data->mobile_bill_amt : '';
													$total_mobile_bill = $total_mobile_bill + $data->mobile_bill_amt; ?></div>
							</td>
							<td>
								<div align="center"><?= ($data->mobile_deduction > 0) ? $data->mobile_deduction : '';
													$total_mobile_deduction = $total_mobile_deduction + $data->mobile_deduction; ?></div>
							</td> <?php
						...
						<? } ?><tr>
							...
							<td><strong>
									<div align="center"><?= $total_mobile_bill_limit; ?></div>
								</strong></td>


							<td><strong>
									<div align="center"><?= $total_mobile_bill; ?></div>
								</strong></td>
							<td><strong>
									<div align="center"><?= $total_mobile_deduction; ?></div>
								</strong></td>
			...
			?>
				...
				<table style="width:auto;margin: 0 auto; padding:0px;">
					...
					<tr>
						<td style="text-align: center;border:0px solid white;"><? $test = new DateTime('01-' . $_POST['mon'] . '-' . $_POST['year'] . ' ');
																				echo date_format($test, 'F-Y'); ?></td>
					</tr>
				</table>
				<table width="50%" border="1" cellspacing="0" cellpadding="0" style="margin:0 auto; padding:0px; ">
					<tr>
						...
						<th width="40%" style="text-align:center; ">Bill Amount</th>
					</tr>
					...
					if ($found == 0) {
						...
						$jl_sql1 = 'select proj.PROJECT_DESC , sum(a.mobile_bill_amt) as total_amt from salary_attendence_lock a, personnel_basic_info b, project proj where a.PBI_ID = b.PBI_ID and  mon=' . $_POST['mon'] . ' and year=' . $_POST['year'] . ' and a.job_location = proj.PROJECT_ID  GROUP BY proj.PROJECT_ID';
					}
					...
					?><tr style="margin:0px;">
							...
							<td style="text-align:right"><?= number_format($jl1_result1->total_amt);
															$all_proj_salary = $all_proj_salary + $jl1_result1->total_amt; ?></td>
						</tr>
						<?
					}
						...
						<tr>
							...
							<td style="text-align: right"><?= number_format($jl_result->total_amt);
															$all_dept_salary = $all_dept_salary + $jl_result->total_amt; ?></td>
						</tr><? }
								?><tr>
						...
						<td style="text-align:right"><strong>
								...
								<?= number_format($total = $all_dept_salary + $all_proj_salary); ?> </strong></td>
					</tr>
					...
					<tr>
						...
						<td style="text-align:right"><strong> <?= $data->mobile_deduction ?> </strong></td>
					</tr>
					<tr>
					<tr>
						<td align="right"><strong>Company Actual Bill</strong></td>
						<td style="text-align:right"><strong> <?= number_format($total - $data->mobile_deduction); ?>


							</strong></td>
					</tr>
					<td align="right"><strong>Grand Total</strong></td>
					<td style="text-align:right"><strong> <?= number_format($total_bill = $all_dept_salary + $all_proj_salary); ?>
						...
						?>
							...
							<tr>
								...
								<td><input type="hidden" name="PBI_ID#<?= $datas[0] ?>" id="PBI_ID#<?= $datas[0] ?>" value="<?= $datas[0] ?>" />
									...
									<input name="mobile#<?= $datas[0] ?>" type="hidden" id="mobile#<?= $datas[0] ?>" value="<?= $datas[11] ?>" />
								</td>
