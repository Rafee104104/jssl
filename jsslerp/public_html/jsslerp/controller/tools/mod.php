...
return $amount;
	...
	function next_transection_no($depot_id,$chalan_date,$table,$tr_no)
		{
			$sdate = mysql_format2stamp($chalan_date);
...
mysql_query('delete from secondary_journal where tr_from="Purchase Return" and tr_no = "'.$pr_no.'"');
mysql_query('delete from journal_item where tr_from="Purchase Return" and sr_no = "'.$pr_no.'"');
...
mysql_query('delete from secondary_journal where tr_from="Reprocess Issue" and tr_no = "'.$pr_no.'"');
mysql_query('delete from journal where tr_from="Reprocess Issue" and tr_no = "'.$pr_no.'"');
mysql_query('delete from journal_item where tr_from="Reprocess Issue" and sr_no = "'.$pr_no.'"');
...
mysql_query('delete from secondary_journal where tr_from="Reprocess Receive" and tr_no = "'.$pr_no.'"');
mysql_query('delete from journal where tr_from="Reprocess Receive" and tr_no = "'.$pr_no.'"');
mysql_query('delete from journal_item where tr_from="Reprocess Receive" and sr_no = "'.$pr_no.'"');
$oi = find_all_field('warehouse_other_issue','requisition_from','oi_no='.$pr_no);
$config = find_all_field('config_group_class','issued_to','group_for='.$_SESSION['user']['group']);
...
mysql_query('delete from journal where tr_from="Purchase" and tr_no = "'.$pr_no.'"');
mysql_query('delete from secondary_journal where tr_from="Purchase" and tr_no = "'.$pr_no.'"');
mysql_query('delete from journal_item where tr_from="Purchase" and sr_no = "'.$pr_no.'"');
...
mysql_query('delete from journal where tr_from="Import" and tr_no = "'.$or_no.'"');
mysql_query('delete from secondary_journal where tr_from="Import" and tr_no = "'.$or_no.'"');
...
if($discount_amt>0)
add_to_sec_journal($proj_id, $jv_no, $jv_date, $config_ledger->sales_discount, $narration, ($discount_amt), '0', $tr_from, $tr_no,'',$tr_id,$cc_code,$group_for);
...
function auto_insert_sales_receipt_secoundary($receipt_no)
{
	...
	$sql = 'select receipt_no, receipt_date, dr_ledger_id, sum(receipt_amt) as receipt_amt from receipt_from_customer where receipt_no="'.$receipt_no.'" GROUP by receipt_no';
	...
	add_to_sec_journal($proj_id, $jv_no, $jv_date, $data->dr_ledger_id, $narration_dr, ($data->receipt_amt), '0', $tr_from, $tr_no,'',$tr_id, $cc_code, $group_for);
...
add_to_sec_journal($proj_id, $jv_no, $jv_date, $data2->ledger_id, $narration_cr, '0', ($data2->receipt_amt),  $tr_from, $tr_no, '', $data2->chalan_no,$cc_code,$group_for);
...
function auto_insert_po_payment_secoundary($payment_no)
{
	...
	$sql = 'select payment_no, payment_date, cr_ledger_id, sum(payment_amt) as payment_amt from payment_vendor where payment_no="'.$payment_no.'" GROUP by payment_no';
	...
	add_to_sec_journal($proj_id, $jv_no, $jv_date, $data->cr_ledger_id, $narration_cr,  '0', ($data->payment_amt), $tr_from, $tr_no,'',$tr_id, $cc_code, $group_for);
...
{	
	...
	$data=mysql_fetch_object($query);
			...
			$pre_query = mysql_query($pre_sql);
			
			while($pre_data=mysql_fetch_object($pre_query)){
...
while($data=mysql_fetch_object($d_query)){
if($data->amount>0)
...
while($data=mysql_fetch_object($d_query)){
if($data->amount>0)
add_to_sec_journal($proj_id, $jv_no, $jv_date, $data->item_ledger, $narration,$data->amount, '0', $tr_from, $pr_no,'',$data->tr_id,$cc_code,$group_for);
...
function auto_insert_staff_sales_chalan_secoundary($chalan_no)
{
	...
	$data = mysql_fetch_object(mysql_query("select sum(total_amt) as total_amt,id,chalan_no,do_no,chalan_date,dealer_code from sale_other_chalan c, item_info i where c.item_id=i.item_id and i.item_brand!='Memo' and unit_price>0 and  chalan_no=".$chalan_no));
	$total_amt_memo = mysql_fetch_object(mysql_query("select sum(c.total_amt) as total_amt from sale_other_chalan c, item_info i where c.item_id=i.item_id and i.item_brand='Memo' and unit_price>0 and  chalan_no=".$chalan_no));
	$jv_date = strtotime($do->chalan_date);
...
add_to_sec_journal($proj_id, $jv_no, $jv_date, $config_ledger->sales_ledger, $narration, '0',  $data->total_amt, $tr_from, $tr_no,'',$tr_id,$cc_code);
						...
						while($data1 = mysql_fetch_object($dquery)){
...
function auto_insert_secondary_claim_chalan_secoundary($chalan_no)
{
	...
	$data = mysql_fetch_object(mysql_query("select o.offer_name,d.do_date,sum(total_amt) as total_amt from sale_so_details d,sale_so_offer o where d.item_id=o.id and  d.do_no=".$chalan_no));
...
$sqld = mysql_query($sql);
if(mysql_num_rows($sqld)>0)
{
	while($frash = mysql_fetch_object($sqld))
	{
...
function next_journal_voucher_id($date='',$tr_from = '',$depot_id='')
	...
	$sql ="select MAX(jv_no) jv_no from journal where jv_no between '".$min."' and '".$max."'";
			$jv_no=mysql_fetch_row(mysql_query($sql));
			if($jv_no[0]>$min)
			$jv=$jv_no[0]+1;
			else
			$jv=$min+1;
...
$sql = mysql_query('delete from journal where tr_from = "Collection" and tr_no = "'.$do_no.'"');
...
function add_to_journal($proj_id, $jv_no, $jv_date, $ledger_id, $narration, $dr_amt, $cr_amt, $tr_from, $tr_no,$sub_ledger='',$tr_id='',$cc_code='')
{
	$journal="INSERT INTO `journal` (
	`proj_id` ,
	`jv_no` ,
...
function add_to_sec_journal($proj_id, $jv_no, $jv_date, $ledger_id, $narration, $dr_amt, $cr_amt, $tr_from, $tr_no,$sub_ledger='',$tr_id='',$cc_code='',$group='',$entry_by='',$entry_at='',$received_from='', $bank='', $cheq_no='',$cheq_date='',$relavent_cash_head='',$checked='',$type='',$employee='',$remarks='',$reference_id='',$note='',$pay_id='') 
...
{
  $sql = 'insert into journal select * from secondary_journal where jv_no = "'.$sec_jv_no.'" and tr_from = "'.$tr_froms.'"';
$query = mysql_query($sql);
}
...
function journal_item_control_monir2($item_id, $warehouse_id,$ji_date,$item_in,$item_ex,$tr_from,$tr_no,$rate='',$r_warehouse='',$sr_no='',$bag_size='',$bag_unit='',$group_for='',$final_price='',$barcode='',$lot_no='')
{

$pre_stock=find_all_field('journal_item_monir','final_stock',' warehouse_id = "'.$warehouse_id.'" and item_id = "'.$item_id .'"  order by id desc');
...
if($item_in>0){
	...
	if($tr_from=='Transfered'){
		...
		$item_price =find_a_field('journal_item_monir','final_price',' item_id = "'.$item_id .'" and  tr_from="Transit" and  item_ex>0 order by id desc');
		$final_price =$item_price;
		$c_price=$final_price;
		}
	else{	
...
$c_price=$final_price;
    ...
    $sql="INSERT INTO journal_item_monir 
  (ji_date, item_id, warehouse_id, pre_stock, pre_price, item_in, item_ex, item_price, final_stock, final_price,tr_from, tr_no, entry_by, entry_at,relevant_warehouse,sr_no,c_price,bag_size,bag_unit,group_for, barcode,lot_no) 
  VALUES 
  ('".$ji_date."', '".$item_id."', '".$warehouse_id."', '".$pre_stock->final_stock."', '".$pre_stock->final_price."', '".$item_in."', '".$item_ex."', '".$item_price."', '".$final_stock."', '".$final_price."', '".$tr_from."', '".$tr_no."', '".$_SESSION['user']['id']."', '".date('Y-m-d H:i:s')."', '".$r_warehouse."', '".$sr_no."','".$c_price."','".$bag_size."', '".$bag_unit."', '".$group_for."', '".$barcode."','".$lot_no."')";
  mysql_query($sql);
}
...
{
	mysql_query('delete from production_floor_receive_detail_receipe_consumption where pr_id = "'.$pr.'"');
...
$update1 = mysql_query("update journal j set narration = '".addslashes($x->narration)."' where dr_amt>0 and tr_from='Collection' and tr_no='".$x->tr_no."'");
...
$data=mysql_query($sql);
while($x = mysql_fetch_object($data)){
$update1 = mysql_query("update journal j set narration = '".addslashes($x->narration)."' where cr_amt>0 and tr_from='Collection' and tr_no='".$x->tr_no."'");
$xx++;
echo $xx.'<BR>';
}
