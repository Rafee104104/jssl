...
a.PBI_MOBILE as mobile,a.PBI_PHONE as phone,
a.PBI_FATHER_NAME as father,
...
(select LOCATION_NAME from office_location where ID=a.JOB_LOCATION) as Location,concat('`',a.nid) as nid,
(select max(TRANSFER_AFFECT_DATE) from transfer_detail where PBI_ID=a.PBI_ID) as posting
...
(select AREA_NAME from area where AREA_CODE=a.PBI_AREA) as area,
(select ZONE_NAME from zon where ZONE_CODE=a.PBI_ZONE) as zone,
(select BRANCH_NAME from branch where BRANCH_ID=a.PBI_BRANCH) as Region,a.PBI_MOBILE as mobile,a.PBI_PHONE as phone,
(select LOCATION_NAME from office_location where ID=a.JOB_LOCATION) as Location,concat('`',a.nid) as nid,

grace_type,employee_type,
(select max(TRANSFER_AFFECT_DATE) from transfer_detail where PBI_ID=a.PBI_ID) as posting
...
echo $sql="SELECT id, PBI_ID, type, lv_dur, s_date, e_date, total_days, half_or_full, reason, paid_status, leave_join_date, leave_address, leave_mobile_number, leave_responsibility_name, leave_status, note 
...
if($_POST['report']==77) 
{
?><table border="0" cellpadding="2" cellspacing="0"><thead><tr><th style="border:0px;" colspan="38"><?=$str?></th></tr>
...
<tr>
  ...
  In word: <?=convertNumberMhafuz($t_net);?>
...
s.special_allowance as sp_Allow,s.incentive_allowance as inc_allow,s.ta as ta_da,s.food_allowance as food_allow,s.mobile_allowance as mobile_allow,s.house_rent
from personnel_basic_info a , salary_info s
...
if($_POST['department']!=''){$department_con=' and t.pbi_department = "'.$_POST['department'].'"';}
...
'.$PBI_ORG_con.$department_con.$job_location_con.$pbi_id_con.$section_con.' 
...
if($_POST['PBI_DOMAIN']!=''){$section_con=' and a.PBI_DOMAIN = "'.$_POST['PBI_DOMAIN'].'"';}
?>
<center><tr><th style="border:0px;" colspan="41"><?=$str?></th></tr></center>
<?
...
?>
<h2>Department: <?=find_a_field('department','DEPT_DESC','DEPT_ID="'.$data2->dept_id.'"');?></h2>
...
?>
<table width="100%" cellspacing="0" cellpadding="2" border="0"><thead><tr><th style="border:0px;" colspan="32"><?=$str?></th></tr>
...
<tr>
  ...
  <td><strong><?=round($total_nos);?></strong></td>
...
?>
<table width="100%" cellspacing="0" cellpadding="2" border="0">
<thead><tr><th style="border:0px;" colspan="30"><?=$str?></th></tr>
...
<td><?=(int)$data->actual_basic_salary?><? $total_actual_basic_salary = $total_actual_basic_salary + $data->actual_basic_salary;?></td>
  ...
  <td>
  <? echo $cash_paid=find_a_field('salary_attendence t,personnel_basic_info a ','sum(t.total_payable-t.bank_paid)','t.pbi_held_up=0 and t.PBI_ID=a.PBI_ID and a.PBI_DEPARTMENT="'.$data->PBI_DEPARTMENT.'"  
...
<tr>
  ...
  <td><strong><?=$total_help_up_paid?></strong></td>
  <td><strong><?=$total_cash_payment?></strong></td>
  <td><strong><?=$total_bank_payment_dbbl?></strong></td>
  <td><strong><?=$total_bank_payment_prime?></strong></td>
  <td><strong><?=$total_bank_payment_ibbl?></strong></td>
  <td><strong><?=$total_net_payable?></strong></td>
  <td>&nbsp;</td>
  <td>&nbsp;</td>
</tr></tbody></table>
...
<td><?=(int)$data->actual_basic_salary?><? $total_actual_basic_salary = $total_actual_basic_salary + $data->actual_basic_salary;?></td>
  ...
  <td>
  <? echo $help_up_paid = find_a_field('salary_attendence t,personnel_basic_info a ','sum(total_payable)','t.pbi_held_up=1 and t.PBI_ID=a.PBI_ID 
  and t.pbi_department="'.$data->PBI_DEPARTMENT.'" '.$org_con.' 
  and t.pbi_job_location="'.$data->job_location.'"
  and t.pbi_region =0 and t.mon='.$_POST['mon'].' and t.year='.$_POST['year'].' '); $total_help_up_paid = $total_help_up_paid + $help_up_paid;?>
  </td>
...
<center>
  <h1><?=$company?></h1>
  <strong>Leave Report-<?=$year;?></strong><br>
  Department: <?=$_POST['department'] ?><br>
...
<tr>
<td colspan="2"><strong>Total :</strong></td>
<td><strong><?=round($g_t_e)?></strong></td>
<td><strong><?=round($g_t_b)?></strong></td>
<td><strong><?=round($g_t_cash)?></strong></td>
<td><strong><?=round($g_t_bank)?></strong></td>
<td><strong><?=round($g_t_negetive)?></strong></td>
<td><strong><?=round($g_t_positive)?></strong></td>
</tr>
</tbody></table>
<?
}
