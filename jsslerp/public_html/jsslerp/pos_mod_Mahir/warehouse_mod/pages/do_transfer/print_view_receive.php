...
mysql_query('update do_issue_master set status = "'.$_POST['status'].'", rec_sl_no = "'.$_POST['rec_sl_no'].'", receive_date = "'.$_POST['receive_date'].'", received_by = "'.$_POST['received_by'].'" where pi_no="'.$req_no.'"');

// all do no
	$sql1="select p.do_no from do_issue_detail p where pi_no='$req_no'";
	$data1=mysql_query($sql1);
	
	while($all1=mysql_fetch_object($data1)){
		$do[] = $all1->do_no;
	}	
// end
