...
if(isset($_POST['confirmm']))
{
		...
		$oi_no = $_POST['oi_no'];
		$master_all=find_all_field('warehouse_other_issue','*','oi_no="'.$oi_no.'"');
        $sql = 'select w.*,s.item_ledger from warehouse_other_issue_detail w, item_info i, item_sub_group s where i.item_id=w.item_id and s.sub_group_id=i.sub_group_id and w.oi_no="'.$oi_no.'" and w.issue_type="CASH Sales"';
		$qry = mysql_query($sql);
		$page_for = 'CASH Sales';
		...
		add_to_sec_journal($proj_id, $jv_no, $jv_date, $data->item_ledger, $narration, '0',$data->amount,  $tr_from, $tr_no,'',$tr_id,$cc_code,$group_for,'','','','','','','','YES');
		$total_amt +=$data->amount;
		}
		add_to_sec_journal($proj_id, $jv_no, $jv_date, $master_all->cash_ledger, $narration, $total_amt,'0', $tr_from, $tr_no,'',$tr_id=0,$cc_code,$group_for,'','','','','','','','YES');
		unset($_POST);
		$_POST[$unique]=$$unique;
		$_POST['entry_by']=$_SESSION['user']['id'];
		$_POST['entry_at']=date('Y-m-d h:s:i');
		...
		$crud   = new crud($table_master);
		$crud->update($unique);
		sec_journal_journal($jv_no,$jv_no,'CASH Sales');
		unset($$unique);
		unset($_SESSION['oi_no2']);
		$type=1;
                ...
                <div class="col-sm-6 col-md-6 col-lg-6 col-xl-6">
                    <div class="container n-form2">
                        <div class="form-group row m-0 pb-1">
							  <? $field='oi_no';?>
                            <label for="<?=$field?>" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Cash Sales No:</label>
                        ...
                        <div class="form-group row m-0 pb-1">
                            ...
                            <label for="<?=$field?>" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Cash Sales Date:</label>
                            <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
...
<div class="form-group row m-0 pb-1">
							...
							<option> </option>
								<? foreign_relation('accounts_ledger','ledger_id','ledger_name',$$field,'ledger_group_id in (126001,126002)')?>
							</select>
...
<input type="hidden" id="issue_type" name="issue_type" value="CASH Sales"  />
</span>
    ...
    <form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
        <div class="container-fluid p-0 ">
            ...
            <div class="n-form-btn-class">
				<input type="hidden" id="<?=$unique?>" name="<?=$unique?>" value="<?=$$unique?>"  />
<input type="hidden" id="oi_date" name="oi_date" value="<?=$oi_date?>"  />
				<input name="confirmm" type="submit" class="btn1 btn1-bg-submit" value="CONFIRM CASH SALES"  />
            </div>
