...
if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and m.sr_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
    ...
    $res="select m.do_no, m.do_no, m.sr_date, m.dealer_code, m.do_type, m.depot_id,  d.dealer_name_e, m.entry_by, m.status from 
sale_return_master m, sale_return_details c,  dealer_info d, user_group u 
where 
  m.group_for=u.id  and m.dealer_code=d.dealer_code and m.status!='MANUAL' and m.do_no=c.do_no ".$group_for_con.$con.$dealer_con."  group by c.do_no order by  m.sr_date DESC ";
		...
		?>
					...
					?>
                        ...
                        <tr>
							...
							<td><?= $row->status?></td>
                            <td>
								<button type="button" name="submitit" id="submitit" onclick="custom(<?= $row->do_no?>)" class="btn2 btn1-bg-submit">
