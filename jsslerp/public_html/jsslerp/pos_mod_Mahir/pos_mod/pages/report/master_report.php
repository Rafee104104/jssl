...
switch ($_POST['report']) {
    case 1:
		...
		$sql='select ji_date,i.item_id,i.finish_goods_code as fg_code,i.item_name,s.sub_group_name as Category,i.unit_name as unit,a.item_in as `IN`,a.item_ex as `OUT`,a.item_price as rate,((a.item_in+a.item_ex)*a.item_price) as amount,a.tr_from as transaction_type,sr_no as transaction_no,a.entry_at,c.fname as User 
...
if($_POST['report']==88888) 

{

$sqlll="select g.group_name, s.sub_group_name, i.item_id, i.item_name,  i.sku_code, i.unit_name,  i.pack_size, 
    i.m_price,i.cost_price, i.d_price, i.t_price, i.f_price, i.status
...
<thead><tr><td style="border:0px;" colspan="7"><?=$str?></td></tr>
...
<th bgcolor="#5390F5"><div align="center">Cost Price</div></th>
<th bgcolor="#FF0000">Status</th>
<th bgcolor="#FF0000" colspan="2">Submit</th>
</tr>
...
while($datas=mysql_fetch_row($queryyy)){$s++;
...
<tr><td><?=$s?></td>
  ...
  <td bgcolor="#99FFFF"><input name="m_price#<?=$datas[2]?>" type="text" id="m_price#<?=$datas[2]?>" value="<?=$datas[7]?>"  style="width:70px;" /></td>
      ...
      <td bgcolor="#99FFFF"><input name="cost_price#<?=$datas[2]?>" type="text" id="cost_price#<?=$datas[2]?>" value="<?=$datas[8]?>"  style="width:70px;" /></td>
  <td>
  ...
  <td colspan="2"><div id="po<?=$datas[2]?>"><input type="button" name="Change" value="Change" onclick="getData2('<?=$ajax_page?>', 'po<?=$datas[2]?>',<?=$datas[2]?>,

document.getElementById('pack_size#<?=$datas[2]?>').value+'#'+
document.getElementById('m_price#<?=$datas[2]?>').value+'#'+
document.getElementById('cost_price#<?=$datas[2]?>').value+'#'+
...
$avg_rate = find_a_field('journal_item', 'avg(item_price)', 'item_id = "'.$data->item_id.'" and warehouse_id="'.$_SESSION['user']['depot'].'" and item_in>0');
...
$cost_price2=find_a_field('item_info','cost_price','item_id="'.$data->item_id.'"');
$sales_price2=find_a_field('item_info','m_price','item_id="'.$data->item_id.'"');
$amt2 = $i->final_stock*$sales_price2;
		   ?>
<tr>
<td><?=$j?></td>
...
<td style="text-align:right"><?=@number_format(find_a_field('item_info','cost_price','item_id="'.$data->item_id.'"'),2)?></td>
<td style="text-align:right"><?=@number_format(find_a_field('item_info','m_price','item_id="'.$data->item_id.'"'),2)?></td>
...
<div class="new-sa">
        ...
        <?
		  ...
		  $barsql = 'select i.item_name,i.item_id,i.finish_goods_code,i.m_price from item_info i where i.item_id="'.$_POST['item_id'].'"';
          ...
          // Example usage:
        $longText = $bardata->item_name;
        $shortenedText = shortenText($longText, 20);
        $bardata->item_name = $shortenedText.' ('.$bardata->m_price.'TK)';
		...
		<!--  <td style="width: 25%; border: none;     padding:  10px 0px;">-->
		<!--      <div align="center">-->
  <!--      		  <?='<img class="barcode" alt="'.$barcodeText.'" src="barcode.php?text='.$barcodeText.'&codetype='.$barcodeType.'&orientation='.$barcodeDisplay.'&size='.$barcodeSize.'&print='.$printText.'" style="width:35mm;height:5.23mm;"/>' ?>-->
  <!--      		  <p style=" margin: 5px 0px; text-align: center; font-size: 8px; "><?=$bardata->finish_goods_code.'-'.$bardata->item_name;?></p>-->
  <!--      	   </div>-->
		<!--  </td>-->
  ...
  <!--      	   </div>-->
		...
		<!--1-->
		  ...
		  <td style="width: 25%; border: none;     padding:  8px 0px;">
		      <div align="center">
        		  <?='<img class="barcode" alt="'.$barcodeText.'" src="barcode.php?text='.$barcodeText.'&codetype='.$barcodeType.'&orientation='.$barcodeDisplay.'&size='.$barcodeSize.'&print='.$printText.'" style="width:35mm;height:5.23mm;"/>' ?>
        		  <p style=" margin: 5px 0px; text-align: center; font-size: 8px; "><?=$bardata->item_name;?></p>
        	   </div>
		  </td>
		...
		<!--2-->
		  ...
		  <td style="width: 25%; border: none;     padding:  18px 0px;">
		      <div align="center">
        		  <?='<img class="barcode" alt="'.$barcodeText.'" src="barcode.php?text='.$barcodeText.'&codetype='.$barcodeType.'&orientation='.$barcodeDisplay.'&size='.$barcodeSize.'&print='.$printText.'" style="width:35mm;height:5.23mm;"/>' ?>
        		  <p style=" margin: 5px 0px; text-align: center; font-size: 8px; "><?=$bardata->item_name;?></p>
        	   </div>
...
<div class="new-sa">
        ...
        // short text usage:
        $longText = $bardata->item_name;
        $shortenedText = shortenText($longText, 24);
        //$bardata->item_name = $shortenedText.' ('.$bardata->m_price.'TK)';
        $bardata->item_name = $shortenedText;
        $price = $bardata->m_price;
  ...
  <!--      	   </div>-->
		...
		<tr  bordercolor="#000000">
		  ...
		  <td style="width: 20%; border: none; padding: 7px 30px 30px 0px;">
		      <div align="center">
        		  <?='<img class="barcode" alt="'.$barcodeText.'" src="barcode.php?text='.$barcodeText.'&codetype='.$barcodeType.'&orientation='.$barcodeDisplay.'&size='.$barcodeSize.'&print='.$printText.'" style="width:60mm;height:11.23mm;"/>' ?>
        		  <p style=" margin: 5px 0px; text-align: center; font-size: 12px; font-weight: 800;"><?=$bardata->finish_goods_code.'-'.$bardata->item_name;?></p>
        		  <p style=" padding-top: 8px; margin: 5px 0px; text-align: center; font-size: 18px; font-weight: 800;">Price: <?=$price?> TK</p>
        	   </div>
		  </td>
		</tr>
