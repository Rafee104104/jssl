...
<thead>
<tr>
 <td colspan="13" style="border:0px"><?=$str?></td>
</tr>
<tr>
<th>S/L</th>
<th>POS No.</th>
...
$possql='select m.pos_id,m.pos_date,d.dealer_name,i.item_name, s.qty,s.rate,s.total_amt,s.discount_amt,m.register_discount,p.name as PBI_NAME,u.fname,m.entry_at,m.vat_percent from sale_pos_master m left join dealer_pos d on d.dealer_code=m.dealer_id left join pos_sales_agent p on p.id=m.sales_person left join user_activity_management u on u.user_id=m.entry_by, sale_pos_details s,item_info i where s.item_id=i.item_id and m.pos_id=s.pos_id '.$warehouse_con.$item_con.$date_con.$dealer_con.' ';
$posqry = mysql_query($possql);		 
while($data=mysql_fetch_object($posqry)){

$total_amount += $total_receivable;
$total_dis = $data->discount_amt+$data->register_discount;
$real_amount=$data->total_amt+$data->discount_amt;
$total_receivable =  $real_amount-$total_dis;
?>
<tr>
<td><?=$j?></td>
<td><a href="../pos/pos_print_view.php?v_no=<?=$data->pos_id?>" target="_blank"><?=$data->pos_id?></a></td>
<td><?=$data->pos_date?></td>
<td><?=$data->dealer_name?></td>
<td><?=$data->item_name?></td>
<td><?=$data->qty?></td>
<td><?=$data->rate?></td>
<td style="text-align:right"><?=number_format($real_amount,2)?></td>
<td style="text-align:right"><?=@number_format($total_dis,2)?></td>
...
<tbody>
	...
	<tr>
		...
		<th><div align="left">Receive type</div></th>
...
if(isset($garden_id)) 	{$garden_id_con=' and b.garden_id='.$garden_id;}
 ...
 $res='
select a.or_no as or_no, a.or_date, e.item_name,a.requisition_from,a.vendor_name as Purchase_From,e.unit_name,b.rate,b.qty,b.amount,a.receive_type,u.username,a.entry_at from  
 warehouse_other_receive a, warehouse_other_receive_detail b, item_info e, user_activity_management u  where a.or_no=b.or_no and a.receive_type="Local Purchase" and b.item_id=e.item_id and u.user_id=a.entry_by '.$warehouse_con.$item_con.$date_con.$dealer_con.' order by a.or_no';
