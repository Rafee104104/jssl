...
$main_sql = "select item_info.item_name,item_info.d_price,item_info.item_id,`$table_details`.id,`$table_details`.qty, `$table_details`.rate, `$table_details`.total_amt, `$table_details`.discount_amt,`$table_details`.discount, (sum(journal_item.item_in-journal_item.item_ex)-`$table_details`.qty) as stock, `$table_details`.serial_no, `$table_details`.dealer_id, `$table_details`.discount_percent from `$table_details` left join item_info on item_info.item_id = `$table_details`.item_id left join journal_item on journal_item.item_id = item_info.item_id where `$table_details`.pos_id='$pos_id' group by sale_pos_details.id order by sale_pos_details.id desc";
...
$datas['item_details'][$i]['discount_percent'] = number_format($discount_percent, 2, '.', '');
