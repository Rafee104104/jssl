	...
	<div class="container-fluid sr">
			...
			<div class="col-sm-8" id='item_details_section'>
				...
				<div class="container n-form1">
				  <div class="n-form-btn-class">
					    ...
					    <input class="btn1 btn1-bg-cancel" name="exchange_sale" type="button" value="Exchange Sales" onclick="exchange_sale_show()" />
						<input type="text" name="search_pos_id" id="search_pos_id" placeholder="Enter POS ID" onchange="us_func(this.value)" style="width:20%; display:none;" />
						<input type="hidden" id="exchange_check" name="exchange_check" value="0" />
				  </div>
					...
					<div class="form-group row m-0 pl-3 pr-0">
						...
						<div class="col-sm-7 p-0">
							<div class="form-group row m-0 pl-3 pr-3">
								<label for="group_name" class="col-sm-5 pl-0 pr-0 col-form-label">Sales Person :</label>
								<div class="col-sm-7 p-0">
									<select name="sales_person" id="sales_person">
										<option value="">--Select One--</option>
										<? foreign_relation('pos_sales_agent','id','name',$sales_person,'status="Active"')?>
									</select>
				...
				<table class="table1  table-striped table-bordered table-hover table-sm">
					...
					<tr>
						<td colspan="9"><h3 align="center" >No Item Found</h3></td>
					</tr>
					</tbody>
				</table>
					...
					<div class="form-group row m-0 pl-3 pr-3">
						...
						<div class="col-sm-8 p-0">
							<select name="payment_method" id="payment_method">
								<option value="">-Select One-</option>
								<? foreign_relation('pos_payment_method','id','payment_method',$payment_method,'status="Active"')?>
						...
						<div class="col-sm-8 p-0">
							<input type="text" name="paid_amt" id="paid_amt" onchange="return_amt()" required>
							<input type="hidden" name="left_amt" id="left_amt" required>
						</div>
						...
						<div class="col-sm-8 p-0">
							<input type="text" name="vat_percent" id="vat_percent" value="0">
						</div>
						
						<label for="group_name" class="col-sm-4 pl-0 pr-0 col-form-label">Receive Amount</label>
						<div class="col-sm-8 p-0">
							<input type="text" name="temp_receive" id="temp_receive" onchange="return_amt()" value="">
						</div>
						
						<label for="group_name" class="col-sm-4 pl-0 pr-0 col-form-label">Return Amount</label>
						<div class="col-sm-8 p-0">
							<input type="text" name="temp_return" id="temp_return" value="" readonly="readonly">
						</div>
		...
		<div class="receipt sr3 pl-2 pr-2">
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				...
				<tr>
					<th id="company_name" style="text-align: center; color:#000;"><h2 style=" font-size: 20px !important; font-weight: 600; "><?=$_SESSION['company_name']?></h2></th>
				...
				<tr>
					<th id="company_address" style="text-align: center; color:#000;"><?=find_a_field('user_group','address','id="'.$_SESSION['user']['group'].'"')?></th>
				</tr>

				<tr>
					<th colspan="6" style="font-size: 1.5625rem; text-align: center; padding-top: 10px; padding-bottom: 10px;">
						<span style="border: 1px solid #000;padding: 5px;padding-left: 10px;padding-right: 10px; color:#000;">Order #<span  id="invoice_no"></span></span>
					</th>
				</tr>
                ...
                <tr>
					<th style="font-weight: normal; color:#000;">Customer Name: <span id="show_customer"></span></th>
				</tr>
				<tr>
					<th style="font-weight: normal; color:#000;">Customer Contact: <span id="show_contact"></span></th>
				</tr>
				<tr>
					<th style="font-weight: normal; color:#000;">Cashier: <?=$_SESSION['user']['fname']?></th>
				</tr>
					...
					<td>
						<table style="width:100%;">
							...
							<tr class="bold">
								...
								<td colspan="1" id="tia" style="text-align: right;  border-top:1px solid #333; "></td>
							</tr>
							
							<tr class="bold">
								<td colspan="5" style="text-align:right;  border-top:1px solid #333; color:#000; ">Total Discount :</td>
								<td colspan="1" id="tot_discount" style="text-align: right;  border-top:1px solid #333; color:#000; "></td>
							</tr>

							<tr class="bold">
								<td colspan="5" style="text-align:right;">VAT <span id="vat_perc"></span>%:</td>
								<td colspan="1" id="vat_amount" style="text-align: right;"></td>
							</tr>
							
							<tr class="bold"  style=" color: #333 !important; ">
								<td colspan="5" style="text-align:right;  border-top:1px solid #333; ">Grand Total :</td>
								<td colspan="1" id="grand_total" style="text-align: right;  border-top:1px solid #333; "></td>
							</tr>
							<!--<tr>
...
-->
				...
				<tr>
					<td colspan="6" style="text-align:center; color:red;"><span id="exchange_msgs"></span></td>
				</tr>
				
				<tr>
					<td colspan="6" style="text-align:center; color:#000;">Exchange items within 3 days of purchase, in original condition with proof of purchase.</td>
				</tr>
...
close_s_sale();
get_all_info(pos_id);
document.getElementById("search_pos_id").style.display='none';
}
...
var pos_id = $("#pos_id").val();
var exchange_checker = $("#exchange_check").val();
...
var disc_percent  = parseFloat($("#item_discount_percent"+details_id).val());
...
if(disc_percent>0){
var disc_amt = (item_total_amt*disc_percent)/100;
var new_amt_percent = item_total_amt-disc_amt;
$("#total_item_amt_"+details_id).html(new_amt_percent);
}
...
i_price:price,
i_disc:disc,
i_disc_percent:disc_percent,
exchange_checker:exchange_checker,
...
function add_payment(){
	...
	var left_amt = $("#left_amt").val()*1;/*total_amt-paid_amt*/
	var payment_method = $("#payment_method").val();
	var render_pa="";
	var payment_result = "";
	if(item_in_cart == 0 && paid_amt>0){
	alert("No Item Found to Pay");	
	}else if(paid_amt>left_amt){
	alert("Paid Amount Should Not Gretter Then Sales Amount");	
	$("#paid_amt").val(left_amt);
	return_amt();
	}
...
key++;
text+="<tr "+now_style+"><td><button title='Click here to remove this item' type='button' id='remove_item_"+val.id+"' title='Remove the item' onclick='remove_item("+val.id+")'>&times;</button></td><td>"+val.serial_no+"</td><td style='width:200px'>"+val.item_name+"</td><td>"+val.stock+"<input type='hidden' style='width:70px' id='stock_"+val.id+"' name='stock_[]' value='"+val.stock+"' /></td><td><input style='height:34px' id='item_price_"+val.id+"' name='item_price[]' value='"+val.rate+"' onchange='cal_again("+val.id+")' /><input type='hidden' id='item_actual_price_"+val.id+"' name='item_actual_price_[]' value='"+val.actual_price+"' /></td><td> <input style='width:50px' type='text' id='item_qty_"+val.id+"' name='item_qty[]' value='"+val.qty+"' "+val.unit_readonly+"  onchange='cal_again("+val.id+")'></td><td> <input style='width:50px' type='text' id='item_discount_percent"+val.id+"' name='item_discount_percent[]' value='"+val.discount_percent+"' onchange='cal_again("+val.id+")'></td><td> <input style='width:50px' type='text' id='item_discount_"+val.id+"' name='item_discount[]' value='"+val.discount+"' onchange='cal_again("+val.id+")'></td><td id='total_item_amt_"+val.id+"' style='width:100px;'>"+val.total_amt+"</td>";
});
text+="</tr><tr><td colspan='9' style='background-color:#e6e6e6;padding:1%;border-radius:0px 0px 10px 10px;'>&nbsp;</td></tr>";
...
var exchange_message = data.master_info.exchange_notes;
$(".receipt").fadeIn(500);
...
r_payment_history+="<tr><td style='color:#000'>"+val2.payment_method+"</td><td class='moveTextToRight' style='color:#000'>"+paid_excluding_vat+"</td></tr>";
});
...
if(exchange_message>0){
$("#exchange_msgs").html('Exchanged Order');
}

	var con= 'Company '+$("#company_name").text()+'. POS ID '+ pos_id;
	// convert - to _
//	con = con.replace(/-/g, "-");
	// convert space to %20
...
function exchange_sale_show(){
//alert('Are you sure!');
var pos_id = prompt("Please enter POS Order No.", "");

//document.getElementById("search_pos_id").style.display='block';
document.getElementById("exchange_check").value=1;
if(pos_id>0){
us_func(pos_id);
}
}
...
function return_amt(){
var receive_amt = document.getElementById("temp_receive").value*1;
 var paid_amt = document.getElementById("paid_amt").value*1;
 var return_amt = receive_amt-paid_amt;
 if(receive_amt>paid_amt){
 document.getElementById("temp_return").value = return_amt;
 }
 
 
}

function receipt22(pos_id){
    window.open('pos_receipt_view.php?v_no='+pos_id,'_blank');
}
