...
if(isset($_REQUEST['show']))
{
  ...
  <tr <?=($xx%2==0)?' bgcolor="#EDEDF4"':'';$xx++;?>>
	...
	<? if($data[4]=='Sales') {?>
		<table id="grp"  class="tabledesign table-bordered table-condensed" width="100%" cellspacing="0" cellpadding="2" border="0" style="font-size:12px;">
			<?php /*?><tr>
				<td width="50%" bgcolor="#CCCCFF"><strong>Item Name</strong></td>
				<td width="10%" bgcolor="#CCCCFF"><strong>Qty</strong></td>
				<td width="10%" bgcolor="#CCCCFF"><strong>Rate</strong></td>
				<td width="15%" bgcolor="#CCCCFF"><strong>Value</strong></td>
			</tr><?php */?>
	...
	<? if($data[4]=='Sales Return') {?>
		<table id="grp"  class="tabledesign table-bordered table-condensed" width="100%" cellspacing="0" cellpadding="2" border="0" style="font-size:12px;">
			...
			<?
				...
				while ($sr_data = mysql_fetch_object ($sr_query)) {
			?>
			<tr>
  ...
  <?php } ?>
  <tr>
    <th colspan="4" align="center"><?php /*?>Difference Balance : <?php echo number_format($t_total,2)." ".$t_type?> <?php */?></th>
