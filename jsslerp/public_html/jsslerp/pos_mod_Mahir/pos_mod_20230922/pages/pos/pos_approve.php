...
$sql = 'select m.pos_id,m.pos_date,s.qty,s.rate,p.total_amt as pos_amt,s.discount,p.paid_amt,i.item_name,d.dealer_name_e,sub.ledger_id from sale_pos_master m left join dealer_info d on d.dealer_code=m.dealer_id left join pos_payment p on p.pos_id=m.pos_id, sale_pos_details s, item_info i, item_sub_group sub where s.item_id=i.item_id and i.sub_group_id=sub.sub_group_id and m.pos_id=s.pos_id and m.status="CHECKED" and m.pos_id="'.$pos_id.'"';
$qry = mysql_query($sql);
while($data=mysql_fetch_object($qry)){
$total_amt +=$data->pos_amt;
$total_paid +=$data->paid_amt;
add_to_sec_journal($proj_id, $jv_no, $jv_date, $data->ledger_id, $narration, '0',$data->pos_amt,  $tr_from, $tr_no,'',$tr_id,$cc_code,$group_for);
}
