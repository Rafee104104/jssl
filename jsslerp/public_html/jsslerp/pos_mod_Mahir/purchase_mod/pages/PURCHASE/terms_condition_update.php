 ...
 "'.$status.'", "'.$entry_at.'", "'.$entry_by.'", "'.$freight_charge.'", "'.$quote_no.'", "'.$view_quote_no.'")';
	...
	mysql_query($ins_sql);
	 ...
	 $sql_trm = 'select * from terms_conditions where 1 order by id';
		...
		$query_trm = mysql_query($sql_trm);

		while($data_trm=mysql_fetch_object($query_trm))
...
<table width="100%" border="0" align="center" id="grp" cellpadding="0" cellspacing="0" style="font-size:12px; text-transform:uppercase;">
  ...
  <tr>
    <th width="21%">Terms </th>
    <th width="79%">Terms &amp; Condition</th>
    </tr>
...
<? //if($po_no[$data->po_no]==0) { } ?>
  ...
  <tr bgcolor="<?=($i%2)?'#F5F5F5':'#fff';?>">
    <td bgcolor="#99CCFF">
	 <input name="order_no_<?=$datafg->id?>" type="hidden" id="order_no_<?=$datafg->id?>" value="<?=$datafg->id; ?>" readonly="" style="width:100px; height:30px; "/>
<input name="tearms_<?=$datafg->id?>" type="text" id="tearms_<?=$datafg->id?>" value="<?=$datafg->tearms; ?>"   style="width:100px; height:30px; "/></td>
 <td bgcolor="#99CCFF"><input name="terms_conditions_<?=$datafg->id?>" type="text" id="terms_conditions_<?=$datafg->id?>" value="<?=$datafg->terms_conditions; ?>" style="width:100px; height:30px; "/></td>
    </tr>
