...
<table id="grp" class="table1  table-striped table-bordered table-hover table-sm">
	...
	<tbody class="tbody1">
<? 
  $res='select w.id,i.item_name,i.item_description,i.unit_name,w.req_qty,w.total_unit from production_issue_detail w,item_info i,item_sub_group s where i.item_id=w.item_id and i.sub_group_id=s.sub_group_id and w.pi_no='.$_GET['old_pi_no'];
	$rs=mysql_query($res);
	  $ledger=0;
	while($row=mysql_fetch_object($rs)){
		$i++;
...
?>
	...
	<?php /*?>  <? if($ledger>0){echo "Please set Ledger Code.";}else{ ?><?php */?>
