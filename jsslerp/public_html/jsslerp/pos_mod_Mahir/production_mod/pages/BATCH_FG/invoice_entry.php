...
if(isset($_POST['confirm'])){
	...
	$queryby = mysql_query($sql_by);
	
	while($databy =mysql_fetch_object($queryby)){
...
journal_item_control($item_id,$warehouse_id,$pr_date,0,$total_unit,'Consumption',$data->id,$unit_price,0,$pr_no,'','',$_SESSION['user']['group'],'');
	...
	$con_sql1="UPDATE production_receive_master SET cost_price='".$pr_unit_price."', cost_amt='".$tot_production_cost."', status='".$status."',  checked_by='".$entry_by."', checked_at='".$entry_at."' WHERE pr_no = '".$pr_no."' ";
	 ...
	 $oh_query = mysql_query($oh_sql);
	 
	 
	 while($oh_data=mysql_fetch_object($oh_query)){
	 ...
	 $rm_query = mysql_query($rm_sql);
	 
	 
	 while($rm_data=mysql_fetch_object($rm_query)){
...
//		$do_con .= ' and m.do_no in ('.$_POST['do_no'].') ';
  ...
  $queryfg = mysql_query($sqlfg);

  while($datafg=mysql_fetch_object($queryfg)){$i++;
...
//		$do_con .= ' and m.do_no in ('.$_POST['do_no'].') ';
    ...
    echo  $sqlby = "select a.*, s.sub_group_name, i.finish_goods_code, i.item_name, i.unit_name from batch_by_product a, item_info i, item_sub_group s where i.item_id=a.item_id  and i.sub_group_id=s.sub_group_id and a.batch_no='".$ms_data->batch_no."' and a.final_qty>0 order by i.sub_group_id, i.item_name";

  $queryby = mysql_query($sqlby);

  while($databy=mysql_fetch_object($queryby)){$i++;
  ?>
