	 ...
	 <td width="20%" rowspan="8" align="center" ><strong>
      ...
      <input type="submit" name="master_data" id="master_data" value="Initiate Entry" style="width:180px; text-align:center; font-weight:bold; font-size:12px; height:30px; color:#090"/>
								  ...
								  <tr>
								    ...
								    <td ><input name="batch_date" type="text" id="batch_date" style="width:90%; height:32px;" value="<?=$batch_date;?>"  required tabindex="1" /></td>
								    ...
								    <td ><input name="batch_qty" type="text" id="batch_qty" style="width:90%; height:32px;" value="<?=$batch_qty;?> " required tabindex="1" /></td>
							      </tr>
								</table>
...
<input name="foe_amt_<?=$data->ledger_id?>" type="text" id="foe_amt_<?=$data->ledger_id?>" value="<?=$foe_exp[$data->ledger_id]; $tot_foe_exp +=$foe_exp[$data->ledger_id];?>" onkeyup="SUMcalculation(<?=$data->ledger_id?>)"  style="width:120px; height:30px;" readonly="" /></td>
 ...
 <? } ?>
  ...
  <tr>
    ...
    <td bgcolor="#90EE90" align="right"><input name="foe_amt" id="foe_amt" type="text" size="10"  value="<?=$tot_foe_exp;?>" style="width:120px; height:30px;" readonly="" /></td>
  </tr>
...
<? //if($po_no[$data->po_no]==0) { } ?>
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
    ...
    <td bgcolor="#99CCFF" class="text-center"><?=find_a_field('journal_item','sum(item_in-item_ex)',' warehouse_id = '.$datafg->warehouse_id.' and item_id='.$datafg->item_id);?></td>
    <td bgcolor="#99CCFF">
...
<? //if($po_no[$data->po_no]==0) { } ?>
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
    ...
    <td> <?=$datafg->item_name?> </td>
    <td bgcolor="#99CCFF"><?=$datafg->unit_name?></td>
	<td bgcolor="#99CCFF"><?=$datafg->type?></td>
	<td bgcolor="#99CCFF"><?=$datafg->rate_ratio?></td>
    <td bgcolor="#99CCFF">
	<input name="order_no_<?=$datafg->id?>" type="hidden" id="order_no_<?=$datafg->id?>" value="<?=$datafg->id?>" />
      <input name="item_id_<?=$datafg->id?>" type="hidden" id="item_id_<?=$datafg->id?>" value="<?=$datafg->item_id?>" />
