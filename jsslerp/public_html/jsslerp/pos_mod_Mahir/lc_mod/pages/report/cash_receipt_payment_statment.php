...
$total_opening_amt = find_a_field_sql($sql_op);
  ...
  <tr>
	...
	$rcount     = mysql_num_rows($rquery);

	if($rcount>0)

	{

	while($infor=mysql_fetch_object($rquery)){
...
while($data2=mysql_fetch_object($query2)){$s++;
...
while($data3=mysql_fetch_object($query3)){$s++;
...
while($data4=mysql_fetch_object($query4)){$s++;
