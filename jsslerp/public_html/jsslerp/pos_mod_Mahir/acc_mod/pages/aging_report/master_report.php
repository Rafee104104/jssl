...
elseif($_REQUEST['report']==230226001)
{
...
<!--		<img src="../../../logo/tecno.png" style="width:220px;">-->
		...
		<table width="100%" border="0" cellpadding="2" cellspacing="0" style=" font-size:14px; color:#000000;">
		
		<thead>
		<?
	  $first_t = $t_date;
      $first_f =  date('Y-m-d', strtotime($first_t.' - 29 days'));
	 
	  $second_t =  date('Y-m-d', strtotime($first_f.' - 1 days'));
	  $second_f =  date('Y-m-d', strtotime($second_t.' - 29 days'));
	  
	  $third_t =  date('Y-m-d', strtotime($second_f.' - 1 days'));
	  $third_f =  date('Y-m-d', strtotime($third_t.' - 29 days'));
	  
	  $last_t =  date('Y-m-d', strtotime($third_f.' - 1 days'));
		...
		<th width="14%" bgcolor="#d6eaf8" style="text-align:center"><?php /*?><?=$first_f;?>  <?=$first_t;?><?php */?> 1-30</th>
		<th width="13%" bgcolor="#99FFFF" style="text-align:center"><?php /*?><?=$second_f;?>  <?=$second_t;?><?php */?> 31-60</th>
		<th width="13%" bgcolor="#ebdef0" style="text-align:center"><?php /*?><?=$third_f;?>  <?=$third_t;?><?php */?> 61-90</th>
		<th width="14%" bgcolor="#90EE90" style="text-align:center"><?php /*?><?=$last_t;?><?php */?>   91-Over</th>
		...
		if(isset($item_type)) 					{$item_type_con=' and i.item_type='.$item_type;}
 		 ...
 		 $sql = "select ledger_id, sum(dr_amt) as first_dr from journal where jv_date between '".$first_f."' and '".$first_t."' group by ledger_id";
		 $query = mysql_query($sql);
		 while($info=mysql_fetch_object($query)){
  		 $first_dr[$info->ledger_id]=$info->first_dr;
		}
		
		 $sql = "select ledger_id, sum(receipt_amt) as first_cr from receipt_from_customer where chalan_date between '".$first_f."' and '".$first_t."' group by ledger_id";
		 $query = mysql_query($sql);
		 while($info=mysql_fetch_object($query)){
		...
		$sql = "select ledger_id, sum(dr_amt) as second_dr from journal where jv_date between '".$second_f."' and '".$second_t."' group by ledger_id";
		 $query = mysql_query($sql);
		 while($info=mysql_fetch_object($query)){
  		 $second_dr[$info->ledger_id]=$info->second_dr;
		}
		
		$sql = "select ledger_id, sum(receipt_amt) as second_cr from receipt_from_customer where chalan_date between '".$second_f."' and '".$second_t."' group by ledger_id";
		 $query = mysql_query($sql);
		 while($info=mysql_fetch_object($query)){
  		 $second_cr[$info->ledger_id]=$info->second_cr;
		}
		
		
		
		$sql = "select ledger_id, sum(dr_amt) as third_dr from journal where jv_date between '".$third_f."' and '".$third_t."' group by ledger_id";
		 $query = mysql_query($sql);
		 while($info=mysql_fetch_object($query)){
  		 $third_dr[$info->ledger_id]=$info->third_dr;
		}
		
		$sql = "select ledger_id, sum(receipt_amt) as third_cr from receipt_from_customer where chalan_date between '".$third_f."' and '".$third_t."' group by ledger_id";
		 $query = mysql_query($sql);
		 while($info=mysql_fetch_object($query)){
  		 $third_cr[$info->ledger_id]=$info->third_cr;
		}
		
		
		 $sql = "select ledger_id, sum(dr_amt) as last_dr from journal where jv_date <='".$last_t."' group by ledger_id ";
		 $query = mysql_query($sql);
		 while($info=mysql_fetch_object($query)){
		 ...
		 $sql = "select ledger_id, sum(receipt_amt) as last_cr from receipt_from_customer where chalan_date <='".$last_t."' group by ledger_id";
		 $query = mysql_query($sql);
		 while($info=mysql_fetch_object($query)){
  		 $last_cr[$info->ledger_id]=$info->last_cr;
		}
...
//		}
		...
		<?
 		...
 		$sql="select dealer_code, account_code, dealer_name_e from dealer_info where 1 ".$con." order by dealer_name_e";
		...
		<td>
		<?=$row->dealer_name_e?></td>
		<td style=" font-size:14px; color:#000000;"><a href="master_report.php?report=230226002&f_date=<?=$first_f;?>&t_date=<?=$first_t;?>&ledger_id=<?=$row->account_code?>" target="_blank"><?=number_format($first_dr_amt=$first_dr[$row->account_code]-$first_cr[$row->account_code],2); $total_first_dr_amt +=$first_dr_amt;?></a></td>
		<td style=" font-size:14px; color:#000000;"><a href="master_report.php?report=230226002&f_date=<?=$second_f;?>&t_date=<?=$second_t;?>&ledger_id=<?=$row->account_code?>" target="_blank">
		<?=number_format($second_dr_amt=$second_dr[$row->account_code]-$second_cr[$row->account_code],2); $total_second_dr_amt +=$second_dr_amt;?></a></td>
		<td style=" font-size:14px; color:#000000;"><a href="master_report.php?report=230226002&f_date=<?=$third_f;?>&t_date=<?=$third_t;?>&ledger_id=<?=$row->account_code?>" target="_blank">
		<?=number_format($third_dr_amt=$third_dr[$row->account_code]-$third_cr[$row->account_code],2); $total_third_dr_amt +=$third_dr_amt;?></a></td>
		<td style=" font-size:14px; color:#000000;">
		<a href="master_report.php?report=230226002&f_date=<?=$last_f;?>&t_date=<?=$last_t;?>&ledger_id=<?=$row->account_code?>" target="_blank">
		<?=number_format($last_dr_amt=$last_dr[$row->account_code]-$last_cr[$row->account_code],2); $total_last_dr_amt +=$last_dr_amt;?></a></td>
		<td style=" font-size:14px; color:#000000;"><?=number_format($total_due_amt = ($first_dr_amt+$second_dr_amt+$third_dr_amt+$last_dr_amt),2); $grand_total_due +=$total_due_amt;?></td>
		</tr>
<?  }?>
		<tr <?=($xx%2==0)?' bgcolor="#F5F5F5"':'';$xx++;?>>
		  <td>&nbsp;</td>
...
elseif($_REQUEST['report']==230226002)
{
...
<!--		<img src="../../../logo/tecno.png" style="width:220px;">-->
		...
		<th width="26%" bgcolor="#FFFACD">CUSTOMER NAME</th>
		<th width="13%" bgcolor="#d6eaf8" style="text-align:center">Invoice No </th>
		<th width="13%" bgcolor="#99FFFF" style="text-align:center">Invoice Date </th>
		<th width="12%" bgcolor="#ebdef0" style="text-align:center">Invoice Amt </th>
		<th width="13%" bgcolor="#FAFAD2" style="text-align:center">Receive Amt </th>
		<th width="15%" bgcolor="#FFA07A" style="text-align:center">Due Amt </th>
		</tr>
		</thead><tbody>
		...
		<?
		 ...
		 $ledger_id_con=' and d.account_code="'.$_REQUEST['ledger_id'].'"';}
		
 		 $sql="SELECT d.account_code, d.dealer_name_e, j.tr_no, j.jv_date, sum(j.dr_amt) as invoice_amt, j.tr_id FROM journal j, dealer_info d
		 WHERE j.ledger_id=d.account_code and j.tr_from in ('Sales') and j.jv_date between '".$_REQUEST['f_date']."' and '".$_REQUEST['t_date']."' ".$ledger_id_con."  group by j.tr_no";
		...
		//if($sales_amt[$row->item_id]<>0 || $foc_amt[$row->item_id]<>0 ){}
		?>
		<? if ($due_amt=$row->invoice_amt-$receipt_amt[$row->account_code][$row->tr_no]+$return_amt[$row->account_code][$row->tr_no]>0) {?>
		<tr <?=($xx%2==0)?' bgcolor="#F5F5F5"':'';$xx++;?>>
		<td><?=$sl++;?></td>
		<td><?=$row->account_code?></td>
		<td>
		<?=$row->dealer_name_e?></td>
		<td><a href="../../../warehouse_mod/pages/wo/sales_invoice_print_view.php?v_no=<?=$row->tr_no?>" target="_blank"><span class="style13" ><?=$row->tr_no?></a></td>
		<td><?php echo date('d-M-Y',strtotime($row->jv_date));?></td>
		<td><?=number_format($row->invoice_amt,2);?></td>
		<td><?=number_format($receipt_amt[$row->account_code][$row->tr_no]+$return_amt[$row->account_code][$row->tr_no],2);?></td>
		<td><? echo number_format($due_amt=$row->invoice_amt-$receipt_amt[$row->account_code][$row->tr_no]+$return_amt[$row->account_code][$row->tr_no],2); $total_due_amt +=$due_amt;?></td>
		</tr>
<? } }?>
		<tr <?=($xx%2==0)?' bgcolor="#F5F5F5"':'';$xx++;?>>
		  <td>&nbsp;</td>
