...
$prold = find_all_field('purchase_receive','pr_no','id='.$jvold);
	...
	$narration = 'GR#'.$prold->id.'/'.$prold->pr_no.'(PO#'.$prold->pr_no.')(Bill#'.$bill_no.'/Dt:'.$bill_date.')';
...
<input type="button" name="Submit" value="EDIT VOUCHER"  onclick="DoNav('<?php echo '&v_no='.$jv_no.'&view=Show' ?>');" /></td>
    <td><? if($jv->checked!='YES'){?><input name="check" type="submit" id="check" value="CHECK" /><? }?>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#339900">
  <tr>
    <td width="1"><input name="button" type="button" onclick="hide();window.print();" value="Print" /></td>
    <td width="80" align="right"><a target="_blank" href="chalan_view2.php?v_no=<?=$pr->pr_no?>">GR-
        <?=$pr->pr_no?>
    </a></td>
...
<a target="_blank" href="chalan_view2.php?v_no=<?=$pr->pr_no?>"></a></div><? }?>
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#000000" class="tabledesign">
	  ...
	  <?
$sql2="SELECT a.ledger_name,b.* FROM accounts_ledger a, secondary_journal b where b.jv_no='$jv_no' and a.ledger_id=b.ledger_id and jv_no=$jv_no and dr_amt>0 order by jv_no desc";
