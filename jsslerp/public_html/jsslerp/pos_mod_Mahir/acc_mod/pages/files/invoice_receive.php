...
<?php js_ledger_subledger_autocomplete_new('receipt',$proj_id,$voucher_type,$_SESSION['user']['group']); 
    $led2=mysql_query("select  concat(ledger_name,'##>>',ledger_id), concat(ledger_name,'##>>',ledger_id) FROM accounts_ledger WHERE ledger_name not like '%cash%' and ledger_group_id='$cash_and_bank_balance' and parent=0 order by ledger_id");
