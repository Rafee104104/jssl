...
<?php
 ...
 $led=mysql_query("select ledger_id,ledger_name from accounts_ledger where group_for=".$_SESSION['user']['group']." order by ledger_name");
      $data = '[';
      $data .= '{ name: "All", id: "%" },';
...
<?php
	if(isset($_REQUEST['show']))
    {
		...
		if($cc_code > 0)
		{
			$total = mysql_fetch_row(mysql_query("select sum(dr_amt),sum(cr_amt) from journal where tr_from='Receipt' and jv_date>'$fdate' and jv_date<'$tdate' and ledger_id like '$ledger_id' and group_for=".$_SESSION['user']['group']." AND cc_code=$cc_code"));
		...
		{
			$total = mysql_fetch_row(mysql_query("select sum(dr_amt),sum(cr_amt) from journal where tr_from='Receipt' and jv_date>'$fdate' and jv_date<'$tdate' and ledger_id like '$ledger_id' and group_for=".$_SESSION['user']['group'].""));
