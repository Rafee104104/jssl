...
echo '<option value="'.$cc->id.'">'.$cc->id.' - '.$cc->center_name.'</option>';
    ...
    <th width="16%" align="center">Opening</th>
...
$depot_con=" and (a.warehouse_id='' or a.warehouse_id=".$_POST['dealer_depot'].") ";
  ...
  $g="select c.group_name,c.group_id FROM ledger_group c where 1 ".$grp_con." and  c.group_id not in (3001,1087,1088,1089,2062,2064,4001,4006,4029,4038,2050) and c.group_for=".$_SESSION['user']['group']."  group by  c.group_id";
  $gsql=mysql_query($g);
...
<?php
	$cc_code = (int) $_REQUEST['cc_code'];
		...
		$p="select a.ledger_name,a.ledger_id from accounts_ledger a where  a.parent=0 and a.ledger_group_id='$g[1]' ".$depot_con." order by a.ledger_id";
...
{ $tagc='(Cr)'; $totalclosing_total=$totalclosing_total*(-1);}
  ...
  ?>
  <tr>
    <th colspan="2" align="left">Sales Accounts </th>
    <th align="right">&nbsp;</th>
    <th align="right">&nbsp;</th>
    <th align="right">&nbsp;</th>
    <th align="right">&nbsp;</th>
  </tr>
  <? if($_POST['dealer_depot']==1){?>
  <tr <? $i++; if($i%2==0)$cls=' class="alt"'; else $cls=''; echo $cls;?>>
    <td align="center">1</td>
    <td align="left"><a href="transaction_listledger.php?show=show&amp;fdate=<?=$_REQUEST['fdate']?>&amp;tdate=<?=$_REQUEST['tdate']?>&amp;ledger_id=<?=$p[1]?>" target="_blank">Sales Accounts - Sylhet</a></td>
    <td align="right"><?= number_format($sales_sylhet_opn= find_a_field('journal j, accounts_ledger a','sum(dr_amt)','j.jv_date < "'.$fdate.'" and j.tr_from="Sales" and j.dr_amt>0 and a.ledger_id=j.ledger_id and a.warehouse_id=1'),2);?></td>
    <td align="right"></td>
    <td align="right"><?= number_format($sales_sylhet= find_a_field('journal j, accounts_ledger a','sum(dr_amt)','j.jv_date between "'.$fdate.'" and "'.$tdate.'" and j.tr_from="Sales" and j.dr_amt>0 and a.ledger_id=j.ledger_id and a.warehouse_id=1'),2); $total_sales_sylhet+=$sales_sylhet;?></td>
    <td align="right"><?= number_format($sales_closing_sylhet=($sales_sylhet_opn+$sales_sylhet),2);?></td>
  </tr>
  <? } elseif($_POST['dealer_depot']==3){?>
  <tr <? $i++; if($i%2==0)$cls=' class="alt"'; else $cls=''; echo $cls;?>>
    <td align="center">2</td>
    <td align="left"><a href="transaction_listledger.php?show=show&amp;fdate=<?=$_REQUEST['fdate']?>&amp;tdate=<?=$_REQUEST['tdate']?>&amp;ledger_id=<?=$p[1]?>" target="_blank">Sales Accounts - Dhaka </a></td>
    <td align="right"><?= number_format($sales_dhaka_opn= find_a_field('journal j, accounts_ledger a','sum(dr_amt)','j.jv_date < "'.$fdate.'" and j.tr_from="Sales" and j.dr_amt>0 and a.ledger_id=j.ledger_id and a.warehouse_id=3'),2);?></td>
    <td align="right">&nbsp;</td>
    <td align="right"><?= number_format($sales_dhaka= find_a_field('journal j, accounts_ledger a','sum(dr_amt)','j.jv_date between "'.$fdate.'" and "'.$tdate.'" and j.tr_from="Sales" and j.dr_amt>0 and a.ledger_id=j.ledger_id and a.warehouse_id=3'),2); $total_sales_dhaka+=$sales_dhaka;?></td>
    <td align="right"><?= number_format($sales_closing_dhaka=($sales_dhaka_opn+$sales_dhaka),2);?></td>
  </tr>
  <? } elseif($_POST['dealer_depot']==2){?>
  <tr <? $i++; if($i%2==0)$cls=' class="alt"'; else $cls=''; echo $cls;?>>
    <td align="center">3</td>
    <td align="left"><a href="transaction_listledger.php?show=show&amp;fdate=<?=$_REQUEST['fdate']?>&amp;tdate=<?=$_REQUEST['tdate']?>&amp;ledger_id=<?=$p[1]?>" target="_blank">Sales Accounts - Chittagong </a></td>
    <td align="right"><?= number_format($sales_ctg_opn= find_a_field('journal j, accounts_ledger a','sum(dr_amt)','j.jv_date < "'.$fdate.'" and j.tr_from="Sales" and j.dr_amt>0 and a.ledger_id=j.ledger_id and a.warehouse_id=2'),2);?></td>
    <td align="right">&nbsp;</td>
    <td align="right"><?= number_format($sales_ctg= find_a_field('journal j, accounts_ledger a','sum(dr_amt)','j.jv_date between "'.$fdate.'" and "'.$tdate.'" and j.tr_from="Sales" and j.dr_amt>0 and a.ledger_id=j.ledger_id and a.warehouse_id=2'),2); $total_sales_ctg+=$sales_ctg;?></td>
    <td align="right"><?= number_format($sales_closing_ctg=($sales_ctg_opn+$sales_ctg),2);?></td>
  </tr>
  <? } else{?>
  <tr bgcolor="#99CCFF">
    <td align="center" bgcolor="#FFFFFF">&nbsp;</td>
    <td align="center" bgcolor="#FFFFFF"><div align="left">Sales Accounts National </div></td>
  ...
  $grand_tot_sales = ($total_sales_sylhet+$total_sales_dhaka+$total_sales_ctg);
  
  $grand_tot_ope = ($sales_sylhet_opn+$sales_dhaka_opn+$sales_ctg_opn);
  
  $grand_tot_closing = ($sales_closing_sylhet+$sales_closing_dhaka+$sales_closing_ctg);
 
 $totalclosing_total+=$grand_tot_closing;
 $total_cr+=$grand_tot_sales;
 $totalopening_total+=$grand_tot_ope;
  ...
  <tr bgcolor="#99CCFF">
    <td colspan="2" align="center"><strong>Total</strong></td>
    <td align="right"><?=number_format($grand_tot_ope,2);?></td>
    <td align="right">&nbsp;</td>
    <td align="right"><?=number_format($grand_tot_sales,2);?></td>
    <td align="right"><?=number_format($grand_tot_closing,2);?></td>
  </tr>
  <tr>
