...
$g="select a.ledger_id,c.group_id,SUM(dr_amt) dr_amt,SUM(cr_amt) cr_amt
  ...
  $g="select c.group_name,c.group_id FROM ledger_group c where 1 ".$grp_con." and  c.group_id in (10,1016,1024,1032,1051,1086,1092,1109,2067) and c.group_for=".$_SESSION['user']['group']."  group by  c.group_id";
  $gsql=mysql_query($g);
...
{ $tagc='(Cr)'; $tclosing_total=$tclosing_total*(-1);}
  ...
  $tclosing_total = 0;
   ...
   $g="select c.group_name,c.group_id FROM ledger_group c where 1 ".$grp_con." and  c.group_id in (1090,1078,3001,3024,4029,4064,4065,4066,1061,1112) and c.group_for=".$_SESSION['user']['group']."  group by  c.group_id";
  $gsql=mysql_query($g);
  while($g=mysql_fetch_row($gsql))
  {
