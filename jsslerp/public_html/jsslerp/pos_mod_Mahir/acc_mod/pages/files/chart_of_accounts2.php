...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	if($_REQUEST['ladger_group']!="" && $_REQUEST['group_class']==""){$con.= " and group_name LIKE '%". mysql_real_escape_string($_REQUEST['ladger_group'] )."%'";}
	
	if($_REQUEST['group_class']!="" && $_REQUEST['ladger_group']==""){$con.= " and group_class LIKE '%". mysql_real_escape_string($_REQUEST['group_class'] )."%'";}
	
	
	
	if($_REQUEST['group_class']!="" && $_REQUEST['ladger_group']!="" ){$con.= " and group_name LIKE '%". mysql_real_escape_string($_REQUEST['ladger_group'] )."%' and group_class LIKE '%". mysql_real_escape_string($_REQUEST['group_class'] )."%'";}
...
//								}?></td>
                  ...
                  <?
							  if($_REQUEST['ledger_name']!="" ){$l_con.= " and ledger_name LIKE '%". mysql_real_escape_string($_REQUEST['ledger_name'] )."%'";}
							  ...
							  while($r = mysql_fetch_object($acc_query)){						  ?>
                  <tr>
                    <td></td>
                  ...
                  <? 
							  ...
							  while($s = mysql_fetch_object($sub_query)){?>
                  <tr>
                    <td></td>
                    <td></td>
                  ...
                  <?  $sub_sub_sql = 'select * from sub_sub_ledger where sub_ledger_id='.$s->sub_ledger_id.' order by sub_sub_ledger asc';
							  $sub_sub_query = mysql_query($sub_sub_sql);	
							  while($ss = mysql_fetch_object($sub_sub_query)){?>
                  <tr>
                    <td></td>
                    <td></td>
                    <td></td>
