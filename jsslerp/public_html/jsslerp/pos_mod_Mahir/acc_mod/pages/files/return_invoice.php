...
if($_POST['invoice_varify'])
{
	...
	for($i=0;$i<($c/7);$i++)
	{
		...
		$lid = mysql_fetch_row(mysql_query("select group_id from item_info where item_id='$item'"));
...
<?php
    ...
    $led11=mysql_query("select id, center_name FROM cost_center WHERE 1 ORDER BY center_name ASC");
    ...
    $led2=mysql_query("select item_id, item_name from item_info order by item_name");
      $data2 = '[';
...
<input name="add" type="button" class="btn1" id="add" value="ADD" onclick="checkhead('purchase_invoice_item');" tabindex="8"/>              </td>
            ...
            <tr>
              <td align="center"><input type="text" name="refno" id="refno" readonly="readonly" /></td>
                ...
                $a=mysql_fetch_row(mysql_query($a2));
