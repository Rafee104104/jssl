...
?>
<?php $led=mysql_query("SELECT DISTINCT
...
if(isset($_REQUEST['show']))
{
  ...
  //echo $filterQryb;
	$cusQry = mysql_query($filterQryb);
		
    while($cus = mysql_fetch_row($cusQry))
    {		  
		$sQuery = 'SELECT 
					  issue_invoice.item,
	  ...
	  {
		  while($data = mysql_fetch_assoc($p))
		  {
		  $pi++;
		  ?>
