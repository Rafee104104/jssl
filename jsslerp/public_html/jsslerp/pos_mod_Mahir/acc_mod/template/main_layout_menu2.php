...
$user_id=$_SESSION['user']['id'];
$module_sql='select m.module_name,m.id,m.module_link,d.module_id,d.user_id from user_module_define d,user_module_manage m where d.module_id=m.id and d.user_id="'.$user_id.'" and m.id!=12 and m.id!='.$mod_id.' limit 7';
$module_query=mysql_query($module_sql);

while($module_data=mysql_fetch_object($module_query))
{
    ?>
                                 <li class="li1 mt-1"><a href="../../../<?=$module_data->module_link;?>?mod_id=<?=$module_data->module_id;?>"><i class="fas fa-calculator"></i></a><ul class="ul2"><li class="li2"><?=$module_data->module_name; ?></li></ul></li>
    <?
}
?>
    ...
    <div class="dashboard1-nav-dropdown"><a href="#" class="dashboard1-nav-item "><i class="fas fa-tachometer-alt"></i> Dashboard </a></div>
<?
