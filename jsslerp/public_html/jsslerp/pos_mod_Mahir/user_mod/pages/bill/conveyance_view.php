...
$sql='UPDATE bills SET cancel_reason="'.mysql_real_escape_string($_POST['cancelreason']).'",status="Pending" WHERE conveyance_no="'.$_GET['con_id'].'"';
$query=mysql_query($sql);		
...
<?  if (isset($_POST['submit'])) {
  ...
  for ($i = 0; $i < count($_POST['amount']); $i++) {
	...
	$remarks = mysql_real_escape_string($_POST['hr_remarks'][$i]);
