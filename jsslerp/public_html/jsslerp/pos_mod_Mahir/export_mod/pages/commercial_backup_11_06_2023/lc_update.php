								...
								<input name="lc_no_view" type="text" id="lc_no_view" style="width:220px; height:32px;" value="<?=$lc_all->lc_no_view?>" readonly="" tabindex="105" />
								 ...
								 <td> Date:</td>
									...
									<select name="dealer_code" id="dealer_code" required style="width:220px;">
										...
										<? foreign_relation('dealer_info_foreign','dealer_code','dealer_name_e',$dealer_code,'dealer_code="'.$lc_all->dealer_code.'"  order by dealer_code');?>
									</select>
									</span>										</td>
								  </tr>
								  <tr>
...
//		$do_con .= ' and m.do_no in ('.$_POST['do_no'].') ';
		 ...
		 $sql = "select   p.pi_id, sum(w.total_unit) as pi_qty, sum(w.total_amt) as pi_amt from pi_master m, pi_details p, sale_do_details_foreign w where m.id=p.pi_id and  m.dealer_code='".$lc_all->dealer_code."' and  p.do_no=w.do_no group by p.pi_id";
		 $query = mysql_query($sql);
		 while($info=mysql_fetch_object($query)){
   ...
   where m.id=d.pi_id and m.dealer_code='".$lc_all->dealer_code."'  group by d.pi_id  order by  d.pi_id ";
