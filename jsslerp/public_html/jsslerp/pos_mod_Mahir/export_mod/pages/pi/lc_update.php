...
<table width="80%" border="0" align="center">
	...
	<? $pi_data = find_all_field('pi_master','','id='.$_POST['pi_id']);?>		</td>
      ...
      <tr>
        ...
        <? foreign_relation('dealer_info_foreign','dealer_code','dealer_name_e',$_POST['dealer_pi'],'dealer_code="'.$pi_data->dealer_code.'"');?>
    </select></td>
...
//		$do_con .= ' and m.do_no in ('.$_POST['do_no'].') ';
		 ...
		 $sql = "select   c.do_no, c.do_no as find_do  from sale_do_master_foreign m, pi_details c where m.do_no=c.do_no and c.pi_type in (2,3) group by c.do_no ";
  ...
  echo  $sql = "select  m.*,  sum(c.total_unit) as wo_qty, sum(c.total_amt) as wo_amt
   from sale_do_master_foreign m, sale_do_details_foreign c, dealer_info_foreign d
   where m.do_no=c.do_no and d.dealer_group='".$pi_data->dealer_group."' and m.dealer_code=d.dealer_code  and m.status in ('COMPLETED','CHECKED') ".$date_con.$dealer_con.$do_con." group by c.do_no  order by  m.job_no ";
