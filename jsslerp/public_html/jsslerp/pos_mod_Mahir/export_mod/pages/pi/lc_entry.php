...
while($drow=mysql_fetch_object($d_query)){
?>
  <option value="<?php echo $drow->dealer_group."#>".$drow->id;?>">
 <?php } ?>
</datalist>-->
									<input type="text" list="bnm" name="dealer_group" id="dealer_group" required style="width:220px;" onchange="getData2('find_dealer_ajax.php', 'find_dealer', this.value,document.getElementById('dealer_group').value);" >
									
									<datalist id="bnm">
	
										<option></option>
								
										<? foreign_relation('dealer_group','concat(id,"#",dealer_group)','concat(id,"#",dealer_group)',$dealer_group,'1 order by id');?>
									</datalist>										</td>
