...
getData2('do_ajax.php', 'do',document.getElementById("item").value,'<?=$do_type;?>');
...
<? if($transport_type<1) {?>
<div>
<label style="width:140px;">Transport Type: </label>

<select name="transport_type" id="transport_type" required style="width:220px; height:22px;" class="onhover" tabindex="0" onchange="getData2('transport_type_ajax.php', 'transport', this.value, document.getElementById('transport_type').value);">
               	    
               	    <? foreign_relation('transport_type', 'id', 'transport_type',$_POST['transport_type'],' 1
					order by id ');?>
           	      </select>
</div>
<? }?>
...
<input name="transport_type2" type="text" id="transport_type2" required readonly="" style="width:220px; " 
value="<?=find_a_field('transport_type','transport_type','id="'.$transport_type.'"');?>" tabindex="105" />

<input name="transport_type" type="hidden" id="transport_type" required readonly="" style="width:220px;" value="<?=$transport_type?>" tabindex="105" />
...
<label style="width:140px;">Transport Rate: </label>
<input name="transport_rate" type="text" id="transport_rate" required readonly="" style="width:220px;" value="<?=$transport_rate?>" tabindex="105" />
...
<td colspan="3">
		...
		<input name="new" type="submit" class="btn1" value="Update Sales Order" style="width:250px; font-weight:bold; font-size:12px;" tabindex="12" />
		...
		<input name="new" type="submit" class="btn1" value="Initiate Sales Order" style="width:250px; font-weight:bold; font-size:12px;" tabindex="12" />
...
<td width="15%" align="center" bgcolor="#0073AA"><span class="style2">Warehouse</span></td>
<td width="15%" align="center" bgcolor="#0073AA"><span class="style2">Stock</span></td>
...
<td width="14%" rowspan="2" align="center" bgcolor="#0073AA"><span class="style2"> Price </span></td>

<td colspan="2" align="center" bgcolor="#0073AA"><span class="style2">Order Qty</span></td>

<td width="22%" rowspan="2" align="center" bgcolor="#0073AA"><span class="style2">Reguler Amt </span></td>
</tr>

<tr>

<td width="15%" align="center" bgcolor="#0073AA"><span class="style2"> Unit </span></td>

<td width="14%" align="center" bgcolor="#0073AA"><span class="style2">Bag</span></td>
</tr>
...
<input type="text" id="itemin" name="itemin"  style="width:350px; height:30px;"  required onblur="grp_check(this.value)" />
...
<input type="hidden" id="via_customer" name="via_customer" value="<?=$via_customer?>"  />
</span></td>

<td align="center" bgcolor="#CCCCCC">
	<select name="depot_id" id="depot_id"  class="onhover" required style="width:250px; height:33px;" 
	onblur="getData2('do_stock_ajax.php', 'st', this.value, document.getElementById('itemin').value);"
>
               	    <option></option>
               	    <? foreign_relation('warehouse', 'warehouse_id', 'warehouse_name',$_POST['depot_id'],' 1 and use_type != "PL"');?>
   	      </select></td>
<td align="center" bgcolor="#CCCCCC"><span id="st">
<input name="stock" type="text" class="input3" id="stock"  style="width:70px; height:30px;"  onkeyup="count()" value="<?=$stock?>" readonly required/>
</span></td>
<td bgcolor="#CCCCCC">
...
<td><input name="return_price" type="text" class="input3" id="return_price"  style="width:70px; height:30px;"  onkeyup="count()" value="<?=$return_price?>" readonly required/>
<input name="item_id" type="hidden" class="input3" id="item_id"  style="width:80px; height:30px;"  value="<?=$item_all->item_id?>" readonly/></td>
</tr>
...
<td><input placeholder="Unit"  name="dist_unit" type="text" class="input3" id="dist_unit" style="width:70px; height:30px;" onKeyUp="count()" onBlur="count()" required="required"  tabindex="0"/></td>

<td><input placeholder="Bag"  name="bag_unit" type="text" class="input3" id="bag_unit" style="width:70px; height:30px;" onKeyUp="count()" onBlur="count()"   tabindex="0"/></td>

<td>

<input  name="pkt_unit" type="hidden" class="input3" id="pkt_unit" style="width:80px; height:30px;" value="0" /></td>

<td><input name="total_unit" type="hidden" class="input3" id="total_unit"  style="width:80px; height:30px;"  onKeyUp="count()"  readonly/>

<input placeholder="Amount" name="total_amt" type="text" class="input3" id="total_amt" style="width:100px; height:30px;" onKeyUp="count()" readonly/></td>
<td>
<input placeholder="Bag"  name="init_bag_unit" type="hidden" class="onhoverfocus" id="init_bag_unit" style="width:77px; height:30px;" onKeyUp="count()" onBlur="count()"   tabindex="0"/>

<input placeholder="Unit"  name="init_dist_unit" type="hidden" class="input3" id="init_dist_unit" style="width:77px; height:30px;" onKeyUp="count()" onBlur="count()" required="required" readonly=""  tabindex="0"/>
	<input placeholder="Unit"  name="init_total_unit" type="hidden" class="input3" id="init_total_unit" style="width:77px; height:30px;" onKeyUp="count()" onBlur="count()" required="required"  tabindex="0"/>
	<input placeholder="Amount" name="init_total_amt" type="hidden" class="input3" id="init_total_amt" style="width:120px; height:30px;" onKeyUp="count()" readonly/></td>
</tr>
...
//, (a.init_pkt_unit*a.unit_price) as Total,(a.init_pkt_unit-a.inStock_ctn) as Shortage
  ...
  $res='select a.id, a.item_id, s.sub_group_name, b.item_name, b.unit_name, a.unit_price,a.dist_unit, a.bag_unit,a.total_amt, a.depot_id from sale_do_details a,item_info b, item_sub_group s where b.item_id=a.item_id  and b.sub_group_id=s.sub_group_id and a.do_no='.$$unique_master.' order by a.id';
...
<th width="30%" rowspan="2">Product Name</th>

<th width="23%" rowspan="2">Warehouse</th>
<th width="6%" rowspan="2">Unit</th>
<th width="6%" rowspan="2">Bag</th>
<th width="11%" rowspan="2">Quantity</th>
<th colspan="2"><div align="center">Sales Amt </div></th>

<th width="5%" rowspan="2">X</th>
</tr>
<tr>
  <th width="9%">Price</th>
...
<td><? if(substr($data->item_id,0,1)!=9) { echo $data->bag_unit; $tot_bag += $data->bag_unit;} ?></td>
<td><? if(substr($data->item_id,0,1)!=9) { echo $data->dist_unit; $tot_dist_unit += $data->dist_unit;} ?></td>
...
$total_return_amt = $total_return_amt + $data->return_amt;
...
<td><?=$tot_bag?></td>
<td><?=number_format($tot_dist_unit,2);?></td>
<td>&nbsp;</td>
...
<? if ($cash_discount>0) { ?>
<tr>
<td colspan="4"><div align="right"><strong>Cash Discount:</strong></div></td>
<td>&nbsp;</td>
<td>&nbsp;</td>
<td>&nbsp;</td>
<td><strong><?=number_format($cash_discount,2);?></strong></td>
<td>&nbsp;</td>
</tr>
<? }?>

<tr>
<td colspan="4"><div align="right"><strong>Receivable Amount:</strong></div></td>
<td>&nbsp;</td>
<td>&nbsp;</td>
<td>&nbsp;</td>
<td><strong><?=number_format($total_amt-$cash_discount,2);?></strong></td>
<td>&nbsp;</td>
</tr>
...
<input name="delete"  type="submit" class="btn1" value="DELETE SO" style="width:100px; font-weight:bold; font-size:12px;color:#F00; height:30px" />
...
<input name="confirm" type="submit" class="btn1" value="CONFIRM THIS SO" style="width:270px; font-weight:bold; font-size:12px; height:30px; color:#090; float:right" />
