...
//		}
		...
		$chalan_no = next_transection_no($group_for,$chalan_date,'sale_do_chalan','chalan_no');
  ...
  $so_invoice = "INSERT INTO `sale_do_chalan` (`chalan_no`, `chalan_date`, `do_no`, `do_date`, `delivery_date`, `cbm_no`, `group_for`, `dealer_code`, `buyer_code`, `merchandizer_code`, `delivery_place`, `po_no`, `order_no`, `reference_no`, `ply`, `paper_combination`, `L_cm`, `W_cm`, `H_cm`, `WL`, `WW`, `item_id`, `sqm_rate`, `unit_price`, `total_unit`, `total_amt`, `depot_id`, `status`,`entry_by`, `entry_at`, remarks)
  VALUES('".$chalan_no."', '".$chalan_date."' ,'".$data->do_no."', '".$data->do_date."',  '".$data->delivery_date."',    '".$data->cbm_no."', '".$data->group_for."' , 
  '".$data->dealer_code."',  '".$data->buyer_code."',  '".$data->merchandizer_code."', '".$data->delivery_place."', '".$data->po_no."', '".$data->id."',  
   '".$data->reference_no."', '".$data->ply."',  '".$data->paper_combination."', '".$data->L_cm."','".$data->W_cm."','".$data->H_cm."','".$data->WL."',  '".$data->WW."', 
   '".$item_id."','".$data->sqm_rate."','".$rate."','".$qty."','".$amount."','".$data->depot_id."', 'CHECKED','".$entry_by."', '".$entry_at."', '".$remarks."'   )";
  ...
  <tr>
    ...
    <? $field='do_date';?>
      ...
      <div>
        ...
        <label style="width:140px;" for="<?=$field?>">Delivery Date:</label>
...
<?php /*?>	<? if($ex<time()){?>
	...
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
      ...
      <tr>
        ...
        <td width="21%" align="right" bgcolor="#9999FF"><strong> Challan Date:</strong></td>
...
<? if($$unique>0){
  ...
  $sql='select a.id,  a.item_id,  a.unit_price,  b.item_name,  b.unit_name, a.ply, a.paper_combination, a.L_cm, a.W_cm, a.H_cm,  a.total_unit as qty from sale_do_details a,item_info b where b.item_id=a.item_id and  a.do_no='.$$unique;
    ...
    <tr>
      ...
      <tbody>
          ...
          <tr>
            ...
            <th bgcolor="#009900">TC Qty  </th>

            <th bgcolor="#009900">Deli Qty </th>
            <th bgcolor="#FFFF00">Pending </th>

            <th bgcolor="#0099CC">Challan</th>
          </tr>
          ...
          <tr bgcolor="<?=(($bg%2)==1)?'#FFEAFF':'#DDFFF9'?>">
			...
			<input type="hidden" name="rate_<?=$row->id?>" id="rate_<?=$row->id?>" value="<?=$row->unit_price?>" />	</td>
              ...
              <td width="2%" align="center"><?=$row->unit_name?>

                <input type="hidden" name="unit_name_<?=$row->id?>" id="unit_name_<?=$row->id?>" value="<?=$row->unit_name?>" /></td>

              <td width="2%" align="center"><?=$row->ply?></td>
              <td width="24%" align="center"><?=$row->paper_combination?></td>
              <td width="13%" align="center"><? if($row->L_cm>0) {?><?=$row->L_cm?><? }?><? if($row->W_cm>0) {?>X<?=$row->W_cm?><? }?><? if($row->H_cm>0) {?>X<?=$row->H_cm?><? }?> cm</td>
              <td width="8%" align="center"><?=number_format($row->qty,2);?></td>

              <td width="9%" align="center"><? echo number_format($so_qty = (find_a_field('sale_do_travel_card','sum(total_unit)','order_no="'.$row->id.'" and item_id="'.$row->item_id.'"')*(1)),2);?></td>

              <td width="10%" align="center"><? echo number_format($challan_qty = (find_a_field('sale_do_chalan','sum(total_unit)','order_no="'.$row->id.'" and item_id="'.$row->item_id.'"')*(1)),2);?></td>
              <td width="4%" align="center"><? echo number_format($unso_qty=($so_qty-$challan_qty),2);?>

                <input type="hidden" name="unso_qty_<?=$row->id?>" id="unso_qty_<?=$row->id?>" value="<?=$unso_qty?>"  onKeyUp="calculation(<?=$row->id?>)" />
				
				<?  number_format($unwo_qty=($row->qty-$challan_qty),2);?>
				</td>
