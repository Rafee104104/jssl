...
//		}
		...
		$tc_no = next_transection_no($group_for,$tc_date,'sale_do_travel_card','tc_no');
  ...
  $so_invoice = "INSERT INTO `sale_do_travel_card` (`tc_no`, `tc_date`, `do_no`, `do_date`, `delivery_date`, `cbm_no`, `group_for`, `dealer_code`, `buyer_code`, `merchandizer_code`, `delivery_place`, `po_no`, `order_no`, `reference_no`, `ply`, `paper_combination`, `L_cm`, `W_cm`, `H_cm`, `WL`, `WW`, `item_id`, `sqm_rate`, `unit_price`, `total_unit`, `total_amt`, `depot_id`, `status`,`entry_by`, `entry_at`, remarks)
  VALUES('".$tc_no."', '".$tc_date."' ,'".$data->do_no."', '".$data->do_date."',  '".$data->delivery_date."',    '".$data->cbm_no."', '".$data->group_for."' , 
  '".$data->dealer_code."',  '".$data->buyer_code."',  '".$data->merchandizer_code."', '".$data->delivery_place."', '".$data->po_no."', '".$data->id."',  
   ...
   '".$item_id."','".$data->sqm_rate."','".$rate."','".$qty."','".$amount."','".$data->depot_id."', 'PROCESSING','".$entry_by."', '".$entry_at."', '".$remarks."'   )";
      ...
      <tbody>
          ...
          <tr bgcolor="<?=(($bg%2)==1)?'#FFEAFF':'#DDFFF9'?>">
			...
			<input type="hidden" name="rate_<?=$row->id?>" id="rate_<?=$row->id?>" value="<?=$row->unit_price?>" />	</td>
              ...
              <td width="15%" align="center"><? if($row->L_cm>0) {?><?=$row->L_cm?><? }?><? if($row->W_cm>0) {?>X<?=$row->W_cm?><? }?><? if($row->H_cm>0) {?>X<?=$row->H_cm?><? }?> cm</td>
...
<td align="center"><input name="confirm" type="submit" class="btn1" value="CONFIRM TRAVEL CARD" style="width:270px; font-weight:bold; float:right; font-size:12px; height:30px; color:#090" /></td>
