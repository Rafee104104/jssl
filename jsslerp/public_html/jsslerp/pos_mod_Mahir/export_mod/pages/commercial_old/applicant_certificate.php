...
$lc_data = find_all_field('lc_master','','lc_no='.$lc_no); 
 ...
 $buyer_all=find_all_field('dealer_info_foreign','','dealer_code='.$lc_data->dealer_code); 
 ...
 $dealer = find_all_field('dealer_info_foreign','','dealer_code='.$lc_data->dealer_code); 
 ...
 <html>
	...
	<td>
		...
		<table style="width:100%;">
			...
			<td style="text-align:left;"><?=find_a_field('dealer_info_foreign','dealer_name_e','dealer_code='.$lc_data->dealer_code);?></td>
			<td style="text-align:right;"><?php echo date('d-m-Y',strtotime($lc_data->lc_date));?></td>
			</tr>
			<tr>
			<td style="text-align:left;"> </td>
			<td style="text-align:left;"><?= find_a_field('dealer_info_foreign','address_e','dealer_code='.$lc_data->dealer_code);?></td>
...
$a=0;
		 ...
		 WHERE a.lc_no=b.lc_no  and b.pi_id=c.pi_id and a.lc_no="'.$lc_no.'" GROUP by c.pi_id ';
			...
			$a++;
			if ($a>1) echo ', ';
echo $pi_data->pi_no.'   Date. '.date('d-m-Y',strtotime($pi_data->pi_date));}?></td>
	  ...
	  <?php } ?>
		...
		<table style="width:100%;">
			...
			$desc_sql = 'SELECT  s.item_id FROM lc_receive l, pi_details p, sale_do_details_foreign s 
		 WHERE l.pi_id=p.pi_id  and p.do_no=s.do_no and l.lc_no="'.$lc_data->lc_no.'" ';
$des_query=mysql_query($desc_sql);
while($row2=mysql_fetch_object($des_query)){
