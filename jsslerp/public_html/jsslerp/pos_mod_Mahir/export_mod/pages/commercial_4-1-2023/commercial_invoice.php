...
<table width="900" border="0" cellspacing="0" cellpadding="0" align="center">
 ...
 <tr> <td colspan="2"><table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0"  style="font-size:14px">
  	...
  	<tr>
  	  <td valign="top">ACCOUNT/ CONSIGNEE:</td>
  	  <td valign="middle" align="center">&nbsp;</td>
  	  <td valign="right" align="right">&nbsp;</td>
	  </tr>
  	<tr>
	...
	<tr>
	  <td colspan="3" valign="top" style="font-size:14px; padding: 5px 0px 0px 0px; letter-spacing: .3px; line-height:20px; text-transform:uppercase;" ><div align="justify">
		 ...
		 <?=$dealer->dealer_name_e?>. <?=$dealer->address_e?>. <br />
			<b>ACCESSORIES FOR 100 % READYMADE GARMENTS INDUSTRY.</b> <br />
			QUALITY, QUANTITY, RATE AND ALL OTHER DETAILS Of THE GOODS ARE IN ACCORDANCE WITH THE BENEFICIARY'S PROFORMA INVOICE NO. 
    ...
    <td colspan="2">
      ...
      <table width="100%" class="tabledesign"   border="1" bordercolor="#CCCCCC" cellspacing="0" cellpadding="5"  style="font-size:12px">
        ...
        <tr style="font-size:12px;">
          ...
          <td align="left" valign="middle">Exp. No: <?= $lc_data->exp_no; ?><br /> Exp. Date: <?php echo date('d-m-Y',strtotime($lc_data->exp_date));?></td>
...
SAY TOTAL <strong>U.S DOLLAR
	 ...
	 echo ' Only';
		...
		?>.
		 ...
		 </strong> DRAWN UNDER <?=$buyer_bank->bank_name;?>, <?=$buyer_bank->branch_name;?>. <br />
...
$a=0;
		 ...
		 WHERE a.lc_no=b.lc_no  and b.pi_id=c.pi_id and a.lc_no="'.$lc_no.'" GROUP by c.pi_id ';
			...
			$a++;
			if ($a>1) echo ', ';
echo $pi_data->pi_no.' DT. '.date('d-m-Y',strtotime($pi_data->pi_date));}?>. <? if($lc_data->importer_irc_no!="") {?> Importer IRC No: <?= $lc_data->importer_irc_no; ?><? }?><? if($lc_data->applicants_tin_no!="") {?>, Applicant's TIN No: <?= $lc_data->applicants_tin_no; ?><? }?><? if($lc_data->applicants_bin_no!="") {?>, Applicant's BIN No: <?= $lc_data->applicants_bin_no; ?><? }?><? if($lc_data->bangladesh_bank_dc_no!="") {?>, Bangladesh Bank DC No: <?= $lc_data->bangladesh_bank_dc_no; ?><? }?><? if($lc_data->issuing_bank_bin_no!="") {?>, Issuing Bank BIN No: <?= $lc_data->issuing_bank_bin_no; ?>.<? }?> BENEFICIARY'S BIN NO. 000073153-0403, H. S. CODE. 4819.10.00.</b> <br />
...
THE IMPORT IS BEING MADE UNDER BACK TO BACK SYSTEM AGAINST EXPORT CONTRACT AND SHIPMENT MADE FROM <?=find_a_field('user_group','group_name','id='.$_SESSION['user']['group'])?> TO <?=$dealer->dealer_name_e?>. <?=$dealer->address_e?>. 
