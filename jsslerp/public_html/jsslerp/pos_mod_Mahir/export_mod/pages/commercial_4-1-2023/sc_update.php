...
//		$do_con .= ' and m.do_no in ('.$_POST['do_no'].') ';
		 ...
		 $sql = "select   p.pi_id, sum(w.total_unit) as pi_qty, sum(w.total_amt) as pi_amt from pi_master m, pi_details p, sale_do_details w where m.id=p.pi_id and  m.dealer_code='".$sc_all->dealer_code."' and  p.do_no=w.do_no group by p.pi_id";
		 $query = mysql_query($sql);
		 while($info=mysql_fetch_object($query)){
