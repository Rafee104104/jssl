...
$journalAll=find_all_field('journal_item','','serial_no="'.$data->serial_no.'"');

$journal_item_sql = 'insert into journal_item (`ji_date`,`item_id`,`warehouse_id`,`serial_no`,`item_ex`,`item_price`,`final_price`,`tr_from`,`tr_no`,`sr_no`,`entry_by`,`entry_at`,`primary_id`) value("'.$data->entry_at.'","'.$data->item_id.'","'.$_SESSION['user']['depot'].'","'.$data->serial_no.'",1,"'.$journalAll->item_price.'","'.$journalAll->final_price.'","depAssetIssue","'.$data->id.'","'.$data->m_id.'","'.$_SESSION['user']['id'].'","'.date('Y-m-d h:i:s').'","'.$data->id.'")';
mysql_query($journal_item_sql);
mysql_query('update fixed_asset_issue_master set status="COMPLETED" where id='.$$unique_master);
...
$det=find_all_field('fixed_asset_issue_details','','serial_no="'.$_POST['serial_no'].'"');

if($det->serial_no!=$_POST['serial_no'] ){	

 $sql='insert into fixed_asset_issue_details(m_id,item_id,serial_no,date,quality,note,entry_by,type,PBI_ID) values

("'.$_POST['ids'].'","'.$_POST['item_id'].'","'.$_POST['serial_no'].'","'.date("Y-m-d").'","'.$_POST['quality'].'","'.$_POST['note'].'","'.$_SESSION['user']['id'].'","Department","'.$_POST['PBI_ID'].'")';
					...
					<div class="container n-form2">
						...
						<div class="form-group row m-0 pb-1">
							...
							<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
								...
								<input name="department_name" type="text" id="department_name" value="<?= $department_name; ?>" required/>
				...
				<table class="table1  table-striped table-bordered table-hover table-sm">
					...
					<tbody class="tbody1">
						...
						<tr>
							...
							<td>
								<select name="item_id" type="text" value="" id="item_id">
							...
							<td><input name="serial_no" type="text" class="input3" id="serial_no" /></td>
