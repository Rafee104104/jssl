...
class Calendar {
    ...
    public function __toString() {
        ...
        for ($i = 1; $i <= $num_days; $i++) {
            ...
            $taskCnt = find_a_field('crm_task_lists', 'count(*)', 'to_time LIKE "'.$specificDate.'%" AND assigned_person_id = "'.$this->userID.'"');

            if($taskCnt > 0){
                $html .= '<span title="You Have ';
                
                if($taskCnt > 0){$html .= $taskCnt.' Task(s) '; $newTask = find_a_field('crm_task_lists', 'task_name', 'to_time LIKE "'.$specificDate.'%" AND assigned_person_id = "'.$this->userID.'"');}else{$newTask = '';}
