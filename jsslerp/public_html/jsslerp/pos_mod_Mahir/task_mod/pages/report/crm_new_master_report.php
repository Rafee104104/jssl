...
<center class="mb-4"><h3>Organization Report</h3></center>
  ...
  ?>
    <tr>
      <th scope="row"><?=$i++?></th>
      <td><?=$data->name?></td>
      <td><?=$data->address?>,<?=$data->city?>,<?=find_a_field('country','country_name','id="'.$data->country.'"')?></td>
...
<center class="mb-4"><h3>Lead Report</h3></center>
  ...
  $con=' and organization="'.$_POST['org'].'"';
    ...
    $sql='select l.*,o.name,p.PBI_NAME,p.PBI_MOBILE from crm_project_lead l,crm_project_org o,personnel_basic_info p where l.organization=o.id and l.assign_person=p.PBI_ID  '.$con.'';
  $query=mysql_query($sql);
  $i=1;
  while($data=mysql_fetch_object($query)){
  ?>
