...
if(isset($_REQUEST['in_time'])){
   ...
   //$insert_id=@insert('hrm_attdump');
    ...
    $sql = 'insert into hrm_attdump(`ztime`,`bizid`,`EMP_CODE`,`time`,`xtime`,`xdate`,`xlocationid`,`latitude`,`longitude`,`schedule_notes`,`sch_latitude_point`,`sch_longitude_point`) 
  value("'.date('Y-m-d H:i:s').'","'.$PBI_ID.'","'.$PBI_ID.'","'.$time.'","'.date('Y-m-d H:i:s').'","'.date('Y-m-d').'","999","'.$_POST['latitude'].'","'.$_POST['longitude'].'",
