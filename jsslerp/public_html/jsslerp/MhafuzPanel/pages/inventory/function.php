...
function system_finance_error_log($check_date, $db_user, $db_pass, $db_name)
{
	...
	$config_class = find_all_field('config_group_class', '', 'group_for="' . $_SESSION['user']['group'] . '"');
	...
	while ($data = mysql_fetch_object($query)) {
		...
		if ($data->tr_from == "Purchase") {
			$con = ' and ledger_id!="' . $config_class->purchase_discount . '" and ledger_id!="' . $config_class->purchase_vat . '" and ledger_id!="' . $config_class->purchase_ait . '" and ledger_id!="' . $config_class->po_transport_charge . '" and ledger_id!="' . $config_class->po_other_charge . '" ';

		}

		$sql_sum = "select " . $field_name . "  from " . $table_name . " where   " . $condition . $con . " and  " . $date_name . " between '" . date('Y-m-d 00:00:00', strtotime($check_date)) . "' and '" . date('Y-m-d 23:59:59', strtotime($check_date)) . "'";
...
function system_acc_error_log($check_date, $db_user, $db_pass, $db_name)
{
	...
	echo $config_sql = "select sum(cr_amt)  as cr_amt,sum(dr_amt) as dr_amt,tr_from from secondary_journal where entry_at between '" . date('Y-m-d 00:00:00', strtotime($check_date)) . "' and '" . date('Y-m-d 23:59:59', strtotime($check_date)) . "'  group by tr_from";
	...
	if (($d_cr <> 0) || ($d_dr <> 0)) {
		...
		$del_sql = "delete  from system_daily_error_log_acc where  cid='".$_SESSION["cid"]."' and date='" . $check_date . "'";
	...
	$exdel='delete from system_daily_error_log_acc where cid="'.$_SESSION['cid'].'" and date="'.$check_date.'"';
	mysql_query($exdel);
	}
