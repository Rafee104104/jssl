...
$lc_no= find_a_field('lc_provision_payment','lc_no','payment_no='.$_REQUEST['payment_no']);
...
//echo 'Omar';
		...
		$rec_date=$_POST['rec_date'];
		 ...
		 $pro_sql = "select sum(pay_amt_in) total_pay ,order_no,qty from lc_provision_payment where lc_no=".$lc_no." group by order_no";
	
		$pro_query = mysql_query($pro_sql);
		while($data= mysql_fetch_object($pro_query)){
			$provision_rate[$data->order_no] = ( $data->total_pay / $data->qty )  ;
		}
   ...
   (pr_no, po_no, order_no, payment_no,  payment_id,  lc_part, rec_date,   lc_no, lc_number, vendor_id, item_id, group_for, warehouse_id, exchange_rate, rate, rate_usd, qty, unit_name, amount, amount_usd, status, entry_by, entry_at) 

   VALUES("'.$rec_no.'", "'.$data->po_no.'", "'.$data->id.'", "'.$payment_no.'",  "'.$payment_id.'",  "'.$lc_part.'", 
   "'.$rec_date.'", "'.$lc_no.'", "'.$lc_number.'",  "'.$data->vendor_id.'", "'.$item_id.'", "'.$group_for.'", "'.$warehouse_id.'",
...
journal_item_control($item_id, $warehouse_id, $rec_date,  $qty, 0,  'Purchase', $rec_no, $rate, $warehouse_id, $data->pr_no,'', '', $data->po_no, $lc_manual_no, '', $data->group_for, $rate);
...
if($rec_no>0)
{
		...
		auto_insert_lc_shipment_secoundary_journal($rec_no);
  ...
  <tr>
    ...
    <? $field='group_for'; $table='user_group';$get_field='id';$show_field='group_name';?>
      ...
      <div>
        ...
        <input  name="warehouse_id2" type="text" id="warehouse_id2" value="<?=find_a_field($table,$show_field,$get_field.'='.$po_data->group_for)?>" readonly="readonly"/>

		<input  name="group_for" type="hidden" id="group_for" value="<?=$po_data->group_for?>" required="required"/>
      ...
      <? $field='vendor_id'; $table='vendor_foreign';$get_field='vendor_id';$show_field='vendor_name';?>
   ...
   <div></div>
      ...
      <input  name="lc_number" type="text" id="lc_number" value="<?=find_a_field($table,$show_field,$get_field.'='.$$field)?>"  readonly="readonly"/>
...
<?php /*?>	<? if($ex<time()){?>
	...
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
      ...
      <tr>
		...
		<?php /*?><?=($rec_date!='')?$rec_date:date('Y-m-d')?><?php */?>
		<input style="width:120px; height:25px;"  name="rec_date" type="text" id="rec_date" value="<?=$_POST['rec_date']?>" required />		</td>
  ...
  <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
    ...
    <tr>
      ...
      <tbody>
          ...
          <? while($row2=mysql_fetch_object($res2)){$bg++?>
...
db_update($table_master, $po_no, $vars, 'po_no');
 ...
 $sql1 = "update lc_number_setup set status='GOODS RECEIVED' where id='".$lc_no."'";
mysql_query($sql1);
