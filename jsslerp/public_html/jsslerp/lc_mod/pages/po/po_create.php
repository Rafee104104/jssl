...
if(isset($_POST['add'])&&($_POST[$unique]>0))
{
		...
		$_POST['amount_bdt']=$_POST['amount_usd'] * $_POST['exchange_rate'];
		$_POST['rate_bdt'] = $_POST['amount_bdt'] / $_POST['qty'];
		
		$_POST['assessable_value'] = 1.02;
		$_POST['item_id']=$iii[0];
		$_POST['entry_by']=$_SESSION['user']['id'];
		$_POST['entry_at']=date('Y-m-d h:s:i');
		$_POST['edit_by']=$_SESSION['user']['id'];
...
var exchange_rate = document.getElementById('exchange_rate#'+id).value*1;
...
var amount_bdt = document.getElementById('amount_bdt#'+id).value= (amount_usd*exchange_rate);

var rate_bdt = document.getElementById('rate_bdt#'+id).value= (amount_bdt/wo_qty);
...
var rate_bdt  = (document.getElementById("rate_bdt#"+id).value);
var amount_bdt = (document.getElementById("amount_bdt#"+id).value)*1;

var rate_ud  = (document.getElementById("rate_ud#"+id).value);
var amount_ud = (document.getElementById("amount_ud#"+id).value)*1;

var info = qty+"<@>"+rate_usd+"<@>"+amount_usd+"<@>"+rate_ud+"<@>"+amount_ud+"<@>"+rate_bdt+"<@>"+amount_bdt;
...
<table width="99%" border="0" cellspacing="0" cellpadding="0" align="center">
  <tr>
	...
	<?php /*?><? $field='po_date'; if($po_date=='') $po_date =date('Y-m-d');?><?php */?>
	  ...
	  <? $field='po_no';?>
	  <div>
        <label style="width:30%" for="<?=$field?>">PO No:</label>
        
		
		 <input  name="po_no" type="text" id="po_no" value="<?=$po_no;?>" required style="width:50%" />
      ...
      <div>
        <label style="width:30%" for="<?=$field?>">PO Date:</label>
    ...
    <td width="48%">
	  ...
	  <div>
        ...
        <select id="pi_reference" name="pi_reference" required style="width:50%">
         ...
         <? foreign_relation('lc_number_setup','id','lc_number',$pi_reference, 'id="'.$pi_data->id.'"');?>
        </select>
		<? } else {?>
		...
		<select id="pi_reference" name="pi_reference" required style="width:50%" >
         ...
         <? foreign_relation('lc_number_setup','id','lc_number',$pi_reference, 'id="'.$pi_reference.'"');?>
        </select>
		<? }?>
      </div>
	  ...
	  <? $field='exchange_rate_ex'; ?>
	   <div>
            <label style="width:30%" for="<?=$field?>">Exchange Rate Expected:</label>
            <input name="<?=$field?>" type="text" id="<?=$field?>" value="<?=$$field?>"  style="width:50%"/>
          </div>
		  
		  <? $field='exchange_rate_cur'; ?>
	   <div>
            <label style="width:30%" for="<?=$field?>">Exchange Rate Current:</label>
            <input name="<?=$field?>" type="text" id="<?=$field?>" value="<?=$$field?>"  style="width:50%"/>
          </div>
		  <? $field='exchange_rate_duty'; ?>
	   <div>
            <label style="width:30%" for="<?=$field?>">Exchange Rate Duty:</label>
            <input name="<?=$field?>" type="text" id="<?=$field?>" value="<?=$$field?>"  style="width:50%"/>
          </div>
...
<? }else{?>
			<table  width="100%" border="1" align="left"  style="border-collapse:collapse; border:1px solid #caf5a5;" cellpadding="2" cellspacing="2">
                      <tr>
						...
						<td align="center" bgcolor="#0099FF">HS Code</td>
...
<input  name="exchange_rate" type="hidden" id="exchange_rate" value="<?=$exchange_rate_ex?>"/>
...
<td bgcolor="#CCCCCC">
<input list= "hs" name="hs_code" id="hs_code"  autocomplete="off" required />

<datalist id="hs">

	<option></option>
	
	<? foreign_relation('hs_code','hs_code','hs_code',$hs_code, '1');?> 
...
<input name="amount_ud" type="hidden" class="input3" id="amount_ud" style="width:90px; height:32px;" readonly="readonly" />
<input name="assessable_rate" type="hidden" id="assessable_rate"  value="1.02" />
	  ...
	  <br />
        ...
        <td><div class="tabledesign2">
          <table id="grp" width="100%" cellspacing="0" cellpadding="0"><tbody>
			...
			<th width="11%" >Rate(BDT) </th>
			<th width="12%" >Amount(BDT)</th>
...
<?
$s=0;
  $res='select a.id,  b.finish_goods_code as item_code, concat(b.item_name) as item_name, a.specification,  a.rate as unit_price,a.hs_code,
 a.qty, a.unit_name, a.amount,  a.rate_usd, a.amount_usd,a.rate_bdt,a.amount_bdt,a.exchange_rate,a.rate_ud, a.amount_ud, "x" from lc_purchase_invoice a,item_info b where b.item_id=a.item_id  and a.po_no="'.$po_no.'" order by id desc';
...
<td><input type="text" name="<?='amount_usd#'.$po_item->id?>" id="<?='amount_usd#'.$po_item->id?>" value="<?=$po_item->amount_usd; $total_amount_usd+=$po_item->amount_usd;?>" style="width:100px;  height:32px;"  readonly=""/>
  ...
  <input type="hidden" name="<?='exchange_rate#'.$wo_item->id?>" id="<?='exchange_rate#'.$po_item->id?>" value="<?=$po_item->exchange_rate?>" style="width:60px;  height:20px;"/>
  ...
  <td>
  <input type="text" name="<?='rate_bdt#'.$wo_item->id?>" id="<?='rate_bdt#'.$po_item->id?>" value="<?=$po_item->rate_bdt?>" style="width:60px;  height:20px;" readonly/>
  </td>
  <td>
  <input type="text" name="<?='amount_bdt#'.$wo_item->id?>" id="<?='amount_bdt#'.$po_item->id?>" value="<?=$po_item->amount_bdt?>" style="width:60px;  height:20px;" readonly />
  </td>
