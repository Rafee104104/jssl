...
{
$booking=' and m.booking_no="'.$_POST['booking_number'].'"';

}

if($_POST['rec_year']!='')
{
$year=' and m.year="'.$_POST['rec_year'].'"';

}
if($_POST['customer']!='')
{
$cust=' and m.customer="'.$_POST['customer'].'"';

}

if($_POST['sr_number']!='')
{
$sr_no=' and s.sr_no="'.$_POST['sr_number'].'"';

}

if($_POST['booking_type']!='')
{
$booking_type=' and b.booking_type="'.$_POST['booking_type'].'"';
 ...
 $sql2 = "
    SELECT m.do_no,
           COUNT(s.id) AS do_n,
           SUM(s.dist_unit) AS unit,
           m.do_date
    FROM sale_do_master m
    JOIN sale_do_details s ON m.do_no = s.do_no
    -- JOIN dealer_info d ON m.dealer_code = d.dealer_code  -- only if needed
    WHERE m.status != 'MANUAL'
    ".$depot_con.$date_con.$item_con.$dealer_con.$dtype_con.$sr_no.$cust.$year.$booking."
    GROUP BY m.do_no, m.do_date
    ORDER BY m.do_no, m.do_date
";

$query2 = mysql_query($sql2);
while($data2 = mysql_fetch_object($query2)){
    $marge[$data2->do_no] = $data2->do_n;
    $total_units[$data2->do_no] = $data2->unit;
}


$s=1;
  $sql="select m.do_no,m.customer,i.item_name,m.do_date,m.entry_at ,m.entry_time,concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name,b.booking_type,
s.*



from sale_do_master m,sale_do_details s,dealer_info d  ,  item_info i,paid_booking b

where m.do_no=s.do_no and m.status!='MANUAL'  and m.dealer_code=d.dealer_code and b.booking_number_eng=m.booking_no  and i.item_id=s.item_id
".$depot_con.$date_con.$item_con.$dealer_con.$dtype_con.$sr_no.$cust.$year.$booking.$booking_type." order by m.do_no,m.do_date asc";
...
<tr>
<? 
 $old_do6 =$data->do_no;  if($old_do6!=$old_mac_do6){ ?>
<td rowspan="<?=$marge[$data->do_no];?>"><?=$s++?></td>
<?  } $old_mac_do6 = $old_do6; ?>
<? 
 $old_do5 =$data->do_no;  if($old_do5!=$old_mac_do5){ ?>
<td rowspan="<?=$marge[$data->do_no];?>"><?=$data->do_date?></td>
<?  } $old_mac_do5 = $old_do5; ?>
<? 
 $old_do =$data->do_no;  if($old_do!=$old_mac_do){ ?>

<td rowspan="<?=$marge[$data->do_no];?>"><?=$data->do_no?></td>
<?  } $old_mac_do = $old_do; ?>
<? 
 $old_do2 =$data->do_no;  if($old_do2!=$old_mac_do2){ ?>
 
<td rowspan="<?=$marge[$data->do_no];?>"><?=$data->customer?></td>
<?  } $old_mac_do2 = $old_do2; ?>
<? 
 $old_do3 =$data->do_no;  if($old_do3!=$old_mac_do3){ ?>
 
<td rowspan="<?=$marge[$data->do_no];?>"><?=find_a_field('dealer_info','contact_no','dealer_code='.$data->dealer_code);?></td>
<?  } $old_mac_do3 = $old_do3; ?>
<? 
 $old_do4 =$data->do_no;  if($old_do4!=$old_mac_do4){ ?>
<td rowspan="<?=$marge[$data->do_no];?>"><?=$data->booking_no?></td>
<?  } $old_mac_do4 = $old_do4; ?>
<td><?=$data->sr_no?></td>
<td><?=$data->dist_unit?></td>
<td>
<? $jritem="select warehouse_id,barcode,item_in from journal_item where tr_from='Other Receive' and barcode='".$data->sr_no."'"; 
$m=mysql_query($jritem);
while($r=mysql_fetch_object($m)){
echo $r->warehouse_id."-".number_format($r->item_in,0).", ";
}
?></td>
<? 
 $old_do7 =$data->do_no;  if($old_do7!=$old_mac_do7){ ?>
<td rowspan="<?=$marge[$data->do_no];?>" style="text-align:right"><?=$total_units[$data->do_no] ;  $tot_unit+=$total_units[$data->do_no];?></td>
<?  } $old_mac_do7 = $old_do7; ?>
<? 
 $old_do8 =$data->do_no;  if($old_do8!=$old_mac_do8){ ?>
<td rowspan="<?=$marge[$data->do_no];?>" style="text-align:right"></td>
<?  } $old_mac_do8 = $old_do8; ?>
</tr>
<? } ?>
<tr class="footer">
...
<td style="text-align:right"><?=number_format($tot_unit,2)?></td>
</tr>
</tbody>
</table>
<? }
