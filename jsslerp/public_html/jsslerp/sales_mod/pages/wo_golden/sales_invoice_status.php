                ...
                <table id=" " class="table1  table-striped table-bordered table-hover table-sm">
                   ...
                   <? 
					...
					if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and m.do_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
						...
						$res2="select m.do_no, m.do_no, m.file_attached, m.do_date, m.dealer_code, m.job_no, m.sales_type, m.depot_id, m.po_no,  d.dealer_name_e, m.entry_by, m.status from 
					sale_do_master m, sale_do_details c,  dealer_info d, user_group u 
					where 
					  m.group_for=u.id  and m.dealer_code=d.dealer_code and m.do_no=c.do_no ".$group_for_con.$con.$dealer_con."  group by c.do_no order by c.do_no desc";
					...
					?>
                        <tr> 
							...
							<td>
							<? if ($data->file_attached!=''){?>		
							<a href="../../../assets/support/upload_view.php?name=<?=$data->file_attached?>&folder=file_attached&proj_id=<?=$_SESSION['proj_id']?>&mod=sales_mod" target="_blank">View Attachment</a>
							<? } else {?>
