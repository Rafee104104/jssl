...

{
		...
		echo '<script>location.href="do.php?do_no='.$$unique_master.'";</script>';
		exit;
...
<div class="form-container_large">
        ...
        <div class="col-sm-6 col-md-6 col-lg-6 col-xl-6">
          <div class="container n-form2">
            ...
            <div class="form-group row m-0 pb-1">
              ...
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
                <input name="do_date" type="text" id="do_date" value="<?=find_a_field('sale_do_master','do_date','1 order by do_no desc'); //($do_date!='')?$do_date:date('Y-m-d')?>"  required />
              </div>
            </div>
            <? }?>
            <? if($do_date!="") {?>
            <div class="form-group row m-0 pb-1">
              <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Order Date</label>
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
                <input name="do_date" type="hidden" id="do_date" value="<?=find_a_field('sale_do_master','do_date','1 order by do_no desc');?>"  required/>
                <input name="do_date2" type="text" id="do_date2" value="<?=find_a_field('sale_do_master','do_date','1 order by do_no desc');?>" readonly="" required/>
              </div>
            </div>
            <? }?>
			...
			<div class="form-group row m-0 pb-1">
              <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Order Name </label>
              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
        ...
        <div class="col-sm-6 col-md-6 col-lg-6 col-xl-6">
          <div class="container n-form2">
            <? if($job_no!="") {?>
                ...
                <input name="job_no_duplicate" type="hidden" id="job_no_duplicate" value="<?=$job_no?>" readonly="" tabindex="105" />
            ...
            <div class="form-group row m-0 pb-1">
			  ...
			  <? $year=date('Y');?>
			    <input name="booking_no" id="booking_no" list="sr_nos" value="<?=$booking_no;?>"  autocomplete="off"  onchange="getData2('tkAjax.php', 'Shuvo', this.value)">
			  
			       <datalist id="sr_nos">
                  <? foreign_relation('sr_token','booking_number', 'sr_number',$master->booking_no,'1 and token_year='.$year.'');?>
              </datalist>
            ...
            <div class="form-group row m-0 pb-1">
			  ...
			  <span id="Shuvo">
                ...
                <select  id="dealer_code" name="dealer_code" class="form-control" required="required"  >
                  <?
  ...
  <? if($$unique_master>0) {?>
  <form action="<?=$page?>?do_no=<?=$$unique_master?>" method="post" name="codz2" id="codz2">
      ...
      <table class="table1  table-striped table-bordered table-hover table-sm">
        ...
        <tbody class="tbody1">
          <tr>
			...
			$year=date("Y");
			?>
			  <input name="sr_no" id="sr_no" list="sr_ar" value=""  autocomplete="off"  onchange="getData2('itemJournalAjaxList.php', 'tk47bd', this.value)" >
			  
			       <datalist id="sr_ar">
                <? foreign_relation('sr_token','sr_number','sr_number','','booking_number="'.$master->booking_no.'" and token_year='.$year.' ');?>
              </datalist>
			
             
            </input>
			  <input type="hidden" id="booking_no" name="booking_no" value="<?=$booking_no?>"  />

			
			
			<?php /*?><select name="sr_no" id="sr_no" onchange="getData2('itemJournalAjaxList.php', 'tk47bd', this.value)">
                <option> </option>
                <? foreign_relation('sr_token','sr_number','sr_number',$sr_no,'booking_number="'.$master->booking_no.'"');?>
   
                <? foreign_relation('sr_token','sr_number','sr_number',$sr_no,'1');?>

           </select>            
			      <input type="hidden" id="booking_no" name="booking_no" value="<?=$master->booking_no?>"  />
<?php */?>
			  </td>
            <td id="sub"><?
							...
							?>
              <?php /*?>   <input name="item_id" type="text" value="100010001" id="item_id" onblur="getData2('sales_invoice_ajax.php', 'so_data_found', this.value, document.getElementById('do_no').value);"/><?php */?>
              <select  name="item_id" id="item_id"  style="width:90%;" required onchange="getData2('sales_invoice_ajax.php', 'so_data_found', this.value, document.getElementById('do_no').value);">
                <option></option>
                <? foreign_relation('item_info','item_id','item_id','100010001','item_id=100010001');?>
              </select>
              ...
              <input type="hidden" id="dealer_code" name="dealer_code" value="<?=$dealer_code?>" required="required"  />
              <input name="do_date" type="hidden" id="do_date" value="<?=$do_date;?>"/>
              <input name="job_no" type="hidden" id="job_no" value="<?=$job_no;?>"/>            </td>
            <td id="tk47bd" colspan="3"><table  width="100%" border="1" align="left" cellpadding="0" cellspacing="2">
                <tr>
                  <td width="50%"><input name="item_name" type="text" readonly=""  autocomplete="off"  value="" id="item_name" />                  </td>
                  <td width="20%"><input name="pcs_stock" type="text" readonly=""  autocomplete="off"  value="" id="pcs_stock" /></td>
                  <td width="30%"><input name="ctn_price" type="text" id="ctn_price" readonly="" required  value="<?=$do_data->ctn_price;?>" /></td>
<?php /*?>                  <td width="20%"><input name="pcs_price" type="text" id="pcs_price" readonly="" required="required"  value="<?=$do_data->pcs_price;?>"  /></td>
<?php */?>                </tr>
              </table></td>
          ...
          <?php /*?>  <td><input name="total_unit" type="hidden" id="total_unit" readonly/>
              <input name="total_amt" type="text" id="total_amt" readonly/>            </td><?php */?>
    ...
    <div class="container-fluid pt-5 p-0 ">
      <?
			  $res='select a.id,b.item_name,a.item_id,a.unit_name,a.pcs_stock, a.item_name, a.total_unit,a.dist_unit, a.total_amt as Net_sale, a.discount, a.vat_amt, a.total_amt_with_vat,a.sr_no from
      ...
      <table class="table1  table-striped table-bordered table-hover table-sm">
        ...
        <tbody class="tbody1">
          ...
          <tr>
			...
			<td><?=round($data->pcs_stock)?> <? $tot_pcs+=$data->pcs_stock; ?></td>
            <td><?=$data->dist_unit; ?><? $tot_Net_sale+=$data->dist_unit; ?></td>
