    ...
    <form action="" method="post" name="codz" id="codz">
        ...
        <div class="container-fluid pt-5 p-0 ">
                <table id="grp" class="table1  table-striped table-bordered table-hover table-sm">
                    ...
                    $res="select m.*,m.do_no,m.do_date,concat(d.dealer_code,'-',d.dealer_name_e) as dealer_name,m.customer,m.entry_at,m.booking_no,m.entry_by
from sale_do_master m,dealer_info d
where m.status in ('PROCESSING')  and m.dealer_code=d.dealer_code ".$con."
  order by m.do_date desc,d.dealer_name_e ";
                    ...
                    while($data = mysql_fetch_object($query))
                    {
                        ...
                        <tr <?=($data->RCV_AMT>0)?'style="background-color:#FFCCFF"':'';?>>
                            ...
                            <td onClick="custom(<?=$data->do_no;?>);" <?=(++$z%2)?'':'class="alt"';?> style="text-align:left">&nbsp;<?=$data->booking_no;?></td>
                            <td onClick="custom(<?=$data->do_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?=$data->customer;?></td>
				...
				?>
				  <?=$data2->sr_no.", ";?>
				  <? } ?> </td>
                            <td onClick="custom(<?=$data->do_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?=$tot_unit=find_a_field('sale_do_details','sum(dist_unit)','do_no='.$data->do_no); $total_unit+=$tot_unit;?></td>
                            <td onClick="custom(<?=$data->do_no;?>);" <?=(++$z%2)?'':'class="alt"';?>> <?=find_a_field('user_activity_management','fname','user_id='.$data->entry_by);?></td>
                            <td onClick="custom(<?=$data->do_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=$data->entry_at;?></td>

                            <td onClick="custom(<?=$data->do_no;?>);">
                                <input type="button" class="btn1 btn1-bg-submit" value="Check DO">                            </td>
                        </tr>
                        <?
                    }
