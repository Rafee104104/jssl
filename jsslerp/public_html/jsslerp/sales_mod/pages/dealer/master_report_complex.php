...
break;
	...
	while($reinfo = mysql_fetch_object($requery))
	{
	...
	$report="Customer Information";
		...
		$sql.= " from dealer_info a left join dealer_type t on t.id=a.dealer_type left join branch r on r.BRANCH_ID=a.region_code left join zon z on z.ZONE_CODE=a.zone_code left join warehouse w on w.warehouse_id=a.depot left join area ar on ar.AREA_CODE=a.area_code where	1 and a.group_for='".$_SESSION['user']['group']."' ".$dealer_con."";
//echo $sql;
break;
		//ROW_NUMBER() OVER(ORDER BY Id) AS Row
}
