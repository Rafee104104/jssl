...
@mysql_connect("localhost", "bdnews_master", "master123");

@mysql_select_db('bdnews_master');
...
-->
	...
	$i=1; 
	 ...
	 $n_query = mysql_query($sqld);
	 $count = mysql_fetch_object($n_query);
	 ...
	 $querylate = mysql_query($sqllate);
	 $count_late = mysql_fetch_object($querylate);
...
<?php if(isset($_POST['send'])){  $id=$_POST['choose'];
    ...
    // $query=mysql_query($sql);
    // while($result=mysql_fetch_object($query)){  echo $result->PROJECT_NAME; }
