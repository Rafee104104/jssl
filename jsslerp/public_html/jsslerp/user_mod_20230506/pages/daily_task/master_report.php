...
//where 1   ' .$task_con.'  order by m.task_date asc'; 
$task_sql = mysql_query($task_query);
$i = 0;
while($task_data = mysql_fetch_object($task_sql)){
$i++;
...
$task_sqll = mysql_query($task_queryy);
$i = 0;
while($task_dataa=mysql_fetch_object($task_sqll)){
$i++;
...
$query=mysql_query($details_query);

//while($deitals=mysql_fetch_object($query)){
...
<td><?php  $s="select p.PROJECT_NAME FROM project p,daily_give_task_details d WHERE d.task_id=".$task_dataa->task_id." and d.project_name=p.PROJECT_ID";
          $q=mysql_query($s);
           while($r=mysql_fetch_object($q)){echo $r->PROJECT_NAME.'</br>';}
   ?> </td>
...
<td><?php  $s="select d.task_name from given_task_details d where  d.master_id=".$task_dataa->task_id ;
   ...
   <td><?php  $s="select d.remarks from given_task_details d where  d.master_id=".$task_dataa->task_id ;
           $q=mysql_query($s);
           while($r=mysql_fetch_object($q)){echo $r->remarks.'</br>';}
   ?></td>   
...
$task_sql11 = mysql_query($task_queryy1);
$i = 0;
while($task_dataaa = mysql_fetch_object($task_sql11)){
$i++;
//$grand_total+=$task_data->total;
?>
<tr>
...
$leave = mysql_query($absence);
$i = 0;
while($row = mysql_fetch_object($leave)){
{?>
<table  width="100%" cellspacing="0" cellpadding="2" border="0">
<thead>
