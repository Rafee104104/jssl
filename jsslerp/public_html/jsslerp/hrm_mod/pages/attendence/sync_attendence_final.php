...
$endTime  = strtotime($end_date);
 ...
 $emp_id  = find_a_field('personnel_basic_info','PBI_ID','PBI_CODE="'.$_POST['emp_id'].'"'); // array iy
...
while($holy_data = mysql_fetch_object($holy_query)){

$holy_day[$holy_data->holy_day]=1;

$holy_final_sql = "update hrm_att_summary s,personnel_basic_info p  set s.holyday = 1 where p.PBI_ID=s.emp_id and p.JOB_LOC_ID=".$job_loc." and  s.att_date='".$holy_data->holy_day."'";// Make Status
...
$query_ot  =mysql_query($sql_ot);

while($data_ot=mysql_fetch_object($query_ot)){
...
// Main Data Fetch
 ...
 $sql = "SELECT h.*,p.define_schedule,p.define_offday,p.punch_type,p.grace_type,p.define_offday2,p.PBI_DOJ
...
elseif($data->define_offday==date('l', strtotime($data->att_date)) || $data->define_offday2==date('l', strtotime($data->att_date))  ) $sch_off_day[$data->emp_id][$data->att_date] = 1;
...
if($emp_id!=$old_emp_id) {$grace_no=0; $grace_pending_min=$total_grace_time[$data->grace_type];}

// START

if(($late_min)>0) // Late Day
{
   // echo $late_min;
    if($grace_no<$grace_days[$data->grace_type] && $grace_pending_min>0) // if Gress Count exist & Gress Monthly Limit Exist
        {
            $grace_no++;
            $grace = $grace_no;
            if(($late_min)<=$grace_time[$data->grace_type]) // if Late min less than Daily gress Limit
                {
                    if(($late_min)<=$grace_pending_min) // if Late min less than Daily gress Limit
                        {
                        $final_late_min = 0; $final_late_status = 0;$grace_pending_min = $grace_pending_min - ($late_min); // Best Case
                        }
                    else
                        {
                        $final_late_min = ($late_min) - $grace_pending_min;$final_late_status = 1;$grace_pending_min = 0;
                        }

                }
                else  // if Late min More than Daily gress Limit
                {
                     if($grace_time[$data->grace_type]<=$grace_pending_min) // if Gress Monthly Limit More than Daily gress Limit
                        {
                        $final_late_min = ($late_min)- $grace_time[$data->grace_type];$final_late_status = 1;$grace_pending_min = $grace_pending_min - $grace_time[$data->grace_type];
                        }
                    else // 
                        {
                        $final_late_min = ($late_min) - $grace_pending_min; $final_late_status = 1;$grace_pending_min = 0; 
...
if(($outs_datetime[$data->emp_id][$data->att_date]>0)  && ($sch_off_day[$data->emp_id][$data->att_date] == 0) && ($holyday[$data->emp_id][$data->att_date]==0)){
$ot_min = (int)(($outs_datetime[$data->emp_id][$data->att_date] - $sch_outs_datetime[$data->emp_id][$data->att_date]) / 60);

$ot_in_min = (int)(($sch_ins_datetime[$data->emp_id][$data->att_date] - $ins_datetime[$data->emp_id][$data->att_date]) / 60);

if($ot_in_min>30) $ot_extra_min = $ot_in_min;
else $ot_extra_min = 0;

if($ot_min>30) $ot_min = $ot_min;
else $ot_min = 0;

if(($ot_extra_min +$ot_min)>0)
$ot_final_min = $ot_extra_min +$ot_min;
	 
if($ot_final_min>0){
$ot_hour = (int)($ot_final_min/60);
$ot_final_hour = round($ot_final_min / 60); //($ot_final_min%60);
$ot_final_status=1;
	}
...
} elseif(($outs_datetime[$data->emp_id][$data->att_date]>0) && ($holyday[$data->emp_id][$data->att_date]==1) && ($ot_holiday_applicable[$data->emp_id]=='YES')){
...
} elseif(($outs_datetime[$data->emp_id][$data->att_date]>0) && ($sch_off_day[$data->emp_id][$data->att_date]==1) && ($ot_weekend_applicable[$data->emp_id]=='YES') ){
...
ot_min='".$ot_min."', ot_final_min='".$ot_final_min."',ot_final_hour='".$ot_final_hour."' ,ot_final_status='".$ot_final_status."',ot_extra_min='".$ot_extra_min."',



grace_no='".$grace."',grace_pending_min='".$grace_pending_min."',
