...
$ORG_con = " and p.PBI_ORG='" . $PBI_ORG . "'";
 ...
 $sql ="SELECT h.emp_id,(count(1)-sum(h.leave_duration)) pre, sum(h.sch_off_day) as offday, sum(h.present) as present , sum(h.final_late_min) l_min,sum(h.final_late_status) l_status,sum(h.ot_final_hour) as total_over_time,p.PBI_DOJ,p.PBI_ID,sum(h.final_early_status) as early_out,sum(h.final_late_status) as total_late,sum(final_late_min) as total_late_min,p.DEPT_ID FROM `hrm_att_summary` h,personnel_basic_info p WHERE 1 and h.present=1  and p.PBI_ID=h.emp_id and h.att_date BETWEEN '" .$start_date ."' AND '" .$end_date ."' " .$emp_con .$ORG_con .$job_location_con.
...
$values[$pi]['hd'] = $values[$pi]['hd'];

$values[$pi]['od'] =  $data->offday; //$values[$pi]['od'];

$values[$pi]['pre'] =   $data->present; //($values[$pi]['pre']-($friday_office+$hollyday_office));
