...
/*$holy_day = find_a_field("salary_holy_day","count(holy_day)",'holy_day between "' . $start_date . '" and "' . $end_date . '" and job_loc_id in(1,3)');
...
/*  $sql_holy ="SELECT h.emp_id,sum(leave_duration) lv

FROM `hrm_att_summary` h, salary_holy_day d, personnel_basic_info p

WHERE h.emp_id=p.PBI_ID and att_date=holy_day and `att_date` BETWEEN '" .$start_date ."' AND '" .$end_date ."' ".$ORG_con."  And leave_id>'0' and d.job_loc_id in (1,3)

group by h.emp_id";

$query_holy = mysql_query($sql_holy);

while ($data_holy = mysql_fetch_object($query_holy)) {

$total_holyday_leave[$data->emp_id] = $data_holy->lv;

}*/

//find_a_field('personnel_basic_info','PBI_ID','PBI_CODE="'.$emp_id.'"')

//if ($emp_id> 0) {$emp_con = " and p.PBI_ID='".find_a_field('personnel_basic_info','PBI_ID','PBI_CODE="'.$emp_id.'"')."'";}
...
echo $sql ="SELECT h.emp_id,sum(h.present) as pre, sum(h.final_late_min) l_min,sum(h.final_late_status) l_status, sum(h.leave_duration) as leave_days_lv,sum(h.sch_off_day) as od,sum(h.Holiday_Work) as holi_work,
  sum(h.ot_final_hour) as total_over_time,p.PBI_DOJ,p.PBI_ID,sum(h.final_early_status) as early_out,sum(h.final_late_status) as total_late,sum(final_late_min) as total_late_min,p.DEPT_ID 
...
$values[$pi]["hd"] = find_a_field("salary_holy_day  ","count(id)",'holy_day between "' . $m_s_date . '" and "' . $m_e_date . '" and job_loc_id in (1,3) '
...
/*$holy_sql = mysql_query('select * from salary_holy_day where holy_day between "'.$start_date.'" and "'.$end_date.'" and job_loc_id in (1,3)');

while($holy_data=mysql_fetch_object($holy_sql)){
...
$values[$pi]['pre'] = ($values[$pi]['pre']-($values[$pi]['od']+$friday_office+$hollyday_office));
