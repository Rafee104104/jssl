...
if ($PBI_ORG > 0) {$ORG_con = " and p.PBI_ORG='" . $PBI_ORG . "'";}
  ...
  $sql ="SELECT h.emp_id,sum(h.present) as pre, sum(h.final_late_min) l_min,sum(h.final_late_status) l_status, sum(h.leave_duration) as leave_days_lv,sum(h.sch_off_day) as od,
  sum(h.annual_leave) as annual_leave_balance,sum(h.holiday_work) as holiday_work,
  sum(h.ot_final_hour) as total_over_time,p.PBI_DOJ,p.PBI_ID,sum(h.final_early_status) as early_out,sum(h.final_late_status) as total_late,sum(final_late_min) as total_late_min,p.DEPT_ID,p.JOB_STATUS_DATE
  
  FROM `hrm_att_summary` h,personnel_basic_info p 
  WHERE 1 and h.present=1  and p.PBI_ID=h.emp_id and h.att_date BETWEEN '" .$start_date ."' AND '" .$end_date ."' " .$emp_con .$ORG_con .$job_location_con."  group by h.emp_id";
...
$leave_days_lwp = find_a_field("hrm_leave_info","sum(total_days)",'PBI_ID="' .$data->PBI_ID .'" and id=9 and s_date between "' .$start_date .'" and "' .$end_date .'" ');
...
$values[$pi]["pre"] =  ($data->pre - ($values[$pi]["od"]+$values[$pi]["hd"]+$data->holi_work));
...
$values[$pi]["hd"] = find_a_field("salary_holy_day  ","count(id)",'holy_day between "' . $m_s_date . '" and "' . $m_e_date . '"');
...
if($data->JOB_STATUS_DATE>0 && strtotime($data->JOB_STATUS_DATE) < $endTime){ 

$resign_emp_holy_day = find_a_field("salary_holy_day", "count(id)",'holy_day between "'.$m_s_date.'" and "'.$data->JOB_STATUS_DATE.'"');

if( strtotime($data->PBI_DOJ) > $startTime){

  $sql = "SELECT SUM(sch_off_day) AS offday_count FROM hrm_att_summary WHERE emp_id='".$data->PBI_ID ."' and att_date BETWEEN '" . $data->PBI_DOJ . "' and '" . $data->JOB_STATUS_DATE ."')";
  $result = mysql_query($sql);
  $row = mysql_fetch_assoc($result);
  $resign_emp_off  = $row['offday_count'];
  $resign_emp_days = ceil((strtotime($data->JOB_STATUS_DATE)-strtotime($data->PBI_DOJ)) / (3600 * 24));

}else{

  $sql = "SELECT SUM(sch_off_day) AS offday_count FROM hrm_att_summary WHERE emp_id='".$data->PBI_ID ."' and att_date BETWEEN '" . $m_s_date . "' and '" . $data->JOB_STATUS_DATE ."')";
  $result = mysql_query($sql);
  $row = mysql_fetch_assoc($result);
  $resign_emp_off  = $row['offday_count'];
  $resign_emp_days = ceil((strtotime($data->JOB_STATUS_DATE)-$startTime) / (3600 * 24));
}
...
$values[$pi]['pre'] = ($values[$pi]['pre']-($values[$pi]['od']+$data->holiday_work));
...
$values[$pi]['ab'] = (($values[$pi]['td']+$values[$pi]['day_deduct'])-($values[$pi]['pre']+$values[$pi]['od']+ $values[$pi]['hd']));
...
(`mon`, `year`, `PBI_ID`, `td`, `od`, `hd`, `lt`, `total_late_min`, `eo`,`ab`, `lv`,`iom`,`lwp`, `pre`, `pay`,`ot`,`annual_leave_balance`, `entry_at`, `entry_by`,`days_mon`,`day_deduct`) 
values

('" .$mon."','".$year."','" .$values[$y]["emp_id"] ."','" .$values[$y]["td"] ."','" .$values[$y]["od"] ."', '" .$values[$y]["hd"] ."','" .$values[$y]["lt"] ."','" .$values[$y]["total_late_min"] ."', 
'" .$values[$y]["early_out"] ."', '" .$values[$y]["ab"] ."','" .$values[$y]["lv"] ."','" .$values[$y]["iom"] ."','" .$values[$y]["lwp"] ."','" .$values[$y]["pre"] ."','" .$values[$y]["pay"] ."',
'" .$values[$y]["total_over_time"] ."','" .$values[$y]["annual_leave_balance"] ."','" .date("Y-m-d H:i:s")."','".$_SESSION["user"]["id"]."','".$values[$y]['days_mon']."','".$values[$y]['day_deduct']."')";
