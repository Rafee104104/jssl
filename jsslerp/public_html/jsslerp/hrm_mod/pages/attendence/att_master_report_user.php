...

{
 ...
 <tr>
		...
		<th width="9%" bgcolor="#FFFACD"> Employee CodeE</th>
 ...
 while($row=mysql_fetch_object($res))
{
	...
	$leave = 0;
       ...
       for($i=$f_date;$i<=$t_date;$i = date('Y-m-d', strtotime( $i . " +1 days"))){
         ...
         $queryiom = mysql_query($sqliom);
         while($infoiom=mysql_fetch_object($queryiom)){
        ...
        $queryho = mysql_query($sql2);
        while($infoholi=mysql_fetch_object($queryho)){
...
<?  } } ?>
 ...
 <tr>
  ...
  <?php /*?> <td><strong>Present : <?=$present+$friday+$leave; $present=0;$friday=0;$leave=0;?></strong></td><?php */?>
   <td><strong>Present : <?=$present;?></strong></td>
