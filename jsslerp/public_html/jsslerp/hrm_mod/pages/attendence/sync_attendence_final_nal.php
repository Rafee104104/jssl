...
echo $sql = "SELECT h.*,p.define_schedule,p.define_offday,p.punch_type,p.grace_type,p.define_offday2,p.PBI_DOJ
...
if(($late_min+$early_min)>0) // Late Day
{
   // echo $late_min;
    if($grace_no<$grace_days[$data->grace_type] && $grace_pending_min>0) // if Gress Count exist & Gress Monthly Limit Exist
        {
            $grace_no++;
            $grace = $grace_no;
            if(($late_min+$early_min)<=$grace_time[$data->grace_type]) // if Late min less than Daily gress Limit
                {
                    if(($late_min+$early_min)<=$grace_pending_min) // if Late min less than Daily gress Limit
                        {
                        $final_late_min = 0; $final_late_status = 0;$grace_pending_min = $grace_pending_min - ($late_min+$early_min); // Best Case
                        }
                    else
                        {
                        $final_late_min = ($late_min+$early_min) - $grace_pending_min;$final_late_status = 1;$grace_pending_min = 0;
                        }
                ...
                else  // if Late min More than Daily gress Limit
                {
                     if($grace_time[$data->grace_type]<=$grace_pending_min) // if Gress Monthly Limit More than Daily gress Limit
                        {
                        $final_late_min = ($late_min+$early_min)- $grace_time[$data->grace_type];$final_late_status = 1;$grace_pending_min = $grace_pending_min - $grace_time[$data->grace_type];
                        }
                    else // 
                        {
                        $final_late_min = ($late_min+$early_min) - $grace_pending_min; $final_late_status = 1;$grace_pending_min = 0; 
    ...
    {
    $final_late_min = ($late_min+$early_min);
    $final_late_status = 1;
    $grace = 0;
    }
