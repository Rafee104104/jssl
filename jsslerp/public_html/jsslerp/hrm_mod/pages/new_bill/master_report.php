...
if($_REQUEST['report']==1001)

{

$report="Monthly Overtime Report";
...
<img src="../../../logo/<?=$_SESSION['proj_id']?>.png" style="width:50%"> 
...
<td colspan="15" align="center" style="font-size:14px;border:0px; border-color:white;">Employee : &nbsp;<strong><?=find_a_field('personnel_basic_info','PBI_NAME','PBI_ID="'.$_POST['PBI_ID'].'"')?></strong>-<?=$_POST['PBI_ID']?></td>

</tr><?php */?>

<tr>

<td colspan="15" align="center" style="font-size:14px;border:0px; border-color:white;">Date For: <strong><?php echo date('d-M-Y',strtotime($_REQUEST['start_date']));?> </strong>Date To: <strong><?php echo date('d-M-Y',strtotime($_REQUEST['end_date']));?></strong></td>
</tr>
</table>
</td>
...
<thead>
	<tr>
		...
		<th bgcolor="#FFFACD">DEPARTMENT</th>
		<th bgcolor="#FFFACD">OVERTIME HOURS</th>
		<th bgcolor="#FFFACD">OVERTIME BILL</th>
		<th bgcolor="#FFFACD">BILL PAID</th>
		<th bgcolor="#FFFACD">DUE BILL</th>
		<th bgcolor="#FFFACD">OVERTIME DATE</th>
	</tr>
</thead>
<tbody>
<?php 
	 
	 $data = "SELECT o.PBI_ID,o.PBI_NAME,p.PBI_DEPARTMENT,o.OVERTIME_HOURS,o.OVERTIME_BILL,o.OVERTIME_BILL_PAID, o.OVERTIME_DATE 
	 FROM hrm_overtime_golden o, personnel_basic_info p
	 WHERE o.OVERTIME_DATE BETWEEN '".$_REQUEST['start_date']."' AND '".$_REQUEST['end_date']."' AND p.PBI_CODE=o.PBI_ID ";
	 
	$query = mysql_query($data);
	
	$sl=1;
	while($row =mysql_fetch_object($query)){
	$bill = $row->OVERTIME_BILL;
	$bill_paid = $row->OVERTIME_BILL_PAID;
	$total = $bill - $bill_paid;
?>
<tr>
<td><?=$sl++?></td>
<td><?=$row->PBI_ID;?></td>
...
<td><?=$row->OVERTIME_BILL;?></td>
<td><?=$row->OVERTIME_BILL_PAID;?></td>
<td><?=$total?> </td>
...
<thead>
	<tr>
		...
		<th bgcolor="#FFFACD">EMPLOYEE CODE</th>
		<th bgcolor="#FFFACD">EMPLOYEE NAME</th>
		<th bgcolor="#FFFACD">CONVEYANCE TYPE</th>
		<th bgcolor="#FFFACD">AMOUNT</th>
		<th bgcolor="#FFFACD">REMARKS</th>
...
<?php 
	 ...
	 $data = "SELECT emp_code, conveyance_type,sum(amount) as totel_amount,remarks,conveyance_date FROM bills WHERE conveyance_date BETWEEN '".$_REQUEST['start_date']."' AND '".$_REQUEST['end_date']."' group by emp_code ";
...
<td><?=$sl++?></td>
<td><?=$row->emp_code;?></td>
<td><?=find_a_field('personnel_basic_info','PBI_NAME','PBI_CODE="'.$row->emp_code.'"')?></td>
<td><?=$row->conveyance_type;?></td>
<td><?=$row->totel_amount;?></td>

<td><?=$row->remarks;?></td>
<td><?=$row->conveyance_date;?></td>
