...
<tr>
  ...
  <th width="21%">Merchandiser Name</th>
  <!--<th>Zone</th>-->
</tr>
...
if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and c.chalan_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
    ...
    $res="select c.chalan_no, c.chalan_date,  m.do_no, m.job_no,  m.do_date, m.dealer_code, m.buyer_code, m.merchandizer_code,  d.dealer_name_e, c.entry_by from 
sale_do_master m, sale_do_chalan c, dealer_info d, user_group u 
where 

 m.group_for=u.id  and m.dealer_code=d.dealer_code and m.do_no=c.do_no ".$group_for_con.$con.$dealer_con."  group by c.chalan_no order by  c.chalan_date, m.do_no,c.chalan_no ";
