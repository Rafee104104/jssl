...
$lc_value = find_all_field_sql($vl_sql);
	 ...
	 $do_no= find_all_field('pi_details p,sale_do_details_foreign d','','p.do_no=d.do_no and p.pi_id="'.$lc_no.'"');
	 
	 
	if(isset($_POST['confirm'])){

	mysql_query('update sale_do_details_foreign set truck_receipt="'.$_POST['truck_amt'].'",truck_no="'.$_POST['truck_no'].'" where do_no="'.$do_no->do_no.'"');
 ...
 <tr> <td colspan="2"><table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr colspan="2" valign="top">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0"  style="font-size:14px">
	  ...
	  <? }?>
	  <? 
	  $lquery=mysql_query('select * from lc_export_contact where lc_no='.$lc_no);
	  while($ldata=mysql_fetch_object($lquery)){
	  ?> 
		  	<tr style="font-weight:bold;">
  ...
  <tr>
    ...
    <td colspan="2">
	 ...
	 $sqlc = 'select s.*, i.item_name, i.unit_name from lc_receive r, pi_details c, sale_do_details_foreign s, item_info i where r.pi_id=c.pi_id and i.item_id=s.item_id and c.do_no=s.do_no and r.lc_no='.$lc_no.' group by s.id order by s.id ';
