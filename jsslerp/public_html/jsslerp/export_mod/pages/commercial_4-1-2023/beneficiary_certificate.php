...
<table width="900" border="0" cellspacing="0" cellpadding="0" align="center">
 ...
 <tr> <td colspan="2"><table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0"  style="font-size:12px">
	...
	THIS IS TO CERTIFY THAT WE HAVE SUPPLIED CARTON TO OUR.  L/C No. 
	      <?=$lc_data->export_lc_no;?> 
	      Dated <?php echo date('d-m-Y',strtotime($lc_data->export_lc_date));?>.  <? if($lc_data->contact_no!="") {?> EXPORT CONTRACT NO.: <?=$lc_data->contact_no;?><? }?>
...
$a=0;
		 ...
		 WHERE a.lc_no=b.lc_no  and b.pi_id=c.pi_id and a.lc_no="'.$lc_no.'" GROUP by c.pi_id ';
			...
			$a++;
			if ($a>1) echo ', ';
echo $pi_data->pi_no.' DT. '.date('d-m-Y',strtotime($pi_data->pi_date));}?>. 
	      <? if($lc_data->importer_irc_no!="") {?> 
	      Importer IRC No: 
	      <?= $lc_data->importer_irc_no; ?>
	      <? }?>
	      <? if($lc_data->applicants_tin_no!="") {?>
	      , Applicant's TIN No: 
	      <?= $lc_data->applicants_tin_no; ?>
	      <? }?>
	      <? if($lc_data->applicants_bin_no!="") {?>
	      , Applicant's BIN No: 
	      <?= $lc_data->applicants_bin_no; ?>
	      <? }?>
	      <? if($lc_data->bangladesh_bank_dc_no!="") {?>
	      , Bangladesh Bank DC No: 
	      <?= $lc_data->bangladesh_bank_dc_no; ?>
	      <? }?>
	      <? if($lc_data->issuing_bank_bin_no!="") {?>
	      , Issuing Bank BIN No: 
	      <?= $lc_data->issuing_bank_bin_no; ?>.
	      <? }?>
	       BENEFICIARY'S BIN NO. 000073153-0403, H. S. CODE. 4819.10.00. TO 
	      <?=$dealer->dealer_name_e?>
	      . 
	      <?=$dealer->address_e?>
	      . AGAINST DELIVERY CHALLAN NO. 
	      <?=$lc_data->lc_no_view;?> 
	      DATED <?php echo date('d-m-Y',strtotime($lc_data->lc_date));?>. 
