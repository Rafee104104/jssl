...
WHERE h.emp_id=p.PBI_ID and p.employee_type = 'Non Roster' and `att_date` BETWEEN '".$start_date."' AND '".$end_date."'  And leave_id>0 
...
WHERE h.emp_id=p.PBI_ID and p.employee_type = 'Non Roster' and att_date=holy_day and `att_date` BETWEEN '".$start_date."' AND '".$end_date."'  And leave_id>'0'
group by emp_id";
...
FROM `hrm_att_summary` h,personnel_basic_info p
WHERE p.JOB_LOCATION in (1,68,70,88) 
and p.PBI_ID=h.emp_id 
and p.employee_type = 'Non Roster' and `att_date` BETWEEN '".$start_date."' 
