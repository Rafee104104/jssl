...
<?
		...
		$str   .='Date: '.$_POST['fdate'].'-'.$_POST['tdate'].'';
...
$new_date = date("Y-m-d",(strtotime($new_date)+2592000));
	...
	$full_query =mysql_query($full_sql);
	
	while($full_data=mysql_fetch_object($full_query)){
	...
	$half_query =mysql_query($half_sql);
	
	while($half_data=mysql_fetch_object($half_query)){
	...
	$adjust_query =mysql_query($adjust_sql);
	
	while($adjust_data=mysql_fetch_object($adjust_query)){
	...
	$late_query =mysql_query($late_sql);
	
	while($late_data=mysql_fetch_object($late_query)){
	...
	$lwp_query =mysql_query($lwp_sql);
	
	while($lwp_data=mysql_fetch_object($lwp_query)){
...
<table width="100%" cellspacing="0" cellpadding="2" border="0" style="margin:auto;"><thead><tr><th style="border:0px;" colspan="<?=6+($c*5)?>"><?=$str?></th></tr>
...
<tr>
  ...
  <tr>
<? for($p=1;$p<=$c;$p++){?><td colspan="5"><?=$period_name[$p]?></td><? } ?>
</tr>
	<tr>
<? for($p=1;$p<=$c;$p++){?>
	<td>Half Day</td>
...
$leave_query=mysql_query($leave_sql);

while($data_leave = mysql_fetch_object($leave_query)){
...
<tr>
	...
	<td style="background-color: aquamarine"><?=$mon_leave;  $all_month[$data_leave->PBI_ID] +=$mon_leave;  ?></td>
...
if($_POST['PBI_ID'] != ''){ $pbi_con = " and a.PBI_ID='".$_POST['PBI_ID']. "' "; } 
if($_POST['JOB_LOC_ID'] != ''){ $job_loc_con = " and a.JOB_LOC_ID='".$_POST['JOB_LOC_ID']. "' "; } 
...
$date_con = " and l.s_date>='".$_POST['fdate']."' and l.e_date<='".$_POST['tdate']."' ";
	 ...
	 $leave_sql="SELECT l.*,a.PBI_CODE,a.PBI_NAME as Employee_Name,(select DESG_DESC from designation where DESG_ID=a.DESG_ID) as designation,(select DEPT_DESC from department where DEPT_ID=a.DEPT_ID) as dept,s.leave_entry_at  FROM hrm_leave_info l, personnel_basic_info a,hrm_att_summary s where s.leave_id=l.id and a.PBI_ID=l.PBI_ID and l.half_or_full='Full' ".$con.$pbi_con.$date_con.$job_loc_con." group by s.leave_id ";
...
<td colspan="8"><div align="right"><b> Total Days</b></div></td>
...
<tbody>
<?
if($_POST['PBI_ID'] != ''){ $pbi_con = " and p.PBI_ID='".$_POST['PBI_ID']. "' "; } 
...
$leave_sql="SELECT l.*,a.PBI_NAME as Employee_Name,a.PBI_CODE,(select DEPT_DESC from department where DEPT_ID=a.DEPT_ID) as department  FROM hrm_att_summary l, personnel_basic_info a where   a.PBI_ID=l.emp_id ".$pbi_con.$date_con.$con." group by l.emp_id";
...
<td align="right"><b><?=$grand_tot_leave?></b></td>
<td align="right"><b><?=$hours = floor($grand_tot_leave / 60).':'.($grand_tot_leave -   floor($grand_tot_leave / 60) * 60);?></b></td>
...
<td align="right"><b><?=$grand_tot_leave?></td>
<td align="right"><b><?=$hours = floor($grand_tot_leave / 60).':'.($grand_tot_leave -   floor($grand_tot_leave / 60) * 60);?></td>
...
<tbody>
<?
if($_POST['PBI_ID'] != ''){ $pbi_con = " and a.PBI_CODE='".$_POST['PBI_ID']. "' "; } 

if($_POST['JOB_LOCATION'] != ''){ $job_con = " and a.JOB_LOC_ID='".$_POST['JOB_LOCATION']. "' "; }
if($_POST['year'] != ''){ 
$startDate = $_POST['year'].'-'.$_POST['mon'].'-01';
$endDate = $_POST['year'].'-'.$_POST['mon'].'-31';
 } 	
...
$date_con = " and l.xdate between '".$_POST['fdate']."' and '".$_POST['tdate']."' ";
  ...
  //$leave_sql="SELECT l.*,a.PBI_NAME as Employee_Name,a.PBI_CODE,(select DEPT_DESC from department where DEPT_ID=a.DEPT_ID) as department  FROM hrm_attdump l, personnel_basic_info a where   a.PBI_ID=l.EMP_CODE ".$pbi_con.$date_con.$con." order by l.xenrollid,l.xtime asc";
  
  
 
echo $sql="SELECT a.PBI_NAME,a.PBI_CODE as Employee_ID,a.PBI_ID,a.define_schedule,a.PBI_DEPARTMENT,i.type,i.s_date,i.e_date,i.s_time,i.e_time,i.total_days,i.reason,s.iom_entry_at
	 
	 FROM hrm_iom_info i, personnel_basic_info a,hrm_att_summary s where i.id=s.iom_id and a.PBI_ID=i.PBI_id and i.s_date>='".$_POST['fdate']."' and i.e_date<='".$_POST['tdate']."' ".$con.$pbi_con." order by i.id desc";
...
while($data = mysql_fetch_object($query)){
    ...
    $end_time = find_a_field('hrm_schedule_info','office_end_time','id='.$data->define_schedule);
	$start_time = find_a_field('hrm_schedule_info','office_start_time','id='.$data->define_schedule);
    
    $last_punch = find_a_field('hrm_attdump','time','bizid='.$data->Employee_ID.' and xdate="'.$data->s_date.'" order by sl desc');
	
    
    $first_punch = find_a_field('hrm_attdump','time','bizid='.$data->Employee_ID.' and xdate="'.$data->s_date.'" order by sl asc');
    
$e_time = strtotime($end_time);

$s_time = strtotime($start_time);
$l_punch = strtotime($last_punch);
$f_punch = strtotime($first_punch);
...
<tr>
	...
	<td><?=$difference; $tot_diff +=$difference;?></td>
	<td><?=date('H:i',mktime(0,$difference));?></td>
...
<? } ?>
	<tr>
		...
		<td><?=$tot_diff;?></td>
		<td><?=date('H:i',mktime(0,$tot_diff));?></td>
		<td colspan="3"></td>
	</tr>
...
<thead>
<tr><th style="border:0px;" colspan="21"><?=$str?></th></tr>
...
$basic=find_all_field('personnel_basic_info','',"PBI_ID=".$_POST['pbi_id_in']);?>

<div class="col-12">	


<div class="panel-heading"><h3 class="panel-title" align="center"><? //=find_a_field('user_group','group_name','id='.$_POST['PBI_ORG'])?></h3></div>
<table width="70%" border="1" align="center" cellpadding="0" cellspacing="0" eight="790" class="table table-bordered table-sm" style="border-color:#CCCCCC">
 <tbody><tr>
 <td width="8%"  align="center" valign="middle" bordercolor="#F0F0F0" bgcolor="#FFFFFF"><div align="center" class="style14">ID</div></td>
 <td width="8%"  align="center" valign="middle" bordercolor="#F0F0F0" bgcolor="#FFFFFF" style="font-size:15px"><div align="center" class="style14">Name</div></td>
 <td width="8%"  align="center" valign="middle" bordercolor="#F0F0F0" bgcolor="#FFFFFF" style="font-size:15px"><div align="center" class="style14">Designation</div></td>
 <td width="8%"  align="center" valign="middle" bordercolor="#F0F0F0" bgcolor="#FFFFFF"><div align="center" class="style14">Gender</div></td>
 <td width="8%"  align="center" valign="middle" bordercolor="#F0F0F0" bgcolor="#FFFFFF"><div align="center" class="style14">Department</div></td>
 </tr>

  <tr align="center" valign="middle" bgcolor="#FFFFFF">
  <td width="8%"><div align="center" class="style4"><?=$basic->PBI_ID?></div></td>

  <td width="8%"><div align="center" class="style4"><?=$basic->PBI_NAME?></div></td>
  <td width="8%"><div align="center" class="style4"><?=find_a_field('designation','DESG_DESC','DESG_ID='.$basic->DESG_ID);?></div></td>
  <td width="8%"><div align="center" class="style4"><?=$basic->PBI_SEX?></div></td>
  <td width="10%"><div align="center" class="style4"><?=find_a_field('department','DEPT_DESC','DEPT_ID='.$basic->DEPT_ID);?></div></td>
  </tr>



  <tr align="center" valign="middle" bgcolor="#FFFFFF">
  <td width="8%"><div align="center" class="style14">Joing Date</div></td>
  <td width="8%"><div align="center" class="style14">Appointment Date</div></td>
  <td width="8%">&nbsp;</td><td width="8%"><div align="center" class="style14">Project</div></td>
  <td width="8%"><div align="center" class="style14"></div>Service Length</td></tr> 

  <tr align="center" valign="middle" bgcolor="#FFFFFF"><td width="8%"><div align="center" class="style4"><?=$basic->PBI_DOJ?></div></td>
  <td width="8%"><div align="center" class="style4"><?=$basic->PBI_APPOINTMENT_LETTER_DATE?></div></td>
  <td width="8%">&nbsp;</td><td width="8%"><div align="center" class="style4"><?=find_a_field('project','PROJECT_DESC','PROJECT_ID='.$basic->JOB_LOCATION);?></div></td>
  <td width="8%"><div align="center" class="style4"><?=$basic->PBI_SERVICE_LENGTH?></div></td>
  </tr>
  </tbody></table>
...
$date_con = " and l.att_date between '".$_POST['fdate']."' and '".$_POST['tdate']."' ";
  ...
  FROM hrm_att_summary l, personnel_basic_info a where   a.PBI_ID=l.emp_id ".$pbi_con.$date_con.$con." order by l.emp_id,l.att_date asc";

$leave_query=mysql_query($leave_sql);


while($data_card = mysql_fetch_object($leave_query)){

	$emp_id = $data_card->emp_id;
...
<tr>
	...
	<td><?=$data_card->att_date?></td>

	<td align="right"><?=date('H:i:s',strtotime($data_card->in_time))?></td>

	<td align="right"><?=date('H:i:s',strtotime($data_card->out_time))?></td>
...
<? $final_data = find_all_field('hrm_attendence_final','',"PBI_ID=".$emp_id);?>
...
<tr>
	...
	<td><?=$final_data->pre?></td>

	<td align="right"><?=$final_data->ab?></td>

	<td align="right"><?=$final_data->od?></td>

	<td><?=$final_data->lv?></td>


	<td><?=$final_data->overtime_hour?></td>
...
<?
if($_POST['PBI_ID'] != ''){ $pbi_con = " and p.PBI_CODE='".$_POST['PBI_ID']. "' "; }
if($_POST['job_status'] != ''){ $status_con = " and a.PBI_JOB_STATUS='".$_POST['job_status']. "' "; }
...
$leave_query=mysql_query($leave_sql);
 ...
 while($t_data=mysql_fetch_object($t_query)){
...
<tr>
	...
	<td><?=$total_empp[$data_leave->DEPT_ID]; $total_emp +=$total_empp[$data_leave->DEPT_ID]; ?></td>
...
<table width="70%" cellspacing="0" cellpadding="2" border="0" style="margin:auto;"><thead>
	<tr><th style="border:0px;" colspan="21"><?=$str?></th></tr>
...
if($_POST['PBI_ORG'] != ''){ $org_con = " and a.PBI_ORG='".$_POST['PBI_ORG']. "' "; }
...

{
    ...
    $leave_sql="SELECT a.PBI_ID,a.PBI_CODE,a.PBI_NAME as Employee_Name,(select DEPT_DESC from department where DEPT_ID=a.DEPT_ID) as department,a.PBI_DOJ as joining_date  FROM personnel_basic_info a where 1 ".$con."";
...
$roster_query=mysql_query($roster_sql);


while($roster = mysql_fetch_object($roster_query)){
  ...
  <table width="100%" cellspacing="0" cellpadding="2" border="0">
    ...
    <thead>
      ...
      <tr>
        ...
        <td style="border:0px;" colspan="28"><div align="center" style="font-size:20px;">
...
?>
        ...
        <tr>
		  ...
		  <td align="right"><?=($data->conveyance>0)? $data->conveyance : '';     $totSpecial += $data->conveyance?></td>
		  ...
		  <td align="right"><?=($data->entertainment>0)? $data->entertainment : '';     $totEntertain += $data->entertainment?></td>
          ...
          <td align="right"><?=($data->other_allowance>0)? $data->other_allowance : '';     $totOthers += $data->other_allowance?></td>
		  ...
		  <td align="right"><?=($data->advance_install>0)? $data->advance_install : '';         $totAdvance_install += $data->advance_install?></td>
...
<table width="100%" cellspacing="0" cellpadding="2" border="1">
  ...
  <thead>
    ...
    <tr>
	  ...
	  Employee Name : <?=find_a_field('personnel_basic_info','PBI_NAME','PBI_ID='.$_POST['pbi_id_in']);?>
	...
	<tr style="border: 0;"><td colspan="13"  style="border: 0;">&nbsp;</td></tr>
...
if($_POST['pbi_id_in']!='') {$pbi_con = ' and j.PBI_ID="'.$_POST['pbi_id_in'].'"';};
...
?>
    ...
    <tr>
      ...
      <td rowspan="3"> <? $before_year=$data->year; echo --$before_year; echo "-".$data->year?></td>
	  ...
	  ?><? $my_before_year_stay = $my_grand_total+$my_year_tax1; echo number_format($my_before_year_stay,2);?></td>
      ...
      <td> <?
	   ...
	   echo number_format($my_advanced_gotten,2); ?> </td>
	  ...
	  $my_year_advance_given = find_a_field('salary_pl_journal','sum(amt_out)',' 1 and fascicle_year="'.$data->year.'" and PBI_ID="'.$data->PBI_ID.'" and tr_from in ("Loan","Loan Opening") '); echo number_format($my_year_advance_given,2);?>
      ...
      <td><? $my_total = (($my_before_year_stay+$my_year_tax+$my_advanced_gotten)-$my_year_advance_given); echo number_format($my_total,2);?></td>
	  ...
	  //$my_before_year_stay_profit = ((($my_before_year_stay/100)*$my_gov_given_percen_on_year)/12*6) ; echo number_format($my_before_year_stay_profit,2);
	   ...
	   $my_before_year_stay_profit = ((($my_before_year_stay/100)*$my_gov_given_percen_on_year)) ; echo number_format($my_before_year_stay_profit,2);
      ...
      <td><? $my_year_total_profit = ($my_before_year_stay_profit+$my_year_tax_on_profit); echo number_format($my_year_total_profit,2);?></td>
	  ...
	  <? $my_grand_total=($my_total+$my_year_total_profit); echo number_format($my_grand_total,2);?>
	...
	<tr style="color:red;">
      ...
      <td>
	   ...
	   <? $gov_year_gov_tax1 = find_a_field('salary_pl_journal','sum(amt_in)',' 1 and fascicle_year="'.$data->year.'" and PBI_ID="'.$data->PBI_ID.'" and tr_from in ("Gov Pf Opening Interest")'); ?>



	  <? $gov_before_year_stay = $gov_grand_total+$gov_year_gov_tax1 ; echo number_format($gov_before_year_stay,2);?>
	  ...
	  <? $gov_year_gov_tax = find_a_field('salary_pl_journal','sum(amt_in)',' 1 and fascicle_year="'.$data->year.'" and PBI_ID="'.$data->PBI_ID.'" and tr_from in ("Government")'); echo number_format($gov_year_gov_tax,2);?>
	  ...
	  <? $gov_advanced_gotten = 0; echo number_format($gov_advanced_gotten,2);?>
	  ...
	  <? $gov_year_advance_given = 0; echo number_format($gov_year_advance_given,2);?>
	  ...
	  <? $gov_total=(($gov_before_year_stay+$gov_year_gov_tax+$gov_advanced_gotten)-$gov_year_advance_given); echo number_format($gov_total,2);?>
	 ...
	 echo number_format($gov_year_tax_on_profit,2);
	  ...
	  <? $gov_year_total_profit = ($gov_before_year_stay_profit+$gov_year_tax_on_profit);echo number_format($gov_year_total_profit,2);?>
	  ...
	  <? $gov_grand_total=($gov_total+$gov_year_total_profit); echo number_format($gov_grand_total,2);?>
	...
	<tr>
      ...
      <td><b><? $grand_before_year_stay = ($my_before_year_stay+$gov_before_year_stay); echo number_format($grand_before_year_stay,2)?></b></td>



      <td><b><? $grand_year_tax = ($my_year_tax+$gov_year_gov_tax); echo number_format($grand_year_tax,2)?></b></td>



      <td><b><? $grand_advanced_gotten = ($my_advanced_gotten+$gov_advanced_gotten); echo number_format($grand_advanced_gotten,2)?></b></td>



      <td><b><? $grand_year_advance_given = ($my_year_advance_given+$gov_year_advance_given); echo number_format($grand_year_advance_given,2)?></b></td>



      <td><b> <? $grand_total = ($gov_total+$my_total); echo number_format($grand_total,2)?></b> </td>



      <td><b><? $grand_before_year_stay_profit = ($gov_before_year_stay_profit+$my_before_year_stay_profit); echo number_format($grand_before_year_stay_profit,2)?></b></td>



      <td><b><? $grand_year_tax_on_profit = ($gov_year_tax_on_profit+$my_year_tax_on_profit); echo number_format($grand_year_tax_on_profit,2)?></b></td>



      <td><b><? $grand_year_total_profit = ($gov_year_total_profit+$my_year_total_profit); echo number_format($grand_year_total_profit,2)?></b></td>



      <td><b><? $g_g_total = ($gov_grand_total+$my_grand_total); echo number_format($g_g_total,2)?></b></td>
...
<table  width="100%" cellspacing="0" cellpadding="2" border="0">
  ...
  <th>Signature</th>
    ...
    <tbody>
      ...
      <td align="center">ID: <strong><?=$data->PBI_CODE?></strong><br />
 ...
 <?=$data->PBI_DEPARTMENT?>
      ...
      <?=($data->total_salary-$data->total_benefits);?>  <? //=($data->total_salary + $data->mobile_allowance)?>
      ...
      <td align="center" style="text-align:right">Total Sal:<strong>
        ...
        <?=$data->total_salary?> <? //=($data->total_salary+$data->mobile_allowance)?>
       ...
       <?php /*?> <?=$data->total_benefits?><?php */?>
        ...
        <?=($data->total_salary-$data->total_deduction)?>  <? //+($data->total_benefits-$data->total_deduction)-=-=-=-= //=($data->total_salary+$data->mobile_allowance)+($data->total_benefits-$data->total_deduction)?>
...
<tr>
  ...
  <td style="border:0px;">&nbsp;<button type="button" id="print" onClick="hide();window.print();">Print</button></td>
	 ...
	 <tr style="line-height:0px;">
		...
		<td style="border:0px;" rowspan="5" align="right"><img src="../../../logo/tecno.png" style="height:110px; width:120px;"></td>
...
echo date_format($test, 'M-Y');?></span></td>
 ...
 echo date_format($test, 'M-Y');?>
  ...
  <table width="600" border="1" cellpadding="0" cellspacing="0" align="center" style="font-size:15px;" class="tabledesign">
	...
	<tr>
      ...
      <td align="right"><? if($paySlip->other_allowance>0) echo number_format($paySlip->other_allowance,0); else echo '-';?></td>
	...
	<tr>
      ...
      <td colspan="2" align="right"><? $total_gross = $gross+$arrear+$mobile_bill; if($total_gross>0) echo number_format($total_gross,2); else echo '-';?></td>
	...
	<tr>
      ...
      <td align="right"><? if($paySlip->absent_deduction>0) echo number_format($absent_deduction = $paySlip->absent_deduction,0); else echo '-';?></td>
	...
	<tr>
      ...
      <td align="right"><? if($paySlip->late_deduction>0) echo number_format($late_deduction = $paySlip->late_deduction,0); else echo '-';?></td>
	...
	<tr>
      ...
      <td align="right"><? if($paySlip->lwp_deduction>0) echo number_format($lwp_deduction = $paySlip->lwp_deduction,0); else echo '-';?></td>
	...
	<tr>
	  ...
	  <td align="right"><? if($paySlip->total_deduction>0) echo number_format($total_deduction = $paySlip->total_deduction,0); else echo '-';?></td>



   <!--   <td colspan="2" align="right"><? $total_deduction = $income_tax+$pf; if($total_deduction>0) echo number_format($total_deduction,2); else echo '-';?></td>-->
  ...
  <tr>
	...
	<td style="border:0px; font-size:15px;" align="right"><input type="text" value="<? if($paySlip->total_payable>0) echo number_format($total_payable = $paySlip->total_payable,0); else echo '-';?>" readonly style="text-align:right; height:30px; width:120px; font-weight:bold;"></td>
   ...
   <!-- <?
    ...
    <tr >
      ...
      <td colspan="2" align="center"><strong>
        ...
        <? $total_earning=$paySlip->gross_salary; echo number_format($total_earning,0);?>
    ...
    <tr >
      ...
      <td><strong>
        ...
        <? $net_pay=$total_earning-$total_deduction;echo number_format($net_pay,0);?>
...
<tr><th style="border:0px;" colspan="16">


<h2 align="center">Alin Food Products Ltd.</h2>
	<h2 align="center" >Salary & Allowance[<?=find_a_field('job_location_type','job_location_name','id="'.$_POST['JOB_LOCATION'].'"');?>]</h2>
	<h2 align="center" >For the Month of <?=date("F", strtotime($_POST['fdate']))?>-<?=date("Y", strtotime($_POST['fdate']))?></h2>
	<h4></h4>
...
$new_query=mysql_query($sql_bank);
...
while($bank_data = mysql_fetch_object($new_query)){
...
<tr>
   ...
   <td align="right"><strong><?=number_format($grand_total,0);?></strong></td>
...
<tr style="page-break-before:always">
  ...
  <td style="border-bottom:0px; border-left:0px; border-right:0px;"><strong>Date : <?=date('d-m-Y')?></strong><br><br>To<br>The Manager<br>Dutch-Bangla Bank Ltd.<br>Foreign Exchange Branch,<br>Motijheel, Dhaka-1000. <br><br><strong>Subject: Salary & allowance disbursement for the month of (<?=$month_name = date("F", mktime(0, 0, 0, $_POST['mon'], 10)).'-'.$_POST['year'];?>)</strong><br><br>Dear Sir,<br>Please transfer TK = <?=number_format($grand_total,0)?>/- <b>(In Word:<? echo convertNumberMhafuz($grand_total);?>)</b> to our following employees bank accounts by debiting our Current A/C No:<b>105.110.19663</b> in the name of Alin Food Products Ltd. maintained with you.<br><br>We have provided you the soft copy of data through e-mail from ID number:<u>account7@alinfoods.com.</u>Sender Name:Monindra Kumar Mondal affirm you that the soft copy of data is true and exact with hard copy of data submitted to you.For any deviation with soft copy and hardcopy,the contains in the soft copy will be treated as final.<br><br>For any query please contract with Mr.Monindra Kumar Modal;Mobile Number:01841918977<br><br><br>Thanking You,<br><br><br><br><br>Authorized Signature<br><br><br><br><br><br>Alin Food Products Ltd.</td>
	<tr>
	<td style="border-bottom:0px; border-left:0px; border-right:0px;float: right;margin-top:-130px;">
	
	Authorized Signature<br><br><br><br><br><br>
...
$new_query2=mysql_query($sql_cash);
...
while($cash_data = mysql_fetch_object($new_query2)){
...
<tr>
   ...
   <td align="right"><strong><?=number_format($grand_total_cash,0);?></strong></td>
...
elseif($_POST['report']==240322){
 ...
 $in_out_sql="select a.PBI_ID,h.in_time,h.out_time,h.att_date
   ...
   //$qrrry = mysql_query($in_out_sql);

   //$row_count = mysql_num_rows($qrrry);
 ...
 <table style="width: 100%; margin: 0 auto; font-size: 20px;text-align:center;" border="1" bordercolor="#FFFFFF">
   ...
   <tr>
     ...
     <td style="border:0px solid white;"><strong>ERP COM BD</strong></td>
   ...
   <tr>
     ...
     <td style="border:0px solid white;">Period : <?=date('M-Y',strtotime($fdate));?></td>
 ...
 <table style="width:100%;" cellpadding="0" cellspacing="0" border="1">
   ...
   $dept_qry = mysql_query($dept_sql);
   while($dept_data=mysql_fetch_object($dept_qry)){
       
 ?>

   <tr>
       <td style="border:0px; font-size:16px; font-weight:bold;" colspan="10"><div align="left"><?=$dept_data->DEPT_DESC?></div></td>
   </tr>
     </table>
...
while($summary=mysql_fetch_object($query)){
...
$h_query =mysql_query($h_sql);

while($h_data=mysql_fetch_object($h_query)){
...
while($leave_data=mysql_fetch_object($leave_query)){
$leave_name[$leave_data->id]=$leave_data->type;
}


 $dept_sql="select DISTINCT a.JOB_LOC_ID,a.PBI_ID,a.DEPT_ID,dept.DEPT_DESC from personnel_basic_info a,department dept   
where    dept.DEPT_ID=a.DEPT_ID ".$con.$job_status_con."  group by a.PBI_ID order by a.DEPT_ID asc";
...
if($att_date[$dept_data->PBI_ID][$i]!=$i){
$dayname = date('D',strtotime($i));
if($dayname=='Fri'){
echo $att_status = '<span style="color:blue; font-weight:bold;">WH</span>';
}else{
...
if($leave[$dept_data->PBI_ID][$i]>0){
	if($leave_type[$dept_data->PBI_ID][$i]=='Full'){	
		echo $att_status = '<span style="color:#ef0fb4; font-weight:bold;">Full</span><br>';
	}else{
		echo $att_status = '<span style="color:#ef0fb4; font-weight:bold;">HL</span><br>';
	}
...
if($in_time[$dept_data->PBI_ID][$i]!=''){

if($in_time[$dept_data->PBI_ID][$i]!='')
echo date('H:i',strtotime($in_time[$dept_data->PBI_ID][$i]));
echo '<br>';
if($out_time[$dept_data->PBI_ID][$i]!='')
...
if($_POST['JOB_LOCATION'] != ''){ $job_con = " and p.JOB_LOC_ID='".$_POST['JOB_LOCATION']. "' "; }

if($_POST['job_status'] != ''){ $job_con .= " and p.PBI_JOB_STATUS='".$_POST['job_status']. "' "; }
 ...
 <table style="width: auto; margin: 0 auto; font-size: 20px;text-align:center;" border="1" bordercolor="#FFFFFF">
   ...
   <tr>
     ...
     <td style="border:0px solid white;"><strong>ALIN FOOD PRODUCTS LTD.</strong></td>
...
$lwp_total = find_a_field('hrm_att_summary','sum(leave_duration)','leave_category="LWP (Leave Without Pay)" and att_date between "'.$fdate.'" and "'.$tdate.'" and emp_id="'.$r->PBI_ID.'"');
...
where a.DEPT_ID=".$dept_data->DEPT_ID." ".$con.$employee_con.$service_con." order by a.PBI_ID";
 ...
 $s = 0;
    ...
    ?>
<tr><td colspan="26">Total : <?=$total;?></td>
</tr>
