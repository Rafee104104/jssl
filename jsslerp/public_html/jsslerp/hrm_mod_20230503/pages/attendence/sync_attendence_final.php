...
$holy_final_sql = "update hrm_att_summary s,personnel_basic_info p  set s.holyday = 1 where p.PBI_ID=s.emp_id and p.JOB_LOC_ID=1 and  s.att_date='".$holy_data->holy_day."'";
...
and if(p.JOB_LOC_ID=1,h.holyday=0,h.holyday in (0,1)) and h.leave_type!='Full' and h.iom_type!='Full'   ".$emp_con.$ORG_con.$loc_con." and if(p.JOB_LOC_ID=1,h.dayname!='Friday',h.dayname!='')
...
if($sch_outs_datetime[$data->emp_id][$data->att_date]>$outs_datetime[$data->emp_id][$data->att_date])
