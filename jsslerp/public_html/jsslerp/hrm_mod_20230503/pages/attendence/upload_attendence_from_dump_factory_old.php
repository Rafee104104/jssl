...
d.xtime between   DATE_ADD(concat(a.roster_date,' ',s.office_start_time), INTERVAL ".min_in." HOUR) and DATE_ADD(concat(a.roster_date,' ',s.office_end_time), INTERVAL ".max_out." HOUR) ".$job_location_con.$emp_id_con."
...
group by a.PBI_ID,a.roster_date";
	...
	else  $value[$sl]['sch_in_time']  = $office_in_times  = $sch_start[$roster_assign[$data->PBI_ID][$data->roster_date]];
	
	if($roster_assign[$data->PBI_ID][$data->roster_date]<1)
	$value[$sl]['sch_out_time'] = $office_out_times = $sch_end[$data->shedule_1];
	else $value[$sl]['sch_out_time']  = $office_in_times  = $sch_end[$roster_assign[$data->PBI_ID][$data->roster_date]];
...
('".$value[$x]['emp_id']."','".$value[$x]['att_date']."', '".$datetime."', '".$_SESSION['user']['id']."', 
...
in_time="'.$value[$x]['in_time'].'", out_time="'.$value[$x]['out_time'].'", sch_in_time="'.$value[$x]['sch_in_time'].'",sch_out_time="'.$value[$x]['sch_out_time'].'" where  emp_id="'.$value[$x]['emp_id'].'" and att_date="'.$value[$x]['att_date'].'" ';
