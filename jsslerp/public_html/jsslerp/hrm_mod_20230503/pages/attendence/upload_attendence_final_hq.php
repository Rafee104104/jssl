...
//$sql ="SELECT h.emp_id,(count(1)-sum(h.leave_duration)) pre, sum(h.final_late_min) l_min,sum(h.final_late_status) l_status,sum(h.ot_final_min) as total_over_time,p.PBI_DOJ,p.PBI_ID,sum(h.final_early_status) as early_out,sum(h.final_late_status) as total_late,sum(final_late_min) as total_late_min,p.DEPT_ID FROM `hrm_att_summary` h,personnel_basic_info p WHERE 1 and p.PBI_ID=h.emp_id and h.att_date BETWEEN '" .$start_date ."' AND '" .$end_date ."' " .$emp_con .$ORG_con .$job_location_con." and if(p.DEPT_ID!=52,h.dayname!='Friday',h.dayname!='') group by h.emp_id";
 ...
 $sql ="SELECT h.emp_id,(count(1)-sum(h.leave_duration)) pre, sum(h.final_late_min) l_min,sum(h.final_late_status) l_status,sum(h.ot_final_hour) as total_over_time,p.PBI_DOJ,p.PBI_ID,sum(h.final_early_status) as early_out,sum(h.final_late_status) as total_late,sum(final_late_min) as total_late_min,p.DEPT_ID FROM `hrm_att_summary` h,personnel_basic_info p WHERE 1 and p.PBI_ID=h.emp_id and h.att_date BETWEEN '" .$start_date ."' AND '" .$end_date ."' " .$emp_con .$ORG_con .$job_location_con.
...
$holy_sql = mysql_query('select * from salary_holy_day where holy_day between "'.$start_date.'" and "'.$end_date.'"');
...
$values[$pi]['pay'] = ($values[$pi]['pre'] + $values[$pi]['lv'] + $values[$pi]['hd'] + $values[$pi]['od'])-($values[$pi]["lwp"]);



$values[$pi]['ab'] = (($values[$pi]['td'])-($values[$pi]['pre'] + $values[$pi]['lv'] + $values[$pi]['hd'] + $values[$pi]['od']));
...
(`mon`, `year`, `PBI_ID`, `td`, `od`, `hd`, `lt`, `total_late_min`, `eo`,`ab`, `lv`,`iom`,`lwp`, `pre`, `pay`,`ot`,`hourly_leave`, `entry_at`, `entry_by`) values



('" .$mon."','".$year."','" .$values[$y]["emp_id"] ."','" .$values[$y]["td"] ."','" .$values[$y]["od"] ."', '" .$values[$y]["hd"] ."','" .$values[$y]["lt"] ."','" .$values[$y]["total_late_min"] ."', '" .$values[$y]["early_out"] ."', '" .$values[$y]["ab"] ."','" .$values[$y]["lv"] ."','" .$values[$y]["iom"] ."','" .$values[$y]["lwp"] ."','" .$values[$y]["pre"] ."','" .$values[$y]["pay"] ."','" .$values[$y]["total_over_time"] ."','" .$values[$y]["hourly_leave"] ."','" .date("Y-m-d H:i:s")."','".$_SESSION["user"]["id"]."')";
