...
<table width="100%" class="oe_list_content">
  ...
  <tbody>
    ...
    <?		
		...
		if($_POST['department']>0)			$con .=' and a.PBI_DEPARTMENT='.$_POST['department'];
		...
		$sql = "select a.PBI_NAME,a.PBI_ID,a.PBI_DESIGNATION,a.PBI_DEPARTMENT,a.PBI_DOMAIN from 

		personnel_basic_info a

		where  1 ".$con."  and a.PBI_JOB_STATUS='In Service' order by a.PBI_DOMAIN,a.PBI_ID ";
    ...
    <tr style="font-size:10px; padding:3px; ">
      ...
      <td><?=$info->PBI_DOMAIN?></td>

      <? while (strtotime($rp2_date) <= strtotime($re_date)){ ?>

      <td><?=find_a_field('hrm_schedule_info','schedule_name','id='.$shedule[$info->PBI_ID][$rp2_date]);?></td>
