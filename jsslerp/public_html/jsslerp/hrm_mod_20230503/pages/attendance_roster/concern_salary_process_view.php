...
//$sql = "SELECT r.PBI_ID,count(1) off_day
//FROM hrm_roster_allocation r, hrm_att_summary2 h, hrm_schedule_info s
//WHERE r.shedule_1 = s.id and h.emp_id = r.PBI_ID and (h.iom_sl_no>0 OR h.leave_id>0) and s.schedule_type = 'Offday' and h.att_date = r.roster_date and r.roster_date BETWEEN '".$start_date."' AND '".$end_date."' 
//group by r.PBI_ID";
//	
//$query = mysql_query($sql);
//	while($data = mysql_fetch_object($query))
...
//$sql = "SELECT r.PBI_ID,count(1) off_day
//FROM hrm_roster_allocation r, hrm_att_summary2 h, hrm_schedule_info s
//WHERE r.shedule_1 = s.id and h.emp_id = r.PBI_ID and (h.iom_sl_no>0 OR h.leave_id>0) and s.schedule_type = 'Holiday' and h.att_date = r.roster_date and r.roster_date BETWEEN '".$start_date."' AND '".$end_date."' 
//group by r.PBI_ID";
//	
//$query = mysql_query($sql);
//	while($data = mysql_fetch_object($query))
//	{
//	$holyiomlv_in_month[$data->PBI_ID] = $data->off_day;
//	}													
...
$sch_query = mysql_query($sch_sql);
	while($sch_data = mysql_fetch_object($sch_query))
	{
...
//	echo '<br>1';
//	echo $data->pre.'<br>2';
//	echo $iom_in_month[$data->PBI_ID].'<br>3';
//	echo $offiomlv_in_month[$data->PBI_ID].'<br>4';
//	echo $holyiomlv_in_month[$data->PBI_ID].'<br>';
