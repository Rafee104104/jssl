...
?>
      ...
      <tr>
        ...
        <td><?  $dd = 'select s_date,e_date from hrm_leave_info where s_date>="'.$m_s_date.'" and PBI_ID="'.$data->PBI_ID.'" and leave_status="GRANTED" and type in (1)';				 				$dumping = mysql_query($dd);				$day_count = '';				$last_date = $_POST['year'].'-'.$_POST['mon'].'-'.'31';				while($leave_d = mysql_fetch_object($dumping)){								 $d = date_parse_from_format("Y-m-d", $leave_d->e_date);                  $next_m =  $d["month"];				  				   if($_POST['mon']<$next_m){				   				   $e_date = $last_date;				   				   }else{				   $e_date = $leave_d->e_date;				   }				$s_date = $leave_d->s_date;				$e_date = date('Y-m-d H:i:s', strtotime($e_date . ' +1 day'));						 $begin = new DateTime($s_date);        $end = new DateTime($e_date);
    ...
    foreach ($period as $dt) {	        $dt->format("l Y-m-d H:i:s\n");    $today = $dt->format("Y-m-d");    if($dt->format("l")!='Friday')    {       $found = 0;       $found = find_a_field('salary_holy_day','1',' holy_day="'.$today.'" ');       if($found==0)       $day_count++;	         }}  }  echo $day_count;?>
        ...
        <td><?  $dd = 'select s_date,e_date from hrm_leave_info where s_date>="'.$m_s_date.'" and PBI_ID="'.$data->PBI_ID.'" and leave_status="GRANTED" and type in (2)';				 				$dumping = mysql_query($dd);				$day_count = '';				$last_date = $_POST['year'].'-'.$_POST['mon'].'-'.'31';				while($leave_d = mysql_fetch_object($dumping)){								 $d = date_parse_from_format("Y-m-d", $leave_d->e_date);                  $next_m =  $d["month"];				  				   if($_POST['mon']<$next_m){				   				   $e_date = $last_date;				   				   }else{				   $e_date = $leave_d->e_date;				   }				   				   				   				 				$s_date = $leave_d->s_date;																				$e_date = date('Y-m-d H:i:s', strtotime($e_date . ' +1 day'));						 $begin = new DateTime($s_date);        $end = new DateTime($e_date);
    ...
    foreach ($period as $dt) {	        $dt->format("l Y-m-d H:i:s\n");    $today = $dt->format("Y-m-d");    if($dt->format("l")!='Friday')    {       $found = 0;       $found = find_a_field('salary_holy_day','1',' holy_day="'.$today.'" ');       if($found==0)       $day_count++;	         }}  }  echo $day_count;?></td>
        ...
        <td><?  $dd = 'select s_date,e_date from hrm_leave_info where s_date>="'.$m_s_date.'" and PBI_ID="'.$data->PBI_ID.'" and leave_status="GRANTED" and type in (3)';				 				$dumping = mysql_query($dd);				$day_count = '';				$last_date = $_POST['year'].'-'.$_POST['mon'].'-'.'31';				while($leave_d = mysql_fetch_object($dumping)){								 $d = date_parse_from_format("Y-m-d", $leave_d->e_date);                  $next_m =  $d["month"];				  				   if($_POST['mon']<$next_m){				   				   $e_date = $last_date;				   				   }else{				   $e_date = $leave_d->e_date;				   }     
